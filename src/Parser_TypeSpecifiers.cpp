...
bool appendTypeOnlyArgSpecifiers(
	std::span<const ASTNode> argument_nodes,
	std::vector<TypeSpecifierNode>& specifiers);

std::optional<std::vector<TypeSpecifierNode>> collectTypeOnlyArgSpecifiers(
	std::span<const ASTNode> argument_nodes) {
	std::vector<TypeSpecifierNode> specifiers;
	if (!appendTypeOnlyArgSpecifiers(argument_nodes, specifiers)) {
		return std::nullopt;
	}
	return specifiers;
}
...
bool appendTypeOnlyArgSpecifiers(
	std::span<const ASTNode> argument_nodes,
	std::vector<TypeSpecifierNode>& specifiers) {
	for (const ASTNode& argument_node : argument_nodes) {
		if (!argument_node.is<TypeSpecifierNode>()) {
			return false;
		}
	}
	specifiers.reserve(specifiers.size() + argument_nodes.size());
	for (const ASTNode& argument_node : argument_nodes) {
		specifiers.push_back(argument_node.as<TypeSpecifierNode>());
	}
	return true;
}
...
void applyCollectedClassTemplateArgSpecs(
	...
	if (!has_non_type_or_template) {
		...
		for (auto& arg : collected.args) {
			type_args.push_back(std::get<TypeSpecifierNode>(std::move(arg)));
		}
		type_spec.set_template_specialization(collected.primary, std::move(type_args));
		return;
	}
...
ParseResult Parser::parse_type_specifier() {
	...
	auto diagnoseMissingTypenameForDependentOwner =
		...
		if (saw_typename_keyword ||
			...
			parsing_implicit_typename_context_ ||
	...
	if (has_explicit_type) {
		...
		if (long_count == 1) {
			if (type_cat == TypeCategory::Double) {
				...
				type_size = get_type_size_bits(TypeCategory::Double);
	...
	// Note: __builtin_va_list/__gnuc_va_list are identifiers, not keywords — string compare required
		...
		if (is_replaying_published_function_template_body_ &&
			...
			if (owner_index.has_value() &&
				...
				std::vector<TypeSpecifierNode> member_template_arg_syntax;
				...
				if (!appendTypeOnlyArgSpecifiers(
						argument_syntax_nodes, member_template_arg_syntax)) {
				...
				while (peek() == "::"_tok) {
					...
					if (peek() == "<"_tok) {
						...
						if (!appendTypeOnlyArgSpecifiers(
								nested_argument_syntax_nodes, member_template_arg_syntax)) {
							can_stamp_member_chain = false;
						}
					...
					auto make_alias_target_dependent_record =
						...
						auto classify_current_instantiation_owner =
							[&](TypeIndex current_owner_type_index,
								...
								TemplateArgumentVector alias_target_args =
			...
			if (template_args.has_value()) {
				...
				if (alias_opt.has_value()) {
					...
					if (!has_dependent_alias_args &&
						...
						if (!materialized_alias.instantiated_name.empty()) {
							FLASH_LOG(Parser, Debug, "Deferred instantiation: type '", materialized_alias.instantiated_name,
									  "' not found after materialization at line ", type_name_token.line());
						}
			...
			if (type_it != getTypesByNameMap().end()) {
				...
				if (template_opt.has_value() && template_opt->is<TemplateClassDeclarationNode>()) {
					...
					for (size_t i = filled_template_args.size(); i < template_params.size(); ++i) {
						...
						TemplateArgumentVector inline_filled_args = filled_template_args;
				...
				if (peek() == "::"_tok) {
					...
					auto create_dependent_qualified_type_placeholder =
						...
						if (const TypeInfo::DependentQualifiedNameRecord* dependent_name =
							...
							for (size_t member_index = 0;
								...
								const std::vector<TypeSpecifierNode>& type_args =
									*all_segment_infos[member_index].type_arg_syntax;
								member_template_arg_syntax.insert(
									member_template_arg_syntax.end(),
									type_args.begin(),
									type_args.end());
							}
					...
					if (has_template_args) {
						...
						StringHandle member_alias_name =
							StringTable::getOrInternStringHandle(qualified_type_name);
						...
						if (parent_type_it != getTypesByNameMap().end() &&
							...
							if (member_alias_opt.has_value()) {
								member_alias_name =
									StringTable::getOrInternStringHandle(template_member_name);
							}
						}
						...
						if (!member_alias_opt.has_value()) {
							...
							if (pattern_name_opt.has_value()) {
								...
								if (member_alias_opt.has_value()) {
									member_alias_name =
										StringTable::getOrInternStringHandle(pattern_member_alias_name);
								}
						...
						if (!member_alias_opt.has_value()) {
							...
							while (!member_alias_opt.has_value() && !base_template_name.empty()) {
								...
								if (member_alias_opt.has_value()) {
									member_alias_name =
										StringTable::getOrInternStringHandle(base_member_alias_name);
								}
								base_builder.reset();
							}
						}
						...
						if (member_alias_opt.has_value()) {
							...
							if (!member_template_args.has_value()) {
								return ParseResult::error(
									std::string(
										StringBuilder()
											.append("Failed to parse template arguments for member template alias: ")
							...
							AliasTemplateMaterializationResult materialized_alias =
								materializeAliasTemplateInstantiation(
									StringTable::getStringView(member_alias_name),
		...
		if (template_opt.has_value() && template_opt->is<TemplateClassDeclarationNode>()) {
			...
			if (all_have_defaults) {
				...
				for (size_t i = 0; i < template_params.size(); ++i) {
					...
					const ASTNode& default_node = param->default_value();
					TemplateArgumentVector inline_filled_args = toInlineTemplateArgs(filled_template_args);
					if (param->kind() == TemplateParameterKind::Type && default_node.is<TypeSpecifierNode>()) {
						ASTNode substituted_default_node = default_node;
						if (!inline_filled_args.empty()) {
							substituted_default_node = substituteTemplateParameters(default_node, template_params, inline_filled_args);
		...
		auto tryStructScopedType = [&]() -> const TypeInfo* {
			...
			for (auto it = struct_parsing_context_stack_.rbegin();
				...
				if (it->local_struct_info && !it->local_struct_info->base_classes.empty()) {
					...
					const TypeInfo* inherited = lookup_inherited_type_alias(
						struct_name_handle, type_name_handle);
					if (inherited)
						return inherited;
				}
			}
...
ParseResult Parser::parse_decltype_specifier() {
	...
	if (keyword == "decltype" && peek() == "auto"_tok) {
		...
		TypeSpecifierNode decltype_auto_type(TypeCategory::DeclTypeAuto, TypeQualifier::None, 0, decltype_token, CVQualifier::None);
...
void Parser::tryStampTypeOnlyClassTemplateSpecialization(
	...
	applyCollectedClassTemplateArgSpecs(type_spec, std::move(*collected));
}
...
void Parser::stampDependentMemberChainFromQualifier(
	...
	size_t member_template_arg_offset = 0;
	for (size_t member_index = 0; member_index < record.member_chain.size(); ++member_index) {
		...
		if (!member.has_template_arguments) {
			// ::template without <> is only a parse disambiguator.
			qualifier = table.dependentName(qualifier, member_name);
			continue;
		}
		const size_t member_template_arg_count = member.template_arguments.size();
		if (member_template_arg_offset > member_template_arg_syntax.size() ||
			member_template_arg_count >
			member_template_arg_syntax.size() - member_template_arg_offset) {
			return;
		}
		const std::span<const TypeSpecifierNode> arg_specs =
			member_template_arg_syntax.subspan(
				member_template_arg_offset, member_template_arg_count);
		member_template_arg_offset += member_template_arg_count;
		std::vector<TypeId> argument_ids;
		argument_ids.reserve(arg_specs.size());
		for (const TypeSpecifierNode& arg_spec : arg_specs) {
...
void Parser::tryStampDependentInstantiationMemberChain(
	...
	argument_ids.reserve(collected->args.size());
	for (const auto& arg : collected->args) {
		if (const TypeSpecifierNode* type_arg = std::get_if<TypeSpecifierNode>(&arg)) {
...
void Parser::tryStampTypeParamOwnedMemberTemplateId(
	...
	const size_t first_scope = qualified_name.find("::");
	if (first_scope == std::string_view::npos) {
		return;
	}
	StringHandle owner_param_name =
		StringTable::getOrInternStringHandle(qualified_name.substr(0, first_scope));
	const auto owner_index = current_template_params_.indexOf(owner_param_name);
	if (!owner_index.has_value()) {
		return;
	}
	const auto owner_kind = currentTemplateParamKind(owner_param_name);
	if (owner_kind.has_value() && *owner_kind != TemplateParameterKind::Type) {
		return;
	}
	auto last_args = collectTypeOnlyArgSpecifiers(last_member_arg_nodes);
	if (!last_args.has_value()) {
		return;
	}
	TemplateParamNameViewVector components =
		splitDependentMemberPathComponents(qualified_name.substr(first_scope + 2));
	if (components.empty()) {
		return;
	}
	if (trailing_members != nullptr) {
		for (const TypeInfo::DependentQualifiedNameRecord::Member& member :
			 trailing_members->member_chain) {
			if (member.has_template_arguments) {
				return;
			}
		}
	}
	TypeInfo::DependentQualifiedNameRecord stamp_record;
	stamp_record.owner_kind =
		TypeInfo::DependentQualifiedNameRecord::OwnerKind::TemplateParameter;
	stamp_record.owner_name = owner_param_name;
	std::vector<TypeSpecifierNode> member_template_arg_syntax;
	for (size_t index = 0; index < components.size(); ++index) {
		TypeInfo::DependentQualifiedNameRecord::Member member;
		member.name = StringTable::getOrInternStringHandle(components[index]);
		if (index + 1 == components.size()) {
			member.has_template_arguments = true;
			stamp_record.member_chain.push_back(std::move(member));
			member_template_arg_syntax.insert(
				member_template_arg_syntax.end(), last_args->begin(), last_args->end());
			continue;
		}
		stamp_record.member_chain.push_back(std::move(member));
	}
	if (trailing_members != nullptr) {
		for (const TypeInfo::DependentQualifiedNameRecord::Member& member :
			 trailing_members->member_chain) {
			stamp_record.member_chain.push_back(member);
		}
	}
	tryStampDependentMemberChain(
		type_spec, owner_param_name, stamp_record, member_template_arg_syntax);
}
