...
#include "IrGenerator.h"
			...
			if (type_spec.size_in_bits() == 0 && type_spec.token().type() == Token::Type::Identifier) {
				...
				auto sep_pos = type_name.rfind("::");
					if (sep_pos != std::string_view::npos) {
						...
						if (member_size > 0) {
							return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(member_size)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
						}
					}
				}
				...
				if (decl) {
					...
					if (array_size.has_value()) {
						...
						return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(*array_size)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
					}
				}
				...
				if (!decl && !lookupSymbol(identifier).has_value() && current_struct_name_.isValid()) {
					...
					if (param_size_bytes > 0) {
						return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(param_size_bytes)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
					}
				}
			}
					...
					if (var_type.type() == Type::Struct) {
						...
						if (type_index < gTypeInfo.size()) {
							...
							if (struct_info && struct_info->total_size > 0) {
								return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(struct_info->total_size)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
							}
							// Fallback: use type_size_ from TypeInfo (works for template instantiations at global scope)
							if (type_info.type_size_ > 0) {
								return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(type_info.type_size_)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
							}
						}
						// Fallback: use size_in_bits from the type specifier node
						if (var_type.size_in_bits() > 0) {
							return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(var_type.size_in_bits() / 8)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
						}
					} else {
						...
						return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(size_in_bytes)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
					}
				}
			}
							...
							if (obj_type.type() == Type::Struct) {
								...
								if (type_index < gTypeInfo.size()) {
									...
									if (direct_member_size > 1) {
										...
										return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(direct_member_size)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
									}
											...
											if (inst_struct_info && !inst_struct_info->members.empty()) {
												for (const auto& member : inst_struct_info->members) {
													if (StringTable::getStringView(member.getName()) == member_name) {
														...
														return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(member.size)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
													}
												}
											}
										}
					...
					if (decl) {
						...
						if (var_type.type() == Type::Struct) {
							...
							if (type_index < gTypeInfo.size()) {
								...
								const StructTypeInfo* struct_info = type_info.getStructInfo();
								if (struct_info) {
									return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(struct_info->alignment)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
								}
							}
						} else {
							// Primitive type - use get_type_size_bits to handle cases where size_in_bits wasn't set
							...
							return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(alignment)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
						}
					}
				}
			}
	...
	ExprResult AstToIr::generateOffsetofIr(const OffsetofExprNode& offsetofNode) {
		...
		return withStorage(makeExprResult(Type::UnsignedLongLong, SizeInBits{64}, IrOperand{static_cast<unsigned long long>(path_result.total_offset)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
	}
	...
	ExprResult AstToIr::generateTypeTraitIr(const TypeTraitExprNode& traitNode) {
		...
		if (traitNode.is_no_arg_trait()) {
			...
			return withStorage(makeExprResult(Type::Bool, SizeInBits{8}, IrOperand{static_cast<unsigned long long>(result ? 1ULL : 0ULL)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
		}
			...
			case TypeTraitKind::UnderlyingType:
				...
				type_spec.type_index().value < gTypeInfo.size()) {
					...
					if (enum_info) {
						...
						return withStorage(makeExprResult(enum_info->underlying_type, SizeInBits{enum_info->underlying_size}, IrOperand{0ULL}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
					}
					// Fallback to int if no enum info
					return withStorage(makeExprResult(Type::Int, SizeInBits{32}, IrOperand{0ULL}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
				}
...
std::optional<ExprResult> AstToIr::emitConversionOperatorCall(
	...
	return withStorage(makeExprResult(target_type, SizeInBits{target_size_bits}, IrOperand{result_var}, target_type_index, PointerDepth{}), ValueStorage::ContainsData);
}
