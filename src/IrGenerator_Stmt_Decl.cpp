...
bool AstToIr::isSameTypeXValueSource(const ASTNode& init_node, const ExprResult& init_operands, const TypeSpecifierNode& target_type) const {
	...
	return init_operands.type_index.isStruct()
	...
	void AstToIr::visitVariableDeclarationNode(const ASTNode& ast_node) {
		...
		if (is_global || is_static_local) {
			...
			auto packStructEvalResultIntoInitData = [&](auto& self, std::vector<char>& init_data, const StructTypeInfo& struct_info, const ConstExpr::EvalResult& eval_result, size_t base_offset, size_t depth) -> void {
				...
				for (const auto& member : struct_info.members) {
						...
						if (!member_result.array_elements.empty()) {
							for (size_t elem_i = 0; elem_i < member_result.array_elements.size() && elem_i < total_elements; ++elem_i) {
								writeRawValueAtOffset(
									...
									element_size,
									evalResultMemberToRaw(member_result.array_elements[elem_i], categoryToType(member.type_index.category())));
							}
						} else {
					...
					unsigned long long value = evalResultMemberToRaw(member_result, categoryToType(member.type_index.category()));
			...
			if (node.initializer()) {
				...
				if (const LambdaExpressionNode* lambda_ptr = extractLambdaFromInitializer(init_node)) {
					...
					if (closure_type.has_value() && lambda_ptr->captures().empty()) {
						...
						op.type_index = closure_type->type_index();
								...
								for (const auto& member : si->members) {
									...
									for (const auto& mem_init : matching_ctor->member_initializers()) {
										if (mem_init.member_name == StringTable::getStringView(member.getName())) {
											...
											if (eval_r.success()) member_val = evalResultMemberToRaw(eval_r, categoryToType(member.type_index.category()));
											break;
										}
									}
			...
			if (is_static_local) {
				...
				info.type_index = type_node.type_index();
			...
			if (is_function_call) {
				...
				if (eval_result.success()) {
					...
					if (const auto* b_val = std::get_if<bool>(&eval_result.value)) {
						decl_op.initializer = makeTypedValue(type_node.type_index(), decl_op.size_in_bits, static_cast<unsigned long long>(*b_val ? 1 : 0));
					} else if (const auto* ll_val = std::get_if<long long>(&eval_result.value)) {
						decl_op.initializer = makeTypedValue(type_node.type_index(), decl_op.size_in_bits, static_cast<unsigned long long>(*ll_val));
					} else if (const auto* ull_val = std::get_if<unsigned long long>(&eval_result.value)) {
						decl_op.initializer = makeTypedValue(type_node.type_index(), decl_op.size_in_bits, *ull_val);
					} else if (std::holds_alternative<double>(eval_result.value)) {
						...
						if (type_node.type() == Type::Float) {
							...
							decl_op.initializer = makeTypedValue(TypeIndex{0, TypeCategory::Float}, SizeInBits{32}, static_cast<unsigned long long>(bits));
						} else {
							unsigned long long bits;
							std::memcpy(&bits, &d, sizeof(double));
							decl_op.initializer = makeTypedValue(TypeIndex{0, TypeCategory::Double}, SizeInBits{64}, bits);
						}
					}
		...
		auto appendExprResultToOperands = [&](const ExprResult& result) {
			...
			operands.emplace_back(static_cast<int>(static_cast<int>(result.type_index.category())));
		...
		if (node.initializer() && !decl.is_array()) {
			...
			std::holds_alternative<LambdaExpressionNode>(init_node.as<ExpressionNode>())) {
				...
				if (!is_copy_init_for_struct) {
					...
					{
						Type init_type = categoryToType(init_operands.type_index.category());
					...
					// infer (e.g., complex sub-expressions or function-call result values).
					{
						...
						if (init_type == Type::Enum && init_operands.type_index.is_valid()
							...
							if (const EnumTypeInfo* enum_info = getTypeInfo(init_operands.type_index).getEnumInfo()) {
								init_type = categoryToType(enum_info->underlying_type);
								}
						}
						if (init_type != decl_type
							...
							if (!sema_applied) {
								const TypeConversionResult conv =
									can_convert_type(typeToCategory(init_type), typeToCategory(decl_type));
		...
		if (node.initializer() && !decl.is_array() && operands.size() >= 11) {
			...
			std::holds_alternative<TempVar>(operands[9])) {
				...
				lvalue_info_opt->array_index.has_value()) {
					...
					addr_op.element_type_index = elem_type;
		...
		if (decl.is_array() && node.initializer().has_value()) {
			...
			if (init_node.is<InitializerListNode>()) {
				...
				for (size_t i = 0; i < initializers.size(); i++) {
					...
					store_op.index = makeTypedValue(TypeIndex{0, TypeCategory::Int}, SizeInBits{32}, static_cast<unsigned long long>(i));
								...
								if (std::holds_alternative<IdentifierNode>(init_node.as<ExpressionNode>())) {
									...
									if (init_decl) {
										...
										if (init_type.is_reference() || init_type.is_rvalue_reference()) {
											...
											init_arg.type_index = init_type.type_index();
											init_arg.ir_type = toIrType(init_type.type_index());
	...
	void AstToIr::visitStructuredBindingNode(const ASTNode& ast_node) {
		...
		bool is_array = false;
		size_t array_size = 0;
		TypeIndex array_element_type = init_type_index.is_valid() ? init_type_index : TypeIndex{0, typeToCategory(init_type)};
					...
					if (symbol->is<DeclarationNode>()) {
						...
						if (decl.is_array() && decl.array_size().has_value()) {
							...
							if (eval_result.success()) {
								...
								array_element_type = type_spec.type_index();
								...
								" element_type=", (int)array_element_type.category(), " element_size=", array_element_size);
							}
						}
					}
		...
		if (is_array) {
			hidden_decl_op.type_index = array_element_type;
			hidden_decl_op.size_in_bits = SizeInBits{static_cast<int>(array_element_size)};
			hidden_decl_op.is_array = true;
			hidden_decl_op.array_element_type_index = array_element_type;
			...
			hidden_decl_op.type_index = init_type_index.is_valid() ? init_type_index : TypeIndex{0, typeToCategory(init_type)};
			...
			if (initializer.is<ExpressionNode>()) {
				...
				if (std::holds_alternative<IdentifierNode>(expr_node)) {
					...
					hidden_decl_op.initializer = withStorage(makeTypedValue(hidden_decl_op.type_index, SizeInBits{64}, addr_temp), ValueStorage::ContainsAddress);
				} else {
		...
		if (is_array && initializer.is<ExpressionNode>()) {
			...
			if (std::holds_alternative<IdentifierNode>(expr_node)) {
				...
				for (size_t i = 0; i < array_size; ++i) {
					...
					access_op.index = makeTypedValue(TypeIndex{0, TypeCategory::Int}, SizeInBits{32}, static_cast<unsigned long long>(i));
					access_op.element_type_index = array_element_type;
					...
					ArrayStoreOp store_op;
					store_op.element_type_index = array_element_type;
		...
		if (is_array) {
			...
			for (size_t i = 0; i < array_size; ++i) {
				...
				TypeSpecifierNode binding_type(categoryToType(array_element_type.category()), TypeQualifier::None,
				...
				if (is_reference_binding) {
					...
					addr_op.index = makeTypedValue(TypeIndex{0, TypeCategory::Int}, SizeInBits{32}, static_cast<unsigned long long>(i));
					addr_op.element_type_index = array_element_type;
					...
					binding_var_decl.type_index = array_element_type;
					...
					load_op.index = makeTypedValue(TypeIndex{0, TypeCategory::Int}, SizeInBits{32}, static_cast<unsigned long long>(i));
					load_op.element_type_index = array_element_type;
		...
		if (tuple_size_it != getTypesByNameMap().end()) {
			...
			if (!found_value) {
				...
				std::vector<std::tuple<StringHandle, TypeIndex, int>> binding_info;  // (mangled_name, return_type_index, return_size_bits) for each get<N>
				...
				for (size_t i = 0; i < tuple_size_value && all_get_found; ++i) {
					...
					if (type_alias_it != getTypesByNameMap().end()) {
						...
						element_type = categoryToType(type_alias_info->category());
						element_type_index = type_alias_info->type_index_;
						element_size = type_alias_info->type_size_;
						if (element_size == 0) {
							element_size = element_type_index.is_valid() ? static_cast<int>(get_type_size_bits(element_type_index)) : get_type_size_bits(element_type);
						}
					...
					if (get_spec.has_value() && get_spec->is<FunctionDeclarationNode>()) {
						...
						binding_info.push_back({mangled_handle, element_type_index.is_valid() ? element_type_index : TypeIndex{0, typeToCategory(element_type)}, element_size});
				...
				if (all_get_found && binding_info.size() == tuple_size_value) {
					...
					for (size_t i = 0; i < tuple_size_value; ++i) {
						...
						auto [get_mangled_name, element_type_index, element_size] = binding_info[i];

						Type element_type = categoryToType(element_type_index.category());
						...
						VariableDeclOp binding_var_decl;
						binding_var_decl.var_name = binding_id;
						binding_var_decl.type_index = element_type_index;
						...
						TypeSpecifierNode binding_type(element_type_index.is_valid() ? categoryToType(element_type_index.category()) : element_type, TypeQualifier::None,
		...
		for (const auto& member : struct_info->members) {
			...
			TypeSpecifierNode binding_type(categoryToType(member.type_index.category()), TypeQualifier::None, member_size_bits, Token());
			...
			if (is_reference_binding) {
				...
				load_op.result.ir_type = toIrType(member.type_index);
