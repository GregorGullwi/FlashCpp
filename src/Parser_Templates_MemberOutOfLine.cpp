		...
		if (past_scope_op) {
			...
			if (peek().is_identifier() && peek_info().value() == potential_class.value()) {
					...
					if (peek() == "("_tok) {
						...
						auto void_type = emplace_node<TypeSpecifierNode>(TypeCategory::Void, TypeQualifier::None, 0, ctor_name_token);
...
std::optional<ASTNode> Parser::parseTemplateBody(
	...
	bool setup_member_context = struct_name.isValid() && struct_type_index.is_valid();
