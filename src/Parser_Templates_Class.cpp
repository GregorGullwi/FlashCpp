...
ParseResult Parser::parse_template_declaration() {
	...
	if (is_concept_template) {
		...
		if (is_specialization) {
			...
			struct_info_ptr->deferred_base_template_names.clear();
			struct_info_ptr->deferred_template_base_specs.clear();
			for (const auto& deferred_base : struct_ref.deferred_template_base_classes()) {
				struct_info_ptr->deferred_base_template_names.push_back(deferred_base.base_template_name);
				struct_info_ptr->deferred_template_base_specs.push_back(deferred_base);
			}
