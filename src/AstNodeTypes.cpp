...
std::string TypeSpecifierNode::getReadableString() const {
	...
	if (!name.empty()) {
		...
		case TypeCategory::TypeAlias:
			...
			break;
		case TypeCategory::FunctionPointer:
			oss << "function_pointer";
			break;
		case TypeCategory::MemberFunctionPointer:
			oss << "member_function_pointer";
			break;
		case TypeCategory::MemberObjectPointer:
			oss << "member_object_pointer";
			break;
		case TypeCategory::Nullptr:
			oss << "nullptr_t";
			break;
...
void StructTypeInfo::propagateAstProperties(StructMemberFunction& mf) {
	...
	if (const auto* fn = get_function_decl_node(mf.function_decl)) {
		...
		if (is_c && is_v)
			mf.cv_qualifier = CVQualifier::ConstVolatile;
		else if (is_c)
			mf.cv_qualifier = CVQualifier::Const;
		else if (is_v)
...
const StructTypeInfo* getBaseStructInfo(const BaseClassSpecifier& base) {
	if (const TypeInfo* base_type = tryGetTypeInfo(base.type_index)) {
		return base_type->getStructInfo();
	}
	return nullptr;
}
...
LayoutBaseCollections collectLayoutBaseCollections(StructTypeInfo& struct_info) {
	...
	auto collectVirtualBases = [&](auto&& self, const StructTypeInfo* current_struct_info) -> void {
	...
	collectVirtualBases(collectVirtualBases, &struct_info);
	return collections;
}
...
void StructTypeInfo::recalculateLayout() {
	...
	const bool has_virtual_members = structIntroducesVirtualMembers(*this);
...
bool StructTypeInfo::hasLeadingEmptySubobjectType(TypeIndex type_index) const {
	return collectLeadingEmptySubobjectTypes(this).contains(type_index);
}
...
bool StructTypeInfo::hasEmptySubobjectTypeAtOffset(TypeIndex type_index, size_t offset) const {
	for (const auto& base : base_classes) {
		...
		if (const StructTypeInfo* base_info = getBaseStructInfo(base);
			base_info && base_info->hasLeadingEmptySubobjectType(type_index)) {
			return true;
		}
	}
	...
	for (const auto& member : members) {
		...
		if (const StructTypeInfo* member_struct_info = tryGetStructTypeInfo(member.type_index);
			member_struct_info && member_struct_info->hasLeadingEmptySubobjectType(type_index)) {
			return true;
		}
	}
