...
void StructTypeInfo::buildVTable() {
    ...
    for (auto& func : member_functions) {
        ...
        if (override_index >= 0) {
            ...
            vtable[static_cast<size_t>(override_index)] = &func;
...
void StructTypeInfo::buildRTTI() {
    ...
    size_t mangled_total_size = length_digits + name_sv.length() + 1;
    
    // Allocate permanent storage for the name string (persists for program lifetime)
    char* itanium_name = (char*)malloc(mangled_total_size);
    if (!itanium_name) {
    ...
    int written = snprintf(itanium_name, mangled_total_size, "%zu%.*s", 
                          name_sv.length(), 
                          static_cast<int>(name_sv.length()), 
                          name_sv.data());
    if (written < 0 || static_cast<size_t>(written) >= mangled_total_size) {
        free(itanium_name);
        return;
    }
