...
ParseResult Parser::validate_and_add_base_class(
	...
	if (base_type_it == getTypesByNameMap().end()) {
		...
		if (!qualified_namespace.empty()) {
			...
			base_type_it = getTypesByNameMap().find(StringTable::getOrInternStringHandle(qualified_name_view));
			if (base_type_it != getTypesByNameMap().end()) {
			...
			for (size_t pos = qualified_namespace.find("::");
				 pos != std::string_view::npos && base_type_it == getTypesByNameMap().end();
...
TypeIndex Parser::substitute_template_parameter(
	...
	auto lookupTypeInfoByName = [&](std::string_view name) -> const TypeInfo* {
		if (name.empty()) {
			return nullptr;
		}
	...
	auto tryResolveConcreteTemplatePlaceholder = [&]() -> bool {
		...
		auto resolvePlaceholderMemberChain =
			[&](std::string_view concrete_instantiated_name,
				const TypeInfo* concrete_instantiated_type_info) -> const TypeInfo* {
			if (concrete_instantiated_type_info == nullptr) {
				return nullptr;
			}

			size_t chain_start = placeholder_name.find(">::");
			if (chain_start == std::string_view::npos) {
				return concrete_instantiated_type_info;
			}

			std::vector<StringHandle> member_type_chain;
			std::string_view remaining_chain = placeholder_name.substr(chain_start + 3);
			while (!remaining_chain.empty()) {
				size_t next_separator = remaining_chain.find("::");
				std::string_view member_name = remaining_chain.substr(0, next_separator);
				...
				remaining_chain = remaining_chain.substr(next_separator + 2);
			}

			if (member_type_chain.empty()) {
				return concrete_instantiated_type_info;
			}
			return resolveBaseClassMemberTypeChain(concrete_instantiated_name, member_type_chain);
		};
		...
		instantiated_type_info =
			resolvePlaceholderMemberChain(instantiated_name, instantiated_type_info);
	...
	if (!substitution_applied && type_name.find('_') != std::string_view::npos) {
		forEachNonPackTemplateParamArgBinding(
			...
			[&](const TemplateParameterNode& template_param, const TemplateTypeArg&, size_t) {
				...
				const std::string_view param_name = template_param.name();
				const size_t param_pos = type_name.rfind(param_name);
				if (param_pos == std::string_view::npos || param_pos == 0 || type_name[param_pos - 1] != '_' ||
					param_pos + param_name.size() != type_name.size()) {
					return;
				}
	...
	if (!substitution_applied) {
		...
		if (placeholder_info && placeholder_info->isTemplateInstantiation()) {
			...
			for (size_t i = 0; i < template_params.size() && i < template_args.size(); ++i) {
				...
				const TemplateTypeArg& concrete_template_arg = template_args[i];
				const TypeInfo* concrete_template_info = tryGetTypeInfo(concrete_template_arg.type_index);
				if (!concrete_template_info) {
					break;
				}
				...
				std::vector<StringHandle> member_type_chain;
				if (size_t chain_start = type_name.find(">::"); chain_start != std::string_view::npos) {
					std::string_view remaining_chain = type_name.substr(chain_start + 3);
					while (!remaining_chain.empty()) {
						size_t next_separator = remaining_chain.find("::");
						std::string_view member_name = remaining_chain.substr(0, next_separator);
						if (size_t template_pos = member_name.find('<'); template_pos != std::string_view::npos) {
				...
				instantiated_type_info =
					resolveBaseClassMemberTypeChain(instantiated_name, member_type_chain);
				if (instantiated_type_info != nullptr) {
					assignResolvedType(*instantiated_type_info);
					substitution_applied = true;
...
std::optional<TypeSpecifierNode> Parser::get_expression_type(const ASTNode& expr_node) {
	...
	if (expr_node.is<LambdaExpressionNode>()) {
		...
		auto type_it = getTypesByNameMap().find(closure_name);
	...
	if (std::holds_alternative<BoolLiteralNode>(expr)) {
		...
		if (!ns_handle.isGlobal()) {
			...
			// Try to find the struct in getTypesByNameMap()
			...
			if (struct_type_it == getTypesByNameMap().end() && gNamespaceRegistry.getDepth(ns_handle) > 1) {
			...
			if (struct_type_it != getTypesByNameMap().end() && struct_type_it->second->isStruct()) {
				...
				TypeSpecifierNode enum_type(TypeCategory::Enum, TypeQualifier::None,
											enum_type_info->getEnumInfo()->sizeInBits().value, Token{}, CVQualifier::None);
...
std::string Parser::type_to_string(const TypeSpecifierNode& type) const {
	...
	case TypeCategory::MemberFunctionPointer:
		result += "member function pointer";
		break;
	case TypeCategory::MemberObjectPointer:
		result += "member object pointer";
		break;
