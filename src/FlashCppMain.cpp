...
void printTimingSummary(double preprocessing_time, double lexer_setup_time, double parsing_time,
	...
	if (other_time < 0)
		other_time = 0;	// Avoid negative due to timing imprecision
...
static void printOutsideDiagnosticTelemetry() {
	...
	if (FrontendContext* active = FrontendContext::active()) {
		if (gSymbolTable.persistentScopePublicationEnabled()) {
			const FrontendContext& publisher = requirePersistentScopePublicationContext(gSymbolTable);
...
#else
			FLASH_LOG(Codegen, Debug, "Generating ELF object file (Linux/Unix target) not supported on this platform yet");
#endif
		} else {
