...
// Substitute template parameters in the default value expression
	...
	if (func_decl.get_definition().has_value()) {
		...
		for (size_t i = 0; i < param_names.size() && i < lazy_info.template_args.size(); ++i) {
			// is_value (non-type) params must NOT be registered as TypeInfo: they don't
			// represent concrete types and would poison gTypesByName with Invalid entries.
			if (lazy_info.template_args[i].is_value) continue;
			if (lazy_info.template_args[i].is_template_template_arg) continue;
