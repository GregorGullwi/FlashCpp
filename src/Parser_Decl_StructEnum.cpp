	...
	while (!peek().is_eof() && peek() != "}"_tok) {
		...
		if (member_result.node()->is<FunctionDeclarationNode>() || peek() == "("_tok) {
			...
			if (peek() == "{"_tok) {
				...
				prepareArrayTypeForBraceInitializer(decl_node, init_type_spec);
				...
				if (init_result.node().has_value()) {
					...
					inferUnsizedArraySizeFromInitializer(decl_node, init_type_spec, default_initializer);
				}
			}
...
ParseResult Parser::parse_enum_declaration() {
	...
	if (enum_namespace_handle.isValid() && !enum_namespace_handle.isGlobal()) {
		StringHandle qualified_enum_name = gNamespaceRegistry.buildQualifiedIdentifier(enum_namespace_handle, enum_name);
