...
#include "IrGenerator.h"
	...
	ExprResult AstToIr::generateStaticCastIr(const StaticCastNode& staticCastNode) {
		...
		if (target_pointer_depth > 0) {
			...
			return withStorage(makeExprResult(target_type, SizeInBits{64}, expr_operands.value, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
		}
		...
		if (source_type == target_type && source_size == target_size) {
			if (source_has_semantic_identity() && !carriesSemanticTypeIndex(target_type)) {
				return withStorage(makeExprResult(target_type, SizeInBits{static_cast<int>(target_size)}, expr_operands.value, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
			}
		...
		if (is_floating_point_type(source_type) && is_integer_type(target_type)) {
			...
			return withStorage(makeExprResult(target_type, SizeInBits{static_cast<int>(target_size)}, IrOperand{result_temp}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
		}
		...
		if (is_integer_type(source_type) && target_type == Type::Bool) {
			...
			return withStorage(makeExprResult(Type::Bool, SizeInBits{8}, IrOperand{result_temp}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
		}
	...
	ExprResult AstToIr::generateTypeidIr(const TypeidNode& typeidNode) {
		...
		return withStorage(makeExprResult(Type::Void, SizeInBits{64}, IrOperand{result_temp}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
	}
	...
	ExprResult AstToIr::generateDynamicCastIr(const DynamicCastNode& dynamicCastNode) {
		...
		return withStorage(makeExprResult(result_type, SizeInBits{static_cast<int>(result_size)}, IrOperand{result_temp}, TypeIndex{}, PointerDepth{}), result_storage);
	}
	...
	ExprResult AstToIr::generateReinterpretCastIr(const ReinterpretCastNode& reinterpretCastNode) {
		...
		return withStorage(makeExprResult(target_type, SizeInBits{static_cast<int>(result_size)}, expr_operands.value, TypeIndex{}, PointerDepth{target_pointer_depth}), ValueStorage::ContainsData);
	}
