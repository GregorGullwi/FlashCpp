...
ExprResult AstToIr::generateStaticCastIr(const StaticCastNode& staticCastNode) {
	...
	// static_cast to a struct type from a primitive: use explicit constructor.
	// This handles cases like static_cast<strong_ordering>(int_value) which the
	// standard library uses for comparison category types.
	if (target_type == TypeCategory::Struct && target_type_node.type_index().is_valid()) {
	...
	// For small structs (like comparison category types that are just a signed char),
	// pass the value through with the struct's type identity.
