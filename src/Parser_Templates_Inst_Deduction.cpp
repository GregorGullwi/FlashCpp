...
// ─────────────────────────────────────────────────────────────────────────────
// Populate a TemplateParamSubstitution vector from parallel (name, arg) pairs
// for body-reparse paths.  Non-type and template-template entries are converted
// to is_value_param / is_type_param respectively so parse_block() can resolve
// references like "return N;" without touching getTypesByNameMap().
//
...
// ============================================================================
// Phase 8 Helper Functions: Unified Template Instantiation After Environment Binding
// ============================================================================

MaterializedFunctionParameters Parser::materializeTemplateFunctionParameters(
	...
	MaterializedFunctionParameters result;
	result.body_source = "";
	
	try {
		// Materialize return type using the binding environment
		const TypeSpecifierNode& orig_return_type = template_func.decl_node().type_specifier_node();
		
		// Create a map from the environment for use with existing substitution functions
		std::unordered_map<std::string_view, TemplateTypeArg> param_map;
		auto append_bindings = [&](const TemplateEnvironment* env) {
			if (!env) return;
			for (const auto& binding : env->bindings) {
				std::string_view name = StringTable::getStringView(binding.name);
				if (!binding.args.empty() && !binding.is_pack) {
					param_map[name] = binding.args.front();
				}
			}
		};
		
		const TemplateEnvironment* current = &binding_environment;
		while (current) {
			append_bindings(current);
			current = current->parent;
		}
		
		// For now, just copy the original return type
		// A full implementation would call substitute_template_parameter here
		result.return_type = orig_return_type;
		
		// Materialize parameters
		for (size_t i = 0; i < template_func.parameter_nodes().size(); ++i) {
			const auto& param_node = template_func.parameter_nodes()[i];
			if (param_node.is<DeclarationNode>()) {
				const DeclarationNode& param_decl = param_node.as<DeclarationNode>();
				const TypeSpecifierNode& orig_param_type = param_decl.type_specifier_node();
				
				// For now, just copy the original parameter type
				// A full implementation would call substitute_template_parameter here
				result.params.push_back(orig_param_type);
			}
		}
		
	} catch (const std::exception& e) {
		FLASH_LOG(Templates, Debug, "Error materializing function parameters: ", e.what());
	}
	
	return result;
}

TemplateMaterializationResult Parser::instantiateBoundFunctionTemplate(
	const FunctionDeclarationNode& template_func,
	std::string_view template_name,
	const TemplateEnvironment& active_binding_environment,
	TemplateSubstitutionFailurePolicy failure_policy) {
	
	TemplateMaterializationResult result;
	result.applied_policy = failure_policy;
	result.had_substitution_failures = false;
	
	try {
		// Materialize function parameters
		MaterializedFunctionParameters materialized_params = materializeTemplateFunctionParameters(
			template_func,
			active_binding_environment,
			failure_policy);
		
		// Create a new function declaration with materialized signature
		auto new_decl = emplace_node<DeclarationNode>(
			materialized_params.return_type,
			template_func.decl_node().identifier_token());
		
		auto [new_func_node, new_func_ref] = emplace_node_ref<FunctionDeclarationNode>(new_decl.as<DeclarationNode>());
		
		// Add materialized parameters
		size_t param_index = 0;
		for (const auto& param_type : materialized_params.params) {
			if (param_index < template_func.parameter_nodes().size()) {
				const auto& orig_param = template_func.parameter_nodes()[param_index];
				if (orig_param.is<DeclarationNode>()) {
					auto param_decl_node = emplace_node<DeclarationNode>(
						param_type,
						orig_param.as<DeclarationNode>().identifier_token());
		...
		// Preserve const/volatile qualifiers from original
		if (template_func.is_const_member_function()) {
			new_func_ref.set_is_const_member_function(true);
		}
		
		result.instantiated_node = new_func_node;
		
	} catch (const CompileError& e) {
		result.had_substitution_failures = true;
		FLASH_LOG(Templates, Debug, "Template instantiation failed: ", e.what());
		if (failure_policy != TemplateSubstitutionFailurePolicy::SfinaeProbe) {
			result.instantiated_node = std::nullopt;
		}
	} catch (const std::exception& e) {
		result.had_substitution_failures = true;
		result.instantiated_node = std::nullopt;
		FLASH_LOG(Templates, Debug, "Unexpected error during instantiation: ", e.what());
	}
	
	return result;
}

bool Parser::finalizeInstantiatedFunction(
	ASTNode& instantiated_func_node,
	StringHandle original_name,
	const TemplateEnvironment& binding_environment) {
	
	// Note: Full implementation deferred to Phase 8.1 work items.
	// For now, return true to establish interface contract.
	// Integration will occur when instantiation finalization is
	// centralized across both implicit and explicit paths.
