...
static const TypeSpecifierNode* tryGetMappedCallArgForDirectTypeTemplateParam(
	const std::vector<ASTNode>& func_params,
	const std::vector<TypeSpecifierNode>& arg_types,
	const Parser::CallArgDeductionInfo& deduction_info,
	StringHandle template_param_name) {
	for (size_t i = 0; i < func_params.size() &&
					   i < deduction_info.func_param_to_call_arg_index.size();
		 ++i) {
		...
		size_t call_arg_index = deduction_info.func_param_to_call_arg_index[i];
		if (call_arg_index == SIZE_MAX || call_arg_index >= arg_types.size()) {
			continue;
		}
		const TypeSpecifierNode& func_param_type = func_param_decl.type_node().as<TypeSpecifierNode>();
		...
		if (!func_param_type_info || func_param_type_info->name() != template_param_name) {
			continue;
		}
				...
				if (explicit_idx < explicit_types.size()) {
					...
					if (!param.has_default() &&
						...
						if (const TypeSpecifierNode* mapped_arg_type =
								tryGetMappedCallArgForDirectTypeTemplateParam(
									func_decl.parameter_nodes(),
									*current_explicit_call_arg_types_,
									*deduction_info,
									param_handle)) {
