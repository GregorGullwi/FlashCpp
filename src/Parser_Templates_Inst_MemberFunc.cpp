	...
	for (const auto& template_param_node : template_params) {
		...
		if (param.kind() == TemplateParameterKind::Template) {
			...
			if (arg_index < arg_types.size()) {
				template_args.push_back(TemplateTypeArg::makeType(arg_types[arg_index].category(), arg_types[arg_index].type_index()));
				arg_index++;
			} else {
				// Not enough arguments - use first argument type
				template_args.push_back(TemplateTypeArg::makeType(arg_types[0].category(), arg_types[0].type_index()));
			}
		} else {
