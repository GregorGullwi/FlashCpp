...
ParseResult Parser::apply_postfix_operators(ASTNode& start_result)
{
	...
	while (result.has_value() && !peek().is_eof() && postfix_iteration < MAX_POSTFIX_ITERATIONS) {
		...
		if (peek().is_punctuator() && peek() == "."_tok) {
			...
			if (peek() == "("_tok) {
				...
				if (instantiated_func.has_value() && instantiated_func->is<FunctionDeclarationNode>()) {
					...
					auto type_spec = emplace_node<TypeSpecifierNode>(TypeCategory::Auto, TypeIndex{}, 0, member_name_token);
		...
		if (peek().is_punctuator() && peek() == "("_tok) {
			...
			auto temp_type = emplace_node<TypeSpecifierNode>(TypeCategory::Int, TypeQualifier::None, 32, operator_token);
...
ParseResult Parser::parse_postfix_expression(ExpressionContext context)
{
	...
	while (result.has_value() && !peek().is_eof() && postfix_iteration < MAX_POSTFIX_ITERATIONS) {
		...
		if (peek().is_punctuator() && peek() == "("_tok) {
			...
			if (is_function_pointer_call && member_access) {
				...
				Token member_token(Token::Type::Identifier, member_access->member_name(),
				                   paren_token.line(), paren_token.column(), paren_token.file_index());
				auto temp_type = emplace_node<TypeSpecifierNode>(TypeCategory::Int, TypeQualifier::None, 32, member_token);
		...
		if (peek().is_punctuator() && peek() == "::"_tok) {
				...
				if (std::holds_alternative<MemberAccessNode>(expr)) {
					...
					while (peek().is_identifier()) {
						...
						if (peek() == "("_tok) {
							...
							auto type_spec = emplace_node<TypeSpecifierNode>(TypeCategory::Auto, TypeIndex{}, 0, qualified_member_token);
			...
			if (peek() == "("_tok) {
				...
				if (!decl_ptr) {
					...
					auto type_node = emplace_node<TypeSpecifierNode>(TypeCategory::Int, TypeQualifier::None, 32, final_identifier);
		...
		if (peek() == "operator"_tok) {
			...
			auto type_spec = emplace_node<TypeSpecifierNode>(TypeCategory::Auto, TypeIndex{}, 0, member_operator_name_token);
		...
		if (peek() == "("_tok) {
			...
			if (instantiated_func.has_value() && instantiated_func->is<FunctionDeclarationNode>()) {
				...
				auto temp_type = emplace_node<TypeSpecifierNode>(TypeCategory::Int, TypeQualifier::None, 32, member_name_token);
