...
std::optional<ASTNode> Parser::try_instantiate_class_template(std::string_view template_name, const std::vector<TemplateTypeArg>& template_args, bool force_eager) {
	...
	// Populate type-owned instantiation context
			{
				...
				for (size_t i = 0; i < template_params.size() && i < template_args.size(); ++i) {
					if (template_params[i].is<TemplateParameterNode>()) {
						param_names.push_back(template_params[i].as<TemplateParameterNode>().nameHandle());
					}
				}
				struct_type_info.setInstantiationContext(std::move(param_names), template_args_info, nullptr);
			}
 ...
 // Copy nested classes from the template with template parameter substitution
	...
	// one and needs the enclosing template's bindings for constexpr evaluation.
			{
				InlineVector<StringHandle, 4> param_names;
				auto all_param_args = convertToTemplateArgInfo(template_args_to_use);
				for (size_t i = 0; i < template_params.size() && i < template_args_to_use.size(); ++i) {
					if (template_params[i].is<TemplateParameterNode>()) {
						param_names.push_back(template_params[i].as<TemplateParameterNode>().nameHandle());
					}
				...
				nested_type_info.setInstantiationContext(
					std::move(param_names), std::move(all_param_args),
