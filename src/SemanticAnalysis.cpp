...
const FunctionDeclarationNode* SemanticAnalysis::findCallableObjectOperator(const FunctionCallNode& call_node) const {
	...
	const size_t argument_count = call_node.arguments().size();
	const FunctionDeclarationNode* exact_arity_match = nullptr;
	...
	for (const auto& member_func : struct_info->member_functions) {
		...
		if (!candidate.is_variadic() && candidate.parameter_nodes().size() == argument_count) {
			exact_arity_match = &candidate;
			break;
		}
		if (!callableOperatorAcceptsArgumentCount(candidate, argument_count)) {
			continue;
		}
