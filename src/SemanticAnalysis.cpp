...
void SemanticAnalysis::normalizeStructDeclaration(const StructDeclarationNode& decl) {
	...
	const TypeInfo* decl_type_info = findStructTypeInfoByNameFragment(StringTable::getStringView(decl.name()));

	std::optional<MemberContext> member_default_init_context;
	if (decl_type_info && decl_type_info->getStructInfo()) {
		member_default_init_context = MemberContext{decl_type_info->type_index_, true, CVQualifier::None};
		member_context_stack_.push_back(*member_default_init_context);
	}
	auto member_default_init_context_cleanup = ScopeGuard([this, &member_default_init_context]() {
	...
	if (decl_type_info && decl_type_info->getStructInfo()) {
		static_member_context = MemberContext{decl_type_info->type_index_, false, CVQualifier::None};
...
void SemanticAnalysis::normalizeStructuredBinding(const StructuredBindingNode& binding, const SemanticContext& ctx) {
	...
	ConstExpr::EvaluationContext eval_ctx = parser_ != nullptr
		? ConstExpr::EvaluationContext(symbols_, parser())
		: ConstExpr::EvaluationContext(symbols_, *this);
		...
		std::visit([&](auto& e) {
			...
			if constexpr (std::is_same_v<T, BinaryOperatorNode>) {
				...
				if (needs_binary_type_inference) {
					...
					// arithmetic conversions. This intentionally runs after
					// child normalization so nested expression result types are
					// settled before this binary operator selects conversions.
...
std::optional<TypeSpecifierNode> SemanticAnalysis::resolveCallQueryType(const CallExprNode& call_expr) const {
	auto applySelfReferenceToReturnType = [&](TypeSpecifierNode& return_type) {
		if (call_expr.has_receiver()) {
	...
	auto tryResolveReturnTypeFromDeclaration = [&](const FunctionDeclarationNode* func_decl) -> std::optional<TypeSpecifierNode> {
		...
		if (isPlaceholderAutoType(return_type.type()) && return_type.has_function_signature()) {
			const FunctionSignature& signature = return_type.function_signature();
			TypeCategory signature_return_category = signature.returnType();
			if (!isPlaceholderAutoType(signature_return_category) &&
				signature_return_category != TypeCategory::Invalid &&
				signature.return_type_index.is_valid()) {
				return_type = TypeSpecifierNode(
					signature.return_type_index.withCategory(signature_return_category),
					SizeInBits{get_type_size_bits(signature_return_category)},
		...
		if (func_decl->is_member_function()) {
			applySelfReferenceToReturnType(return_type);
		}

		if (return_type.type() == TypeCategory::Invalid || isPlaceholderAutoType(return_type.type())) {
			if (auto resolved_return = const_cast<SemanticAnalysis*>(this)->resolveCallReturnType(*func_decl, call_expr);
				resolved_return.has_value()) {
	...
	if (const FunctionDeclarationNode* parser_stored_target = call_expr.callee().function_declaration_or_null();
		parser_stored_target != nullptr) {
		if (auto return_type = tryResolveReturnTypeFromDeclaration(parser_stored_target); return_type.has_value()) {
			return return_type;
		}
	}
			...
			if (e.binding() == IdentifierBinding::StaticMember) {
				...
				if (op == "<<" || op == ">>") {
					...
					if (lhs_desc.category() == TypeCategory::Struct ||
						...
						return {};
					const TypeCategory unpromoted_category = resolveEnumUnderlyingTypeCategory(lhs_desc.type_index);
					const TypeCategory result_category = promote_integer_type(unpromoted_category);
					CanonicalTypeDesc desc;
