...
bool SemanticAnalysis::tryAnnotateCopyInitConvertingConstructor(const ASTNode& expr_node,
	...
	// Special case: don't error for integer → comparison category type conversions.
	// In deferred template bodies, inferExpressionType may not fully resolve
	// built-in <=> results as comparison category types, so the semantic analysis
	// sees "int → strong_ordering" instead of "strong_ordering → strong_ordering".
	// The codegen's built-in <=> handler produces the correct type at runtime.
			const StructTypeInfo* target_struct = to_type_info->getStructInfo();
			if (is_integer_type(from_desc.category()) && target_struct &&
				target_struct->total_size == 1) {
				std::string_view target_name = StringTable::getStringView(to_type_info->name());
				if (target_name.find("ordering") != std::string_view::npos) {
