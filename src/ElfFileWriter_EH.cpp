...
void ElfFileWriter::add_text_relocation(uint64_t offset, const std::string& symbol_name, uint32_t relocation_type, int64_t addend) {
 ...
 */
ELFIO::section* ElfFileWriter::getSectionByName(const std::string& name) {
 ...
 */
ELFIO::symbol_section_accessor* ElfFileWriter::getSymbolAccessor() {
 ...
 */
ELFIO::relocation_section_accessor* ElfFileWriter::getRelocationAccessor(const std::string& rela_section_name) {
...
ELFIO::Elf_Word ElfFileWriter::lookupSymbolIndex(const std::string& name) {
	buildSymbolIndexCache();
...
std::vector<uint8_t> ElfFileWriter::dwarfFbregExpr(int32_t rbp_off) {
...
std::vector<uint8_t> ElfFileWriter::dwarfRegExpr(int dwarf_reg) {
	std::vector<uint8_t> expr;
