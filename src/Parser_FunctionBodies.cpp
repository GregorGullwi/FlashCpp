...
void Parser::register_member_functions_in_scope(StructDeclarationNode* struct_node, TypeIndex struct_type_index) {
	...
	TemplateVector<TypeIndex, 8> base_classes_to_search;
	...
	if (block_result.node().has_value()) {
		...
		if (func_node && func_node->is_static()) {
			if (const StructTypeInfo* struct_info = tryGetStructTypeInfo(delayed.struct_type_index)) {
				parsed_body = rebindStaticMemberInitializerFunctionCalls(parsed_body, struct_info, false);
			}
		}
...
void Parser::copy_function_properties(FunctionDeclarationNode& dest, const FunctionDeclarationNode& src) {
	...
	if (src.has_template_body_position()) {
		dest.set_template_body_position(src.template_body_position());
	}
	if (src.has_template_declaration_position()) {
		dest.set_template_declaration_position(src.template_declaration_position());
	}
