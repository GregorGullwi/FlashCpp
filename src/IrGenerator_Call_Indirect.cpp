	...
	ExprResult AstToIr::generateMemberFunctionCallIr(const MemberFunctionCallNode& memberFunctionCallNode) {
		...
		if (lambda_ptr) {
			...
			if (lambda.captures().empty()) {
				...
				return withStorage(makeExprResult(lambda_return_type, SizeInBits{static_cast<int>(lambda_return_size)}, IrOperand{ret_var}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
			}
		...
		if (object_expr && std::holds_alternative<IdentifierNode>(*object_expr)) {
			...
			if (resolveMemberAccessType(member_access, resolved_struct_info, resolved_member)) {
				if (resolved_member && resolved_member->type == Type::FunctionPointer) {
					...
					return withStorage(makeExprResult(ret_type, SizeInBits{static_cast<int>(ret_size)}, IrOperand{ret_var}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
				}
		...
		if (func_decl.is_consteval()) {
			...
			if (ret_type == Type::Float) {
				...
				return withStorage(makeExprResult(ret_type, SizeInBits{32}, IrOperand{static_cast<unsigned long long>(fbits)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
			}
			if (ret_type == Type::Double || ret_type == Type::LongDouble) {
				...
				return withStorage(makeExprResult(ret_type, SizeInBits{64}, IrOperand{dbits}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
			}
			...
			return withStorage(makeExprResult(ret_type, ret_size, IrOperand{evalResultScalarToRaw(eval_result)}, TypeIndex{}, PointerDepth{}), ValueStorage::ContainsData);
		}
