...
ExprResult AstToIr::generateMemberFunctionCallIr(const CallExprNode& callExprNode, ExpressionContext context, const void* sema_call_key) {
	...
	if (is_virtual_call && vtable_index >= 0) {
		...
		if (struct_info && !function_name.isValid()) {
			...
			{
				// Mangle the concrete declaration selected before lowering. Prefer the
				// declaration from struct_info when it has substituted template types.
