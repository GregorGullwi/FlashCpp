...
struct LogConfig {
	...
	static void setLevel(LogCategory cat, LogLevel level) {
		if (!isSingleBitCategory(cat))
			return;	// Ignore multi-bit categories like All
	...
	static LogLevel getLevelForCategory(LogCategory cat) {
		if (!isSingleBitCategory(cat))
			return runtimeLevel;	 // Multi-bit categories use runtime level
		size_t idx = categoryToIndex(cat);
...
struct Logger {
	...
	static constexpr const char* colorCode() {
		...
		case LogLevel::Trace:
			return detail::BLUE;
		default:
			return "";  // No color for Info, Debug
		}
	}
