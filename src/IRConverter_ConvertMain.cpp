...
void IrToObjConverter<TWriterClass>::convert(const Ir& ir, const std::string_view filename, const std::string_view source_filename , bool show_timing)  {
		...
		for (size_t ir_idx = 0; ir_idx < instructions.size(); ++ir_idx) {
			...
			case IrOpcode::ElfCatchNoMatch:
				handleElfCatchNoMatch(instruction);
				break;
...
template<class TWriterClass>
std::pair<std::vector<ObjectFileWriter::TryBlockInfo>, std::vector<ObjectFileWriter::UnwindMapEntryInfo>> IrToObjConverter<TWriterClass>::convertExceptionInfoToWriterFormat()  {
...
template<class TWriterClass>
std::vector<ObjectFileWriter::SehTryBlockInfo> IrToObjConverter<TWriterClass>::convertSehInfoToWriterFormat()  {
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::RegToRegEncoding IrToObjConverter<TWriterClass>::encodeRegToRegInstruction(X64Register reg_field, X64Register rm_field, bool include_rex_w)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitOpcodeExtInstruction(uint8_t opcode, typename IrToObjConverter<TWriterClass>::X64OpcodeExtension opcode_ext, X64Register rm_field, int size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitBinaryOpInstruction(uint8_t opcode, X64Register src_reg, X64Register dst_reg, int size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovRegToReg(X64Register src_reg, X64Register dst_reg, int src_size_in_bits)  {
		emitBinaryOpInstruction(0x89, src_reg, dst_reg, src_size_in_bits);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitComparisonInstruction(const typename IrToObjConverter<TWriterClass>::ArithmeticOperationContext& ctx, uint8_t setcc_opcode)  {
...
void IrToObjConverter<TWriterClass>::emitFloatComparisonInstruction(typename IrToObjConverter<TWriterClass>::ArithmeticOperationContext& ctx, uint8_t setcc_opcode)  {
		...
		ctx.result_value.type_index = TypeIndex{ctx.result_value.type_index.index(), TypeCategory::Bool}; ctx.result_value.ir_type = IrType::Integer;
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::ArithmeticOperationContext IrToObjConverter<TWriterClass>::setupAndLoadArithmeticOperation(const IrInstruction& instruction, const char* operation_name)  {
		...
		Type result_type = categoryToType(bin_op.lhs.type_index.category());
		...
		Type operand_type = categoryToType(bin_op.lhs.type_index.category());
		...
		if (operand_ir_type == IrType::Integer) {
			...
			if (!is_integer_type(operand_type)) {
				...
				if (!is_comparison) {
					ctx.result_value.type_index = TypeIndex{ctx.result_value.type_index.index(), typeToCategory(int_type)}; ctx.result_value.ir_type = toIrType(int_type);
			...
			if (!is_comparison) {
				ctx.result_value.type_index = TypeIndex{ctx.result_value.type_index.index(), TypeCategory::UnsignedLongLong}; ctx.result_value.ir_type = IrType::Integer;
				ctx.result_value.size_in_bits = SizeInBits{64};
			}
			ctx.operand_type = Type::UnsignedLongLong;
			ctx.operand_size_in_bits = 64;
		}
		{ Type _rv_type = categoryToType(ctx.result_value.type_index.category()); if (!is_integer_type(_rv_type) && !is_bool_type(_rv_type) && !is_floating_point_type(_rv_type)) {
			auto type_name = getTypeName(_rv_type);
			std::string type_desc = type_name.empty() ? std::string("type_id=") + std::to_string(static_cast<int>(_rv_type)) : std::string(type_name);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeArithmeticResult(const typename IrToObjConverter<TWriterClass>::ArithmeticOperationContext& ctx, X64Register source_reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::groupInstructionsByFunction(const Ir& ir)  {
...
template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::shouldImplicitlyDeref(const typename IrToObjConverter<TWriterClass>::IndirectStorageInfo& info) const  {
		return !info.holds_address_only;
	}

template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::isPointerBaseStorage(int32_t stack_offset, TempVar temp_var) const  {
...
template<class TWriterClass>
std::string IrToObjConverter<TWriterClass>::buildDestructorMangledName(const StructTypeInfo& si)  {
...
template<class TWriterClass>
std::string IrToObjConverter<TWriterClass>::buildDestructorMangledNameFromString(std::string_view class_sv)  {
...
template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::hasIndirectStorage(TempVar temp_var, int32_t stack_offset) const  {
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::StackSpaceSize IrToObjConverter<TWriterClass>::calculateFunctionStackSpace(std::string_view func_name, typename IrToObjConverter<TWriterClass>::StackVariableScope& var_scope, size_t param_count)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovToFrameBySize(X64Register sourceRegister, int32_t offset, int size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovToFrameSized(SizedRegister source, SizedStackSlot dest)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovFromFrameBySize(X64Register destinationRegister, int32_t offset, int size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitPtrMovFromFrame(X64Register destinationRegister, int32_t offset)  {
		auto opcodes = generatePtrMovFromFrame(destinationRegister, offset);
		textSectionData.insert(textSectionData.end(), opcodes.op_codes.begin(), opcodes.op_codes.begin() + opcodes.size_in_bytes);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitCmpFrameImm32(int32_t frame_offset, int32_t imm_value)  {
...
template<class TWriterClass>
int32_t IrToObjConverter<TWriterClass>::allocateElfTempStackSlot(int size_bytes)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovFromFrameSized(SizedRegister dest, SizedStackSlot source)  {
		OpCodeWithSize opcodes;
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitLeaFromFrame(X64Register destinationRegister, int32_t offset)  {
		auto opcodes = generateLeaFromFrame(destinationRegister, offset);
		textSectionData.insert(textSectionData.end(), opcodes.op_codes.begin(), opcodes.op_codes.begin() + opcodes.size_in_bytes);
	}

template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitLeaRipRelative(X64Register destinationRegister)  {
		// LEA reg, [RIP + disp32]
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovToFrame(X64Register sourceRegister, int32_t offset, int size_in_bits)  {
		auto opcodes = generateMovToFrameBySize(sourceRegister, offset, size_in_bits);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovqXmmToGpr(X64Register xmm_src, X64Register gpr_dest)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovqGprToXmm(X64Register gpr_src, X64Register xmm_dest)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitCvtss2sd(X64Register xmm_dest, X64Register xmm_src)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitFloatMovFromFrame(X64Register destinationRegister, int32_t offset, bool is_float)  {
		auto opcodes = generateFloatMovFromFrame(destinationRegister, offset, is_float);
		textSectionData.insert(textSectionData.end(), opcodes.op_codes.begin(), opcodes.op_codes.begin() + opcodes.size_in_bytes);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitFloatMovToFrame(X64Register sourceRegister, int32_t offset, bool is_float)  {
		auto opcodes = generateFloatMovToFrame(sourceRegister, offset, is_float);
		textSectionData.insert(textSectionData.end(), opcodes.op_codes.begin(), opcodes.op_codes.begin() + opcodes.size_in_bytes);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitFloatMovFromMemory(X64Register xmm_dest, X64Register base_reg, int32_t offset, bool is_float)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitFloatMovRegToReg(X64Register xmm_dest, X64Register xmm_src, bool is_double)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovdquToFrame(X64Register xmm_src, int32_t offset)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovDwordPtrImmToRegOffset(X64Register base_reg, int32_t offset, uint32_t imm32)  {
		// Assert that base_reg is NOT an XMM register
		assert(static_cast<uint8_t>(base_reg) < 16 &&
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovQwordPtrImmToRegOffset(X64Register base_reg, int32_t offset, uint32_t imm32)  {
		// Assert that base_reg is NOT an XMM register
		assert(static_cast<uint8_t>(base_reg) < 16 &&
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovQwordPtrRegToRegOffset(X64Register base_reg, int32_t offset, X64Register src_reg)  {
		// Assert that base_reg is NOT an XMM register
		assert(static_cast<uint8_t>(base_reg) < 16 &&
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovImm32(X64Register destinationRegister, uint32_t immediate_value)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovImm64(X64Register destinationRegister, uint64_t immediate_value)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitSubRSP(uint8_t amount)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitAddRSP(uint8_t amount)  {
		textSectionData.push_back(0x48); // REX.W prefix
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitAndImm64(X64Register reg, uint64_t mask)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitShlImm(X64Register reg, uint8_t shift_amount)  {
		uint8_t reg_enc = static_cast<uint8_t>(reg);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitOrReg(X64Register dest, X64Register src)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitShrImm(X64Register reg, uint8_t shift_amount)  {
		uint8_t reg_enc = static_cast<uint8_t>(reg);
		textSectionData.push_back(0x48 | ((reg_enc >> 3) & 0x01)); // REX.W + REX.B branchless
...
template<class TWriterClass>
uint64_t IrToObjConverter<TWriterClass>::bitfieldMask(size_t width)  {
		return (width < 64) ? ((1ULL << width) - 1) : ~0ULL;
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitCall(const std::string& symbol_name)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovRegReg(X64Register dest, X64Register src)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovFromMemory(X64Register dest, X64Register base, int32_t offset, size_t size_bytes)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovRegFromMemRegDisp8(X64Register dest, X64Register src_addr, int8_t disp)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovRegFromMemRegSized(X64Register dest, X64Register src_addr, int size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitTestRegReg(X64Register reg)  {
		uint8_t rex = 0x48; // REX.W
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitTestAL()  {
		textSectionData.push_back(0x84); // TEST r/m8, r8
		textSectionData.push_back(0xC0); // ModR/M: AL, AL
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitLeaRipRelativeWithRelocation(X64Register dest, std::string_view symbol_name)  {
		uint8_t rex = 0x48; // REX.W
		if (static_cast<uint8_t>(dest) >= 8) rex |= 0x04;
...
template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitMovRipRelative(X64Register dest, int size_in_bits)  {
...
template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitFloatMovRipRelative(X64Register xmm_dest, bool is_float)  {
...
template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitMovRipRelativeStore(X64Register src, int size_in_bits)  {
...
template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitFloatMovRipRelativeStore(X64Register xmm_src, bool is_float)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitCmpRegReg(X64Register r1, X64Register r2)  {
		uint8_t rex = 0x48; // REX.W for 64-bit
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitCmpRegWithMem(X64Register reg, X64Register mem_base)  {
		// CMP reg, [mem_base]
		uint8_t rex = 0x48; // REX.W
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJumpIfZero(int8_t offset)  {
		textSectionData.push_back(0x74); // JZ rel8
		textSectionData.push_back(static_cast<uint8_t>(offset));
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJumpIfEqual(int8_t offset)  {
		textSectionData.push_back(0x74); // JE rel8 (same as JZ)
		textSectionData.push_back(static_cast<uint8_t>(offset));
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJumpIfNotZero(int8_t offset)  {
		textSectionData.push_back(0x75); // JNZ rel8
		textSectionData.push_back(static_cast<uint8_t>(offset));
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJumpUnconditional(int8_t offset)  {
		textSectionData.push_back(0xEB); // JMP rel8
		textSectionData.push_back(static_cast<uint8_t>(offset));
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitXorRegReg(X64Register reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitRepMovsb()  {
		textSectionData.push_back(0xF3); // REP prefix
		textSectionData.push_back(0xA4); // MOVSB
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovToRSPDisp8(X64Register sourceRegister, int8_t displacement)  {
		// MOV [RSP+disp8], reg
		uint8_t rex = 0x48; // REX.W for 64-bit
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitRet()  {
		textSectionData.push_back(0xC3); // RET
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitMovRegImm8(X64Register reg, uint8_t imm)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitPushReg(X64Register reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitPopReg(X64Register reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitIncReg(X64Register reg)  {
		uint8_t rex = 0x48; // REX.W
		if (static_cast<uint8_t>(reg) >= 8) rex |= 0x01; // REX.B
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitCmpRegImm32(X64Register reg, uint32_t imm)  {
		uint8_t rex = 0x48; // REX.W
		if (static_cast<uint8_t>(reg) >= 8) rex |= 0x01; // REX.B
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJumpIfAbove(int8_t offset)  {
		textSectionData.push_back(0x77); // JA rel8 (unsigned >)
		textSectionData.push_back(static_cast<uint8_t>(offset));
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJumpIfBelow(int8_t offset)  {
		textSectionData.push_back(0x72); // JB rel8 (unsigned <)
		textSectionData.push_back(static_cast<uint8_t>(offset));
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitLeaRegScaledIndex(X64Register dest, X64Register base, X64Register index, uint8_t scale, int8_t disp)  {
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::allocateRegisterWithSpilling()  {
		return allocateRegisterWithSpilling(X64Register::Count);
	}

template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::isRestrictedCatchFuncletRegister(X64Register reg) const  {
			if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
				return false;
			}
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::allocateRegisterWithSpilling(X64Register exclude)  {
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::allocateXMMRegisterWithSpilling(X64Register exclude)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFunctionCall(const IrInstruction& instruction)  {
		// Use typed payload
		if (instruction.hasTypedPayload()) {
			...
			if (call_op.returns_reference && call_op.return_type_index.category() != TypeCategory::Function) {
				registerObjectReferenceCallResult(
					result_offset,
					categoryToType(call_op.return_type_index.category()),
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleDestructorCall(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleVirtualCall(const IrInstruction& instruction)  {
		...
		// Get object offset
		int object_offset = 0;
		if (const auto* temp_var_ptr = std::get_if<TempVar>(&op.object)) {
			const TempVar& temp_var = *temp_var_ptr;
			object_offset = getStackOffsetFromTempVar(temp_var);
		} else {
		...
		registerObjectReferenceCallResult(
			result_offset,
			categoryToType(op.result.type_index.category()),
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleHeapAlloc(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleHeapAllocArray(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleHeapFree(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleHeapFreeArray(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handlePlacementNew(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleTypeid(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleDynamicCast(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::spillAndInvalidateRegisterForManualOverwrite(X64Register target_reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleGlobalVariableDecl(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleGlobalLoad(const IrInstruction& instruction)  {
		...
		Type result_type = categoryToType(op.result.type_index.category());
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleGlobalStore(const IrInstruction& instruction)  {
		...
		Type var_type = categoryToType(global_info->type_index.category());
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleVariableDecl(const IrInstruction& instruction)  {
		// Extract typed payload
		const VariableDeclOp& op = std::any_cast<const VariableDeclOp&>(instruction.getTypedPayload());
		...
		Type var_type = categoryToType(op.type_index.category());
...
template<class TWriterClass>
uint16_t IrToObjConverter<TWriterClass>::getX64RegisterCodeViewCode(X64Register reg)  {
		switch (reg) {
...
void IrToObjConverter<TWriterClass>::resetFunctionState()  {
		...
		current_function_return_type_index_ = TypeIndex{};
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFunctionDecl(const IrInstruction& instruction)  {
		...
		if (!instruction.hasTypedPayload()) {
			...
			for (size_t i = 0; i < func_decl.parameters.size(); ++i) {
				...
				// so this path is gated on !is_variadic.
				bool is_two_reg_struct = !is_variadic &&
				                        isTwoRegisterStructRaw(toIrType(param.type_index), param.size_in_bits.value, param.is_reference(), param.pointer_depth.value);
				...
				// Reference parameters (including rvalue references) are passed as pointers,
				// so they should use integer registers regardless of the underlying type
				bool is_float_param = isIrFloatingPointType(toIrType(param.type_index)) && !param.pointer_depth.is_pointer() && !param.is_reference();
				...
				bool is_passed_by_reference = param.is_reference() ||
				                              (!is_two_reg_struct && isIrStructType(toIrType(param.type_index)) && param.size_in_bits.value > 64);
				if (is_passed_by_reference) {
					setReferenceInfo(offset, categoryToType(param.type_index.category()), param.size_in_bits.value, param.is_rvalue_reference(), TempVar{0});
				}
				...
				if (param.pointer_depth.is_pointer()) {
					...
					switch (categoryToType(param.type_index.category())) {
						case Type::Int: param_type_index = 0x74; break;  // T_INT4
						case Type::Float: param_type_index = 0x40; break; // T_REAL32
						case Type::Double: param_type_index = 0x41; break; // T_REAL64
						case Type::Char: param_type_index = 0x10; break;  // T_CHAR
				...
				if (use_register) {
					...
					parameters.push_back({categoryToType(param.type_index.category()), param.size_in_bits.value, StringTable::getStringView(param.getName()), paramNumber, offset, src_reg, param.pointer_depth.value, param.is_reference(), second_reg});
				}
			}
		}
...
template<class TWriterClass>
int32_t IrToObjConverter<TWriterClass>::ensureCatchFuncletReturnSlot()  {
...
template<class TWriterClass>
int32_t IrToObjConverter<TWriterClass>::ensureCatchFuncletReturnFlagSlot()  {
...
template<class TWriterClass>
StringHandle IrToObjConverter<TWriterClass>::getOrCreateCatchContinuationFixupLabel(StringHandle continuation_handle)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleReturn(const IrInstruction& instruction)  {
		...
		if (instruction.hasTypedPayload()) {
			...
			if (!ret_op.return_value.has_value()) {
				...
				if (std::holds_alternative<unsigned long long>(ret_val)) {
					...
					bool is_float_return = ret_op.return_type_index.has_value() &&
					                        isFloatingPointType(ret_op.return_type_index.value().category());
					...
					// Check if this is a reference variable - if so, dereference it
						...
						if (ref_info.has_value() &&
							...
							if (current_function_has_hidden_return_param_ && is_rvo_eligible) {
								...
								if (ret_op.return_type_index.has_value() && ret_op.return_type_index.value().category() == TypeCategory::Struct &&
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBinaryArithmetic(const IrInstruction& instruction, uint8_t opcode, const char* description)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::reserveDivisionFixedRegisters()  {
		flushAllDirtyRegisters();
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::allocateDivisionScratchRegister(X64Register avoid1, X64Register avoid2 , X64Register avoid3)  {
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::preserveDivisorAcrossRaxMove(X64Register divisor_reg, X64Register result_reg, int operand_size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleAdd(const IrInstruction& instruction)  {
		handleBinaryArithmetic(instruction, 0x01, "addition"); // ADD dst, src
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSubtract(const IrInstruction& instruction)  {
		handleBinaryArithmetic(instruction, 0x29, "subtraction"); // SUB dst, src
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleMultiply(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleDivide(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnsignedDivide(const IrInstruction& instruction)  {
		reserveDivisionFixedRegisters();
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnsignedShiftRight(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBitwiseArithmetic(const IrInstruction& instruction, uint8_t opcode, const char* description)  {
		auto ctx = setupAndLoadArithmeticOperation(instruction, description);
		emitBinaryOpInstruction(opcode, ctx.rhs_physical_reg, ctx.result_physical_reg, ctx.operand_size_in_bits);
		storeArithmeticResult(ctx);
		if (ctx.rhs_physical_reg != ctx.result_physical_reg) {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBitwiseAnd(const IrInstruction& instruction)  {
		handleBitwiseArithmetic(instruction, 0x21, "bitwise AND"); // AND dst, src
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBitwiseOr(const IrInstruction& instruction)  {
		handleBitwiseArithmetic(instruction, 0x09, "bitwise OR"); // OR dst, src
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBitwiseXor(const IrInstruction& instruction)  {
		handleBitwiseArithmetic(instruction, 0x31, "bitwise XOR"); // XOR dst, src
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleModulo(const IrInstruction& instruction)  {
		reserveDivisionFixedRegisters();
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleGreaterThan(const IrInstruction& instruction)  {
		auto ctx = setupAndLoadArithmeticOperation(instruction, "greater than comparison");
		emitComparisonInstruction(ctx, 0x9F); // SETG
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleGreaterEqual(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnsignedLessThan(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnsignedLessEqual(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnsignedGreaterThan(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnsignedGreaterEqual(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleLogicalAnd(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleLogicalOr(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleLogicalNot(const IrInstruction& instruction)  {
		handleUnaryOperation(instruction, UnaryOperation::LogicalNot);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBitwiseNot(const IrInstruction& instruction)  {
		handleUnaryOperation(instruction, UnaryOperation::BitwiseNot);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleNegate(const IrInstruction& instruction)  {
		handleUnaryOperation(instruction, UnaryOperation::Negate);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeUnaryResult(const IrOperand& result_operand, X64Register result_physical_reg, int size_in_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatAdd(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatSubtract(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatMultiply(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatDivide(const IrInstruction& instruction)  {
		// Setup and load operands
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatEqual(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatLessThan(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatLessEqual(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatGreaterThan(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatGreaterEqual(const IrInstruction& instruction)  {
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::loadFromIndirectStorage(int32_t stack_offset, const typename IrToObjConverter<TWriterClass>::IndirectStorageInfo& ref_info)  {
...
template<class TWriterClass>
X64Register IrToObjConverter<TWriterClass>::loadOperandIntoRegister(const IrInstruction& instruction, size_t operand_index)  {
...
template<class TWriterClass>
const typename IrToObjConverter<TWriterClass>::VariableInfo* IrToObjConverter<TWriterClass>::findVariableInfo(StringHandle name) const  {
		for (auto scope_it = variable_scopes.rbegin(); scope_it != variable_scopes.rend(); ++scope_it) {
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::UnaryOperandLocation IrToObjConverter<TWriterClass>::UnaryOperandLocation::stack(int32_t offset)  {
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::UnaryOperandLocation IrToObjConverter<TWriterClass>::UnaryOperandLocation::global(StringHandle name)  {
			UnaryOperandLocation loc;
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::UnaryOperandLocation IrToObjConverter<TWriterClass>::resolveUnaryOperandLocation(const IrInstruction& instruction, size_t operand_index)  {
		if (instruction.isOperandType<TempVar>(operand_index)) {
			auto temp = instruction.getOperandAs<TempVar>(operand_index);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::appendRipRelativePlaceholder(StringHandle global_name)  {
		uint32_t reloc_offset = static_cast<uint32_t>(textSectionData.size());
		textSectionData.push_back(0x00);
		textSectionData.push_back(0x00);
		textSectionData.push_back(0x00);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::loadValueFromStack(int32_t offset, int size_in_bits, X64Register target_reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitStoreWordToFrame(X64Register source_reg, int32_t offset)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitStoreByteToFrame(X64Register source_reg, int32_t offset)  {
		bool needs_rex = static_cast<uint8_t>(source_reg) >= static_cast<uint8_t>(X64Register::R8);
		if (needs_rex) {
			uint8_t rex = 0x40 | (1 << 2); // REX.R
			textSectionData.push_back(rex);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeValueToStack(int32_t offset, int size_in_bits, X64Register source_reg)  {
		switch (size_in_bits) {
		case 64:
		case 32:
			emitMovToFrameSized(
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::loadValueFromGlobal(StringHandle global_name, int size_in_bits, X64Register target_reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::moveImmediateToRegister(X64Register reg, uint64_t value)  {
		uint8_t rex = 0x48;
		if (static_cast<uint8_t>(reg) >= static_cast<uint8_t>(X64Register::R8)) {
			rex |= 0x01;
		}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::loadValuePointedByRegister(X64Register reg, int value_size_bits)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::loadValueFromReferenceSlot(int32_t offset, const typename IrToObjConverter<TWriterClass>::IndirectStorageInfo& ref_info, X64Register target_reg)  {
...
template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::loadAddressForOperand(const IrInstruction& instruction, size_t operand_index, X64Register target_reg)  {
		if (instruction.isOperandType<std::string_view>(operand_index)) {
			auto name = instruction.getOperandAs<std::string_view>(operand_index);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeValueToGlobal(StringHandle global_name, int size_in_bits, X64Register source_reg)  {
		uint8_t reg_bits = static_cast<uint8_t>(source_reg) & 0x07;
		bool needs_rex = static_cast<uint8_t>(source_reg) >= static_cast<uint8_t>(X64Register::R8);
		switch (size_in_bits) {
		case 64: {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::loadUnaryOperandValue(const typename IrToObjConverter<TWriterClass>::UnaryOperandLocation& location, int size_in_bits, X64Register target_reg)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeUnaryOperandValue(const typename IrToObjConverter<TWriterClass>::UnaryOperandLocation& location, int size_in_bits, X64Register source_reg)  {
		switch (location.kind) {
			case UnaryOperandLocation::Kind::Stack:
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeIncDecResultValue(TempVar result_var, X64Register source_reg, int size_in_bits)  {
...
template<class TWriterClass>
typename IrToObjConverter<TWriterClass>::UnaryOperandLocation IrToObjConverter<TWriterClass>::resolveTypedValueLocation(const TypedValue& typed_value)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitIncDecInstruction(X64Register target_reg, bool is_increment)  {
		uint8_t rex = 0x48;
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleIncDecCommon(const IrInstruction& instruction, typename IrToObjConverter<TWriterClass>::IncDecKind kind)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::storeConversionResult(const IrInstruction& instruction, X64Register result_reg, int size_in_bits)  {
		TempVar result_var;
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handlePreIncrement(const IrInstruction& instruction)  {
		handleIncDecCommon(instruction, IncDecKind::PreIncrement);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handlePostIncrement(const IrInstruction& instruction)  {
		handleIncDecCommon(instruction, IncDecKind::PostIncrement);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handlePreDecrement(const IrInstruction& instruction)  {
		handleIncDecCommon(instruction, IncDecKind::PreDecrement);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handlePostDecrement(const IrInstruction& instruction)  {
		handleIncDecCommon(instruction, IncDecKind::PostDecrement);
	}

template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleUnaryOperation(const IrInstruction& instruction, typename IrToObjConverter<TWriterClass>::UnaryOperation op)  {
		// Extract UnaryOp from typed payload
		const UnaryOp& unary_op = instruction.getTypedPayload<UnaryOp>();
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSignExtend(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleZeroExtend(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleTruncate(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatToInt(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleIntToFloat(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFloatToFloat(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleAddAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSubAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleMulAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleDivAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleModAssign(const IrInstruction& instruction)  {
		reserveDivisionFixedRegisters();
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleAndAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleOrAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleXorAssign(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleAssignment(const IrInstruction& instruction)  {
		...
		Type lhs_type = categoryToType(op.lhs.type_index.category());
		...
		if (lhs_is_reference) {
			...
			if (lhs_ref_info.has_value()) {
				...
				value_type = categoryToType(op.lhs.type_index.category());
		...
		// Get RHS source
		Type rhs_type = categoryToType(op.rhs.type_index.category());
		X64Register source_reg = X64Register::RAX;
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleLabel(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBranch(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleLoopBegin(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleLoopEnd(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleBreak(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleContinue(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleArrayAccess(const IrInstruction& instruction)  {
		...
		Type element_type = categoryToType(op.element_type_index.category());
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleArrayElementAddress(const IrInstruction& instruction)  {
...
void IrToObjConverter<TWriterClass>::handleArrayStore(const IrInstruction& instruction)  {
		...
		if (instruction.hasTypedPayload()) {
			...
			bool is_float_store = isFloatingPointType(op.element_type_index.category());
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleStringLiteral(const IrInstruction& instruction)  {
...
void IrToObjConverter<TWriterClass>::handleMemberAccess(const IrInstruction& instruction)  {
		...
		bool unresolved_user_defined_member = (member_size_bytes == 0 &&
			...
			(op.result.type_index.category() != TypeCategory::Struct || !op.result.type_index.is_valid()));
		...
		if (member_size_bytes > 8) {
			...
			setReferenceInfo(result_offset, categoryToType(op.result.type_index.category()), op.result.size_in_bits.value, false, result_var);
			return;
		}
			...
			if (is_float_type) {
				...
				if (member_size_bytes == 8) {
					...
					FLASH_LOG_FORMAT(Codegen, Warning,
						...
						member_size_bytes, StringTable::getStringView(op.member_name), static_cast<int>(categoryToType(op.result.type_index.category())),
		...
		if (op.is_reference()) {
			...
			regAlloc.release(temp_reg);
			setReferenceInfo(result_offset, categoryToType(op.result.type_index.category()), op.result.size_in_bits.value, op.is_rvalue_reference(), result_var);
			return;
		}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleAddressOf(const IrInstruction& instruction)  {
		// Address-of: &x
		...
		if (instruction.hasTypedPayload()) {
			...
			setAddressOnlyInfo(result_offset, categoryToType(op.operand.type_index.category()), op.operand.size_in_bits.value, op.result);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleAddressOfMember(const IrInstruction& instruction)  {
		...
		setAddressOnlyInfo(result_offset, categoryToType(op.member_type_index.category()), op.member_size_in_bits, op.result);
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleComputeAddress(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleDereference(const IrInstruction& instruction)  {
		// Dereference: *ptr
		// Check for typed payload
		if (instruction.hasTypedPayload()) {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleDereferenceStore(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleConditionalBranch(const IrInstruction& instruction)  {
		...
		} else if (const auto* ull_val = std::get_if<unsigned long long>(&condition_value)) {
			// Immediate value
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFunctionAddress(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleIndirectCall(const IrInstruction& instruction)  {
		...
		for (size_t i = 0; i < op.arguments.size() && i < 4; ++i) {
			const auto& arg = op.arguments[i];
			Type argType = categoryToType(arg.type_index.category());
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleTryBegin([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleTryEnd([[maybe_unused]] const IrInstruction& instruction)  {
		// Skip exception handling if disabled
		if (!g_enable_exceptions) {
			return;
		}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::materializeCatchObjectFromRax(const CatchBeginOp& catch_op)  {
			if (catch_op.exception_temp.var_number == 0) {
				return;
			}
			...
			if (catch_op.is_reference() || catch_op.is_rvalue_reference()) {
				...
				emitMovToFrame(X64Register::RAX, stack_offset, 64);
				setReferenceInfo(stack_offset, categoryToType(catch_op.type_index.category()),
				                 64,
			...
			switch (categoryToType(catch_op.type_index.category())) {
				case Type::Bool:
				case Type::Char:
			...
			if (is_builtin) {
				type_size_bits = get_type_size_bits(categoryToType(catch_op.type_index.category()));
			...
			if (g_enable_debug_output) {
				std::cerr << "[DEBUG][Codegen] CatchBegin: exception_type=" << static_cast<int>(catch_op.type_index.category())
...
bool IrToObjConverter<TWriterClass>::currentFunctionReturnsFloatingPointInXmm0() const  {
		return current_function_return_type_index_.category() != TypeCategory::Void &&
			isFloatingPointType(current_function_return_type_index_.category()) &&
...
template<class TWriterClass>
int IrToObjConverter<TWriterClass>::getCatchParentReturnSpillSizeBits() const  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitSavePendingCatchParentReturnValue()  {
		if (!currentFunctionHasCatchParentReturnValue()) {
			return;
		}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitRestorePendingCatchParentReturnValue()  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitRestorePendingCatchParentReturnValue(int32_t catch_return_slot_offset)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleCatchBegin(const IrInstruction& instruction)  {
		// Skip exception handling if disabled
		if (!g_enable_exceptions) {
			return;
		}
		...
		if (pending_catch_try_index_ != SIZE_MAX && pending_catch_try_index_ < current_function_try_blocks_.size()) {
			...
			handler.exception_type = categoryToType(catch_op.type_index.category());  // Bridge from TypeIndex
			...
			if (!handler.is_catch_all && catch_op.exception_temp.var_number != 0) {
				...
				if (!catch_op.is_reference() && !catch_op.is_rvalue_reference()) {
					if (catch_op.type_index.is_valid() && catch_op.type_index.index() < getTypeInfoCount()) {
						...
						int builtin_size = get_type_size_bits(categoryToType(catch_op.type_index.category()));
						if (builtin_size > 0) {
		...
		if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
			...
			if (!catch_op.is_catch_all && catch_op.exception_temp.var_number != 0 && catch_op.is_reference()) {
				int32_t stack_offset = getStackOffsetFromTempVar(catch_op.exception_temp);
				setReferenceInfo(stack_offset, categoryToType(catch_op.type_index.category()), 64, false, catch_op.exception_temp);
			}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleCatchEnd(const IrInstruction& instruction)  {
		// Skip exception handling if disabled
		if (!g_enable_exceptions) {
			return;
		}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleThrow(const IrInstruction& instruction)  {
		...
		size_t aligned_exception_size = (exception_size + 7) & ~7;
			...
			emitMovRegReg(X64Register::R15, X64Register::RAX);
				...
				if (throw_op.type_index.category() == TypeCategory::Struct && throw_op.type_index.is_valid() && !throw_op.value_is_materialized) {
					...
					source_value.type_index = TypeIndex{source_value.type_index.index(), throw_op.type_index.category()}; source_value.ir_type = toIrType(throw_op.type_index);
			...
			Type exception_type = categoryToType(throw_op.type_index.category());
			...
			if (!variable_scopes.empty()) {
				...
				std::string throw_destructor_symbol;
					...
					throw_type_name = std::string(getTypeName(categoryToType(throw_op.type_index.category())));
				}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleRethrow([[maybe_unused]] const IrInstruction& instruction)  {
		// Skip exception handling if disabled - generate abort() instead
		if (!g_enable_exceptions) {
			// Call abort() to terminate when exceptions are disabled
			emitCall("abort");
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitFuncletLeaRbpFromRdx(std::vector<uint32_t>& patch_list)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitWindowsCleanupFuncletsAndPopulateUnwindMap()  {
			if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
				return;
			} else {
				current_function_unwind_map_.clear();
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitJmpToLabel(StringHandle target_label)  {
			textSectionData.push_back(0xE9); // JMP rel32
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emitLeaLabelAddress(X64Register destination_register, StringHandle target_label)  {
			uint32_t patch_position = emitLeaRipRelative(destination_register);
			pending_branches_.push_back({target_label, patch_position});
		}

template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitJumpIfZeroRel32Placeholder()  {
			textSectionData.push_back(0x0F); // Two-byte opcode prefix
			textSectionData.push_back(0x84); // JZ rel32
...
template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitSubRSPImm32Placeholder()  {
			textSectionData.push_back(0x48); // REX.W
...
template<class TWriterClass>
uint32_t IrToObjConverter<TWriterClass>::emitLeaFromRSPDisp32Placeholder(X64Register destination_register)  {
			uint8_t rex = 0x48; // REX.W
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::patchRel32(uint32_t patch_position, uint32_t target_position)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleElfCatchNoMatch([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleFunctionCleanupLP(const IrInstruction& instruction)  {
			if (!g_enable_exceptions) return;
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehTryBegin([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehTryEnd([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehExceptBegin(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehExceptEnd([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehFinallyCall(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehFinallyBegin([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehFinallyEnd([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehFilterBegin([[maybe_unused]] const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehGetExceptionCode(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehGetExceptionInfo(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehFilterEnd(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehSaveExceptionCode(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehGetExceptionCodeBody(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehAbnormalTermination(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::handleSehLeave(const IrInstruction& instruction)  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emit_dynamic_cast_runtime_helpers()  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emit_dynamic_cast_check_function()  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::emit_dynamic_cast_throw_function()  {
		// Record function start
		uint32_t function_offset = static_cast<uint32_t>(textSectionData.size());
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::patchBranches()  {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::finalizeFunctionBranches()  {
		patchBranches();
		pending_branches_.clear();
		label_positions_.clear();
	}
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::addLineMapping(uint32_t line_number, int32_t manual_offset)  {
		if (current_function_name_.isValid()) {
...
template<class TWriterClass>
void IrToObjConverter<TWriterClass>::logAsmEmit(const char* context, const uint8_t* bytes, size_t count)  {
...
template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::isGlobalVariable(StringHandle name) const  {
		for (const auto& global : global_variables_) {
			if (global.name == name) {
				return true;
			}
...
template<class TWriterClass>
bool IrToObjConverter<TWriterClass>::CatchHandler::is_reference() const  { return ref_qualifier != CVReferenceQualifier::None; }
