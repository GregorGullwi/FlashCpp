...
uint32_t codeViewTypeIndex(TypeCategory type_cat) {
	...
	case TypeCategory::Float:
		return 0x40;	 // T_REAL32
	case TypeCategory::Double:
		return 0x41;	 // T_REAL64
	case TypeCategory::Char:
		return 0x10;	 // T_CHAR
	case TypeCategory::Bool:
		return 0x30;	 // T_BOOL08
	case TypeCategory::Struct:
		return 0x603;  // T_64PVOID for struct pointers / aggregate references
	default:
...
void IrToObjConverter<TWriterClass>::emitMovQwordPtrRegToRegOffset(X64Register base_reg, int32_t offset, X64Register src_reg) {
	...
	uint8_t rex = 0x48;
	if (static_cast<uint8_t>(src_reg) >= 8)
		rex |= 0x04;	 // REX.R if src is R8-R15
	if (static_cast<uint8_t>(base_reg) >= 8)
		rex |= 0x01; // REX.B if base is R8-R15
	textSectionData.push_back(rex);
...
void IrToObjConverter<TWriterClass>::emitMovRegReg(X64Register dest, X64Register src) {
	...
	if (static_cast<uint8_t>(src) >= 8)
		rex |= 0x04;	 // REX.R extends reg field (source)
	if (static_cast<uint8_t>(dest) >= 8)
		rex |= 0x01; // REX.B extends r/m field (dest)
...
void IrToObjConverter<TWriterClass>::emitCmpRegReg(X64Register r1, X64Register r2) {
	uint8_t rex = 0x48; // REX.W for 64-bit
	if (static_cast<uint8_t>(r1) >= 8)
		rex |= 0x01; // REX.B
	if (static_cast<uint8_t>(r2) >= 8)
		rex |= 0x04; // REX.R
...
void IrToObjConverter<TWriterClass>::emitCmpRegWithMem(X64Register reg, X64Register mem_base) {
	...
	if (static_cast<uint8_t>(reg) >= 8)
		rex |= 0x04; // REX.R
	if (static_cast<uint8_t>(mem_base) >= 8)
		rex |= 0x01; // REX.B
...
void IrToObjConverter<TWriterClass>::emitLeaRegScaledIndex(X64Register dest, X64Register base, X64Register index, uint8_t scale, int8_t disp) {
	...
	if (static_cast<uint8_t>(dest) >= 8)
		rex |= 0x04; // REX.R
	if (static_cast<uint8_t>(index) >= 8)
		rex |= 0x02; // REX.X
	if (static_cast<uint8_t>(base) >= 8)
		rex |= 0x01; // REX.B
...
void IrToObjConverter<TWriterClass>::handleTypeid(const IrInstruction& instruction) {
	...
	auto getTypeinfoSymbol = [&](TypeIndex type_index, StringHandle type_name_handle) -> std::string {
		if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
			if (type_index.category() == TypeCategory::Struct) {
				...
				if (type_name_handle.isValid()) {
					return writer.get_or_create_class_typeinfo(StringTable::getStringView(type_name_handle));
				}
				return {};
			}
	...
	if (op.is_type) {
		...
		if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
				// typeid(*null_polymorphic_ptr) must throw std::bad_typeid.
			emitTestRegReg(X64Register::RAX);
			size_t non_null_object_jump = textSectionData.size();
			emitJumpIfNotZero(0);
...
uint16_t IrToObjConverter<TWriterClass>::getX64RegisterCodeViewCode(X64Register reg) {
	...
	case X64Register::XMM0:
		return 154;	// CV_AMD64_XMM0
	case X64Register::XMM1:
		return 155;	// CV_AMD64_XMM1
	case X64Register::XMM2:
		return 156;	// CV_AMD64_XMM2
	case X64Register::XMM3:
		return 157;	// CV_AMD64_XMM3
	case X64Register::XMM4:
		return 158;	// CV_AMD64_XMM4
	case X64Register::XMM5:
		return 159;	// CV_AMD64_XMM5
	case X64Register::XMM6:
		return 160;	// CV_AMD64_XMM6
	case X64Register::XMM7:
		return 161;	// CV_AMD64_XMM7
	case X64Register::XMM8:
		return 162;	// CV_AMD64_XMM8
	case X64Register::XMM9:
		return 163;	// CV_AMD64_XMM9
	case X64Register::XMM10:
		return 164; // CV_AMD64_XMM10
	case X64Register::XMM11:
		return 165; // CV_AMD64_XMM11
	case X64Register::XMM12:
		return 166; // CV_AMD64_XMM12
	case X64Register::XMM13:
		return 167; // CV_AMD64_XMM13
	case X64Register::XMM14:
		return 168; // CV_AMD64_XMM14
	case X64Register::XMM15:
		return 169; // CV_AMD64_XMM15
	default:
		throw InternalError("Unsupported X64Register");
	}
}
...
void IrToObjConverter<TWriterClass>::handleFunctionDecl(const IrInstruction& instruction) {
	...
	if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
		current_function_cfi_.push_back({ElfFileWriter::CFIInstruction::PUSH_RBP,
										 static_cast<uint32_t>(textSectionData.size() - current_function_offset_),
										 0});
	}
	...
	if (use_eh_prologue_style) {
		...
		textSectionData.push_back(0xEC); // mov rbp, rsp

			// Track CFI: After mov rbp, rsp, CFA = RBP+16
		if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
			current_function_cfi_.push_back({ElfFileWriter::CFIInstruction::MOV_RSP_RBP,
											 static_cast<uint32_t>(textSectionData.size() - current_function_offset_),
											 0});
		}
	...
	next_temp_var_offset_ = 8 + static_cast<int>(current_function_reserved_catch_obj_padding_size_ + current_function_reserved_catch_return_slot_size_);
...
int32_t IrToObjConverter<TWriterClass>::ensureCatchFuncletReturnSlot() {
	...
	if (current_function_reserved_catch_return_slot_size_ >= 16) {
		uint32_t base_named_vars_size = current_function_named_vars_size_ - current_function_reserved_catch_ref_temp_size_ - current_function_reserved_catch_obj_padding_size_ - current_function_reserved_catch_return_slot_size_;
		int32_t reserved_base = -static_cast<int32_t>(base_named_vars_size + current_function_reserved_catch_ref_temp_size_ + current_function_reserved_catch_obj_padding_size_);
...
void IrToObjConverter<TWriterClass>::handleReturn(const IrInstruction& instruction) {
	...
	if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
		...
		current_function_cfi_.push_back({ElfFileWriter::CFIInstruction::REMEMBER_STATE,
		...
		current_function_cfi_.push_back({ElfFileWriter::CFIInstruction::POP_RBP,
										 static_cast<uint32_t>(textSectionData.size() - current_function_offset_),
										 0});
	}
	...
	if constexpr (std::is_same_v<TWriterClass, ElfFileWriter>) {
		current_function_cfi_.push_back({ElfFileWriter::CFIInstruction::RESTORE_STATE,
										 static_cast<uint32_t>(textSectionData.size() - current_function_offset_),
										 0});
	}
...
void IrToObjConverter<TWriterClass>::relocateLhsOutOfRCX(ArithmeticOperationContext& ctx) {
	...
	if (ctx.result_physical_reg != X64Register::RCX)
		return;
...
void IrToObjConverter<TWriterClass>::handleSignExtend(const IrInstruction& instruction) {
	...
	if (fromSize == 8 && (toSize == 32 || toSize == 64)) {
		...
		if (static_cast<uint8_t>(result_reg) >= 8)
			rex |= 0x04; // REX.R
		if (static_cast<uint8_t>(source_reg) >= 8)
			rex |= 0x01; // REX.B
...
void IrToObjConverter<TWriterClass>::handleTruncate(const IrInstruction& instruction) {
	...
	if (toSize == 8) {
		...
		if (static_cast<uint8_t>(result_reg) >= 8 || static_cast<uint8_t>(source_reg) >= 8) {
			uint8_t rex = 0x40;
			if (static_cast<uint8_t>(source_reg) >= 8)
				rex |= 0x04; // REX.R for source in reg field
			if (static_cast<uint8_t>(result_reg) >= 8)
				rex |= 0x01; // REX.B for dest in r/m field
...
void IrToObjConverter<TWriterClass>::handleElfCatchNoMatch([[maybe_unused]] const IrInstruction& instruction) {
	...
	if (elf_exc_ptr_offset_ == 0)
		return;	// No active landing pad context
