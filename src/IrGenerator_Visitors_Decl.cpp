...
void AstToIr::visitFunctionDeclarationNode(const FunctionDeclarationNode& node) {
	...
	if (function_operator_kind == OverloadableOperator::Spaceship && node.is_implicit()) {
				...
				int member_bits = static_cast<int>(member.size * 8);
					...
					if (member.type_index.category() == TypeCategory::Struct) {
						...
						if (member_spaceship_mangled.isValid()) {
							...
							{
								emitReturn(IrValue{call_result}, nativeTypeIndex(TypeCategory::Int), 32,
					...
					{
						emitReturn(IrValue{0xFFFFFFFFULL}, nativeTypeIndex(TypeCategory::Int), 32,
							func_decl.identifier_token(), ValueStorage::ContainsData);
					}
					...
					{
						emitReturn(IrValue{1ULL}, nativeTypeIndex(TypeCategory::Int), 32,
							func_decl.identifier_token(), ValueStorage::ContainsData);
					}
		...
		emitReturn(IrValue{0ULL}, nativeTypeIndex(TypeCategory::Int), 32,
			func_decl.identifier_token(), ValueStorage::ContainsData);
	...
	if (synthesized_cmp_opcode) {
		...
		if (spaceship_mangled.isValid()) {
			...
			emitReturn(IrValue{cmp_result}, nativeTypeIndex(TypeCategory::Bool), 8,
				func_decl.identifier_token(), ValueStorage::ContainsData);
			...
			emitReturn(IrValue{1ULL}, nativeTypeIndex(TypeCategory::Bool), 8,
				func_decl.identifier_token(), ValueStorage::ContainsData);
			ir_.addInstruction(IrInstruction(IrOpcode::Label, LabelOp{.label_name = equal_false_label}, func_decl.identifier_token()));
			emitReturn(IrValue{0ULL}, nativeTypeIndex(TypeCategory::Bool), 8,
				func_decl.identifier_token(), ValueStorage::ContainsData);
		} else {
			...
			if (type_it != getTypesByNameMap().end()) {
				...
				if (struct_info) {
					...
					emitReturn(StringTable::getOrInternStringHandle("this"),
						...
						func_decl.identifier_token(), ValueStorage::ContainsAddress);
				}
			}
		}
	} else {
...
void AstToIr::visitEnumDeclarationNode(const EnumDeclarationNode& node) {
	...
	for (const Enumerator& e : enum_info->enumerators) {
		...
		ASTNode type_node = ASTNode::emplace_node<TypeSpecifierNode>(
			type_info->type_index_.withCategory(TypeCategory::Enum), enum_info->sizeInBits().value, enumerator_token, CVQualifier::None, ReferenceQualifier::None);
...
void AstToIr::visitConstructorDeclarationNode(const ConstructorDeclarationNode& node) {
	...
	auto emitVptrStores = [&](const StructTypeInfo* struct_info, const TypeInfo* struct_type_info, bool include_virtual_base_vptrs) {
		...
		auto emitVirtualBaseTable = [&](const StructTypeInfo* source_struct_info,
			...
			TemplateVector<int32_t, 1> offsets;
...
ExprResult AstToIr::generateInitializerListConstructionIr(const InitializerListConstructionNode& init_list) {
	...
	int init_list_size_bits = static_cast<int>(init_list_struct_info->sizeInBits().value);
			...
			constructorCallNode.arguments().visit([&](ASTNode argument) {
				...
				if (argument.is<InitializerListNode>() &&
					...
					if (nested_ti && nested_ti->getStructInfo()) {
						int nested_bits = nested_ti->getStructInfo()->sizeInBits().value;
