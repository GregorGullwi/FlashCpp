...
{
	FLASH_LOG(Parser, Debug, "parse_statement_or_declaration: current_token = ", current_token_.has_value() ? current_token_->value() : "none");
...
{
	FLASH_LOG(Parser, Debug, "parse_unary_expression: current_token = ", current_token_.has_value() ? current_token_->value() : "none");
...
{
	FLASH_LOG(Parser, Debug, "parse_expression: current_token = ", current_token_.has_value() ? current_token_->value() : "none");
	...
	FLASH_LOG(Parser, Debug, "Returning from parse_expression, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
	return result;
}
...
{
	FLASH_LOG(Parser, Debug, "parse_primary_expression: current_token = ", current_token_.has_value() ? current_token_->value() : "none");
	...
	// Check for global namespace scope operator :: at the beginning
		...
		FLASH_LOG(Parser, Debug, "Before consuming literal, current_token = ", current_token_->value());
		consume_token();
		FLASH_LOG(Parser, Debug, "After consuming literal, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
	}
...
found_member_variable:  // Label for member variable detection - jump here to skip error checking
	...
	while (result.has_value() && peek_token().has_value()) {
		FLASH_LOG(Parser, Debug, "Postfix loop: peek_token = ", peek_token()->value(), ", type = ", static_cast<int>(peek_token()->type()));
		...
		if (peek_token()->type() == Token::Type::Punctuator && peek_token()->value() == "[") {
			FLASH_LOG(Parser, Debug, "Found '[' for array subscript, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
			Token bracket_token = *peek_token();
			consume_token(); // consume '['

			// Parse the index expression
			FLASH_LOG(Parser, Debug, "Parsing array index, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
		...
		if (peek_token()->type() == Token::Type::Punctuator && peek_token()->value() == "."sv) {
			...
			FLASH_LOG(Parser, Debug, "Breaking from postfix loop, peek_token = ", peek_token().has_value() ? peek_token()->value() : "none");
	...
	FLASH_LOG(Parser, Debug, "End of parse_primary_expression, result.has_value = ", result.has_value());
	if (result.has_value())
		return ParseResult::success(*result);
...
ParseResult Parser::parse_lambda_expression() {
        ...
        while (true) {
            ...
            if (token->value() == "=") {
                ...
                if (token->value() == "this") {
                    ...
                    FLASH_LOG(Parser, Debug, "Lambda capture: id_token = ", id_token.value());
                    consume_token();
                    
                    FLASH_LOG(Parser, Debug, "After consuming id, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
                    
                    // Check for init-capture: [x = expr]
                    if (peek_token().has_value() && peek_token()->value() == "=") {
                        FLASH_LOG(Parser, Debug, "Found '=', parsing init-capture");
                        consume_token(); // consume '='
            ...
            if (peek_token().has_value() && peek_token()->value() == ",") {
                ...
                FLASH_LOG(Parser, Debug, "Breaking from capture list loop");
                break;
            }
        }
    }
    ...
    // Expect ']'
    FLASH_LOG(Parser, Debug, "After capture list loop, expecting ']', current_token = ", current_token_.has_value() ? current_token_->value() : "none");
    if (!consume_punctuator("]")) {
    ...
    if (peek_token().has_value() && peek_token()->value() == "(") {
        FLASH_LOG(Parser, Debug, "Parsing lambda parameter list");
        FlashCpp::ParsedParameterList params;
        auto param_result = parse_parameter_list(params);
        if (param_result.is_error()) {
            return param_result;
        }
        FLASH_LOG(Parser, Debug, "After parsing parameter list, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
    ...
    FLASH_LOG(Parser, Debug, "Before parse_block, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
    ParseResult body_result = parse_block();
    FLASH_LOG(Parser, Debug, "After parse_block, current_token = ", current_token_.has_value() ? current_token_->value() : "none");
