...
private:
	...
	ASTNode substituteTypeidExpr(const TypeidNode& typeid_expr);
	...
	struct DependentNameRestampResult {
		DependentNameRestampAction action;
		TypeId type;
	};
	DependentNameRestampResult tryRestampDependentNameType(const TypeSpecifierNode& type) const;
	...
	MaterializedStoredTemplateArgs materializeStoredTemplateArgs(
		...
		int depth);
	TemplateArgumentVector materializeDependentRecordTemplateArgs(
