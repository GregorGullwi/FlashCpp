...

		{
			...
			if (!catch_clause.is_catch_all()) {
				...
				if (!exception_var_name.empty()) {
					...
					init_value.pointer_depth = PointerDepth{static_cast<int>(type_node.runtime_pointer_depth())};
	...
	if (node.is_rethrow()) {
		...
		if (!catch_scope_stack_.empty()) {
			...
			if (needs_materialization) {
				...
				materialized_throw_decl.type_index = exception_type_index.withCategory(expr_type);
