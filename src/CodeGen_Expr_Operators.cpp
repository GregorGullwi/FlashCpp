		...
		if (lhsType == Type::Struct && lhsIrOperands.size() >= 4) {
			...
			if (overloadable_binary_ops.count(op) > 0 && lhs_type_index > 0) {
				...
				if (overload_result.has_overload) {
					...
					// Resolve self-referential return types (e.g., W& → W<int>&) for template structs
					if (return_type.type() == Type::Struct && return_type.type_index() > 0 && return_type.type_index() < gTypeInfo.size()) {
						...
						if (!ret_ti.struct_info_ || ret_ti.struct_info_->total_size == 0) {
							if (lhs_type_index < gTypeInfo.size()) {
								return_type.set_type_index(lhs_type_index);
							}
						}
					}
					...
					for (const auto& param_node : func_decl.parameter_nodes()) {
						if (param_node.is<DeclarationNode>()) {
							...
							// For template-instantiated structs, the stored member function may
							// reference the template base class (e.g., W) instead of the
							// instantiation (e.g., W<int>). Resolve self-references so the mangled
							// name matches the definition generated by lazy instantiation.
							if (param_type.type() == Type::Struct && param_type.type_index() > 0 && param_type.type_index() < gTypeInfo.size()) {
								auto& param_ti = gTypeInfo[param_type.type_index()];
								if (!param_ti.struct_info_ || param_ti.struct_info_->total_size == 0) {
									// Unfinalized struct — likely the template base. Resolve to
									// the enclosing instantiated struct if its name matches.
									if (lhs_type_index < gTypeInfo.size()) {
										param_type.set_type_index(lhs_type_index);
									}
								}
							}
							param_types.push_back(param_type);
						...
						if (spaceship_op && spaceship_op->function_decl.is<FunctionDeclarationNode>()) {
							...
							for (const auto& param_node : func_decl.parameter_nodes()) {
								if (param_node.is<DeclarationNode>()) {
									const auto& param_decl = param_node.as<DeclarationNode>();
									TypeSpecifierNode param_type = param_decl.type_node().as<TypeSpecifierNode>();
									// Resolve self-referential types for template structs
									if (param_type.type() == Type::Struct && param_type.type_index() > 0 && param_type.type_index() < gTypeInfo.size()) {
										auto& param_ti = gTypeInfo[param_type.type_index()];
										if (!param_ti.struct_info_ || param_ti.struct_info_->total_size == 0) {
											if (lhs_type_index < gTypeInfo.size()) {
	...
	std::string_view generateMangledNameForCall(const FunctionDeclarationNode& func_node, std::string_view struct_name_override = "", const std::vector<std::string>& namespace_path = {}) {
		...
		if (!struct_name.empty()) {
			...
			if (struct_it != gTypesByName.end()) {
				...
				// Quick check: do any param types need resolution?
					...
					for (const auto& param : func_node.parameter_nodes()) {
						if (param.is<DeclarationNode>()) {
							...
							if (pt.type() == Type::Struct && pt.type_index() > 0 && pt.type_index() < gTypeInfo.size()) {
								auto& ti = gTypeInfo[pt.type_index()];
								if (!ti.struct_info_ || ti.struct_info_->total_size == 0) {
									pt.set_type_index(struct_type_index);
								}
							}
