...
struct TempVarMetadata {
	...
	static TempVarMetadata makeLValue(LValueInfo lv_info, TypeIndex type_index = TypeIndex{}, int size_bits = 0) {
		TempVarMetadata meta;
		meta.category = ValueCategory::LValue;
		meta.lvalue_info = lv_info;
		meta.value_type_index = type_index;
		meta.ir_type = toIrType(type_index);
		meta.value_size_bits = SizeInBits{size_bits};
		return meta;
	}
	
	// Helper to create xvalue metadata
	static TempVarMetadata makeXValue(LValueInfo lv_info, TypeIndex type_index = TypeIndex{}, int size_bits = 0) {
		TempVarMetadata meta;
	...
	static TempVarMetadata makeReference(TypeIndex type_index, SizeInBits size_bits, ValueCategory category) {
		TempVarMetadata meta;
	...
	static TempVarMetadata makeAddressOnly(TypeIndex type_index, SizeInBits size_bits, ValueCategory category) {
		TempVarMetadata meta;
		meta.category = category;
