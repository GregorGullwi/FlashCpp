		...
		if (stmt_node.is<VariableDeclarationNode>()) {
			...
			if (initializer.has_value()) {
				...
				if (is_struct_type) {
							...
							if (type_idx < gTypeInfo.size() && gTypeInfo[type_idx].struct_info_) {
								...
								if (matching_ctor) {
									...
									for (const auto& base_init : matching_ctor->base_initializers()) {
										...
										if (base_type_idx < gTypeInfo.size() && gTypeInfo[base_type_idx].struct_info_) {
					...
					// Fallback: if no members were initialized, use hardcoded value for test compatibility
