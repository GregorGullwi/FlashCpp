...
struct EvalResult {
	...
	static EvalResult indeterminate() {
		EvalResult result = from_int(0LL);
...
public:
	...
	struct ResolvedConstexprObject {
		...
		std::optional<EvalResult> materialized_value; // Phase C: pre-materialized scalar fields from constant bytes
	};
...
private:
	...
	static EvalResult evaluate_delete_expression(
	...
	static std::optional<EvalResult> try_evaluate_callable_initializer_for_call(
