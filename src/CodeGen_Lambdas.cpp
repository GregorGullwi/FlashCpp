#include "CodeGen.h"
	...
	ExprResult AstToIr::generateLambdaExpressionIr(const LambdaExpressionNode& lambda, std::string_view target_var_name) {
		...
		return makeExprResult(Type::Struct, closure_size_bits, IrOperand{StringTable::getOrInternStringHandle(closure_var_name)}, 0, 0, static_cast<unsigned long long>(closure_type->type_index_));
	}
