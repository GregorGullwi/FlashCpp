...
#include "TemplateInlineVectorAliases.h"
...
struct TemplateTypeArg {
	...
	// Drop dependent-placeholder bits once the argument already names a concrete type.
	// Instantiation cycle keys must treat `int` and a still-flagged dependent `int`
	// as the same specialization request.
	TemplateTypeArg canonicalizeForInstantiationIdentity() const {
		TemplateTypeArg canonical = *this;
		...
		if (concrete) {
			...
			canonical.dependent_name = StringHandle{};
			canonical.dependent_expr.reset();
		}
		return canonical;
	}
	...
	if (!substituted_dependent_name && arg_info.dependent_expr().has_value() && arg_info.is_value) {
		...
		if constexpr (!std::is_null_pointer_v<std::decay_t<EvalFn>>) {
			...
			if constexpr (std::is_same_v<TemplateParamNodeType, ASTNode>) {
				...
				TemplateAstNodeVector ast_template_params;
