...
private:
	...
	// True only while reparsing a published function-template body. This keeps
...
std::optional<CallArgDeductionInfo> buildDeductionMapFromCallArgs(
	...
	static void copyDefinitionParameterTypes(
		std::span<ASTNode> instantiated_params,
		std::span<const ASTNode> definition_params);
	static void syncOutOfLineConstructorTemplateParameters(
		std::span<ASTNode> instantiated_params,
		std::span<const ASTNode> definition_params);
...
private:	 // Resume private methods
	...
	void stampActiveTemplateParameterDecl(TypeSpecifierNode& type_spec, StringHandle param_name) {
		...
		// templates, and member templates under unpublished enclosing classes).
		// Member primaries under published classes or primary class templates set
		// active_template_decl_id_ while their body is parsed. Stamping is a no-op
		// when that id is empty.
	...
	// Stamp dependent-member chains rooted in a published type parameter.
	// Plain segments use DependentName; type-only template-id segments use
	// DependentTemplateMember. No-op for unpublished templates, non-type owners,
	// other qualifier families, NTTP/pack/template-template args, or any argument
	// that does not import Supported. ::template is ignored. Does not recover
	// identity from flat TypeIndex names.
	void tryStampDependentMemberChain(
		TypeSpecifierNode& type_spec,
