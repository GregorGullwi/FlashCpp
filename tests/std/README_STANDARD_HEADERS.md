...
| `<span>` | `test_std_span.cpp` | ❌ Codegen Error | ~1440ms (targeted retest 2026-04-07). The older `__detail::__extent_storage` primary-template stop is still gone, and the latest namespace-scope empty-brace / empty-object-size fixes also remove the bogus `_Synth3way` constructor failure from the ranges CPO setup. The current blocker remains later iterator/ranges fallout: placeholder-sized `move` / `fill_n` parameters, missing `_Extent` symbol recovery in `__extent_storage::_M_extent`, and a final `Operator-` failure. |
