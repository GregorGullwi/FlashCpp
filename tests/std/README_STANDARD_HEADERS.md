...
| `<map>` | `test_std_map.cpp` | ❌ Compile Error | 10.03s compile (retested 2026-08-22, Windows/MSVC STL 14.44). Instantiated `noexcept` still fails: `conjunction$…::value` is an undefined qualified identifier after `_Conjunction<static_cast<bool>(Trait::value), …>` base materialization. Member-template defaults that spell the injected-class-name (`Myself = pair`) now bind the current specialization (`tests/test_injected_class_name_member_default_ret0.cpp`); remaining `_Tree` traits still skip the `_Conjunction` NTTP. Reduced non-`std` coverage of inherited-trait-value folding through deferred `Helper<T>::type` bases lives in `tests/test_conjunction_inherited_value_deferred_base_ret0.cpp`. |
...
- Empty trailing class-template packs are now recorded in the instantiation context (`tests/test_class_template_trailing_empty_pack_ret0.cpp`). Variadic `__is_nothrow_constructible(T, Args...)` in `noexcept` is covered by `tests/test_nothrow_constructible_pack_noexcept_ret0.cpp`; MSVC-style `conjunction` with `static_cast<bool>(Trait::value)` is covered by `tests/test_conjunction_static_cast_bool_nttp_noexcept_ret0.cpp` and `tests/test_conjunction_nothrow_assignable_noexcept_ret0.cpp`. Injected-class-name member-template defaults such as `template<class Myself = Pair>` now denote the current specialization (`tests/test_injected_class_name_member_default_ret0.cpp`). Variable templates instantiated with an empty pack argument (e.g. `conjunction_v<>`) no longer keep the stale dependent-qualified record on the Phase-3-rebound initializer qualifier (`tests/test_conjunction_inherited_value_deferred_base_ret0.cpp`); before that fix the instantiation was emitted as a non-constant global and read `false` at runtime. Nested STL `conjunction$…::value` after deferred `_Conjunction` bases can still fail in `<map>`/`<set>`/`<iostream>` when `Trait::value` is an inherited static member of a lazy trait instantiation.
- Local pointer-to-array initialization and pointer-to-array parameters now work end to end: declarator structure is recorded on the type (`pointee_array_declarator`, C++20 [dcl.ptr]/1), parameters skip the array-to-pointer adjustment ([dcl.fct]/5), locals lower as scalar pointers that keep their initializer, and `*p` yields an address-valued array lvalue instead of a load ([expr.unary.op]/1, [conv.array]). Covered by `tests/test_local_pointer_to_array_init_ret42.cpp` and `tests/test_pointer_to_array_param_ret42.cpp`. Remaining gaps live in docs/KNOWN_ISSUES.md: some flat consumers still cannot handle interleaved pointer/array declarators, and alias-template targets with pointer-to-array declarators remain unsupported.
