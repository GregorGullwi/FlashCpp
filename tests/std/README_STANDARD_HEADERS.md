...
|--------|-----------|--------|-------|
| `<limits>` | `test_std_limits.cpp` | ✅ Compiled | ~5303ms (`TOTAL`) / ~5.4s wall (retested 2026-07-28, Windows/MSVC STL 14.44). |
| `<type_traits>` | `test_std_type_traits.cpp` | ✅ Compiled | ~1156ms (`TOTAL`) / ~1.2s wall (retested 2026-07-28, Windows/MSVC STL 14.44). |
...
| `<ratio>` | `test_std_ratio.cpp` | ✅ Compiled | ~5010ms (`TOTAL`) / ~5.1s wall (retested 2026-07-28, Windows/MSVC STL 14.44). Definition-context lookup now preserves namespace-scope constexpr calls during template replay, and deferred base aliases resolve in their declaration namespace. Regressions: `tests/test_dependent_base_static_member_multi_expr_ret0.cpp` and `tests/std/test_std_ratio_equal_only.cpp`. |
| `<optional>` | `test_std_optional.cpp` | ❌ Parse Error | ~8305ms (`TOTAL`) / ~8.4s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Shared `view_interface::empty` `auto& _Self = _Cast()` stop is fixed; first hard stop is again structural class-type NTTP in MSVC `optional:269`. |
...
| `<utility>` | `test_std_utility.cpp` | ✅ Compiled | ~1515ms (`TOTAL`) / ~1.6s wall (retested 2026-07-28, Windows/MSVC STL 14.44). Template-parameter spelling now wins over stale dependent `TypeInfo`, so `pair<int, float>::first` has type `int`. Object generation succeeds; the linked probe still crashes at runtime, leaving a separate codegen issue. |
| `<concepts>` | `test_std_concepts.cpp` | ✅ Compiled | ~1275ms (`TOTAL`) / ~1.3s wall (retested 2026-07-28, Windows/MSVC STL 14.44). |
| `<bit>` | `test_std_bit.cpp` | ✅ Compiled | ~1083ms (retested 2026-05-23, Linux/libstdc++-14). |
| `<string_view>` | `test_std_string_view.cpp` | ❌ Compile Error | ~8725ms (`TOTAL`) / ~8.8s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Past `view_interface::empty` auto deduction; current stop is instantiated `noexcept` specs that are not constant expressions after substitution. |
...
| `<tuple>` | `test_std_tuple.cpp` | ❌ Semantic Error | ~1990ms (`TOTAL`) / ~2.0s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Nested variadic function-template deduction and pack replay through dependent alias return types are fixed generically, including the explicit NTTP in a `get<0>`-shaped call. The full header still fails `auto val = std::get<0>(t)`: its next blocker is materializing the MSVC `tuple_element_t` return alias across the friend declaration/overload set. Link of a construct-only probe also still misses emitted `tuple`/`swap` symbols. |
| `<vector>` | `test_std_vector.cpp` | ❌ Compile Error | ~9552ms (`TOTAL`) / ~9.6s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Past `view_interface::empty` CRTP `auto&` deduction; current stop is instantiated `noexcept` specs that are not constant expressions after substitution. Regressions: `tests/test_nested_template_member_access_auto_ref_ret42.cpp`, `tests/test_crtp_auto_ref_from_member_call_ret0.cpp`. |
...
| `<vector>` | Sema: instantiated `noexcept` expression not constant after substitution | Constexpr-evaluate substituted member `noexcept` specs (including dependent calls) during class/member instantiation |
| `<string_view>` | Sema: instantiated `noexcept` expression not constant after substitution | Same noexcept constant-evaluation path as `<vector>` |
...
For the next `<vector>` / `<string_view>` implementation, the shared first failure is `Instantiated noexcept specification is not a constant expression` in `substituteInstantiatedNoexceptSpecification` / lazy member noexcept evaluation. After template-parameter substitution the `noexcept(...)` operand still fails both `ConstExpr::Evaluator` and `try_evaluate_constant_expression` even though the template args are concrete. Reduce with a generic member `noexcept(expr)` that depends on a substituted call/trait before changing the standard-header path.
