...
| `<string>` | `test_std_string.cpp` | ❌ Compile Error | ~4790ms (targeted retest 2026-04-06). Constructor-style calls through typedef / using aliases such as `wstring(...)` now parse as temporary object construction instead of falling into function-call lookup. `<string>` gets past the old `No matching function for call to 'wstring'` stop and currently fails later on the phase-1 non-dependent-name error for `__str` in `basic_string.h`. |
...
| `<ranges>` | `test_std_ranges.cpp` | ❌ Compile Error | ~5350ms (targeted retest 2026-04-06). The earlier phase-1 `streamoff` and `size_t(-1)` fixes still hold, and alias-based constructor parsing now also gets through the old `wstring(...)` stop inherited from `<string>`. The current blocker is the later phase-1 non-dependent-name error for `__str` in `basic_string.h`. |
...
| `<stdexcept>` | `test_std_stdexcept.cpp` | ❌ Compile Error | ~4850ms (targeted retest 2026-04-06). Same new state as `<string>`: alias-based constructor calls now parse correctly, so the old `wstring(...)` failure is gone. The current blocker is the later phase-1 non-dependent-name error for `__str` in `basic_string.h`. |
