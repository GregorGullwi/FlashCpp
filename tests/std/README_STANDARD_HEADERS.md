...
| `<atomic>` | `test_std_atomic.cpp` | ❌ Compile Error | ~770ms (targeted retest 2026-04-06). Dependent member aliases used inside `sizeof(...)` now remap through the instantiated template parameter, so libstdc++ gets past the old `bits/atomic_wait.h` `sizeof(__waiter_type)` static-assert stop. The current blocker is still a later `Itanium name mangling: unknown type` follow-on deeper in the atomic implementation. |
...
| `<latch>` | `test_std_latch.cpp` | ❌ Compile Error | ~760ms (targeted retest 2026-04-06). Shares the same new `<atomic>` state: the old `bits/atomic_wait.h` `sizeof(__waiter_type)` blocker is gone, and the current failure remains a later `Itanium name mangling: unknown type` follow-on in the atomic implementation stack. |
...
4. **Template-expression substitution now remaps dependent member aliases through the canonical instantiated template-parameter name before evaluating the type**: when a templated body replays `sizeof(value_type)` or libstdc++'s `sizeof(__waiter_type)`, the substitutor now falls back from the alias token spelling to the underlying template-parameter `TypeInfo` name and then substitutes that concrete argument. This removes the old `bits/atomic_wait.h` `sizeof(__waiter_type)` static-assert failure and moves `<atomic>` / `<latch>` to the later mangling follow-on. Regression test: `tests/test_member_alias_sizeof_static_assert_ret0.cpp`.
