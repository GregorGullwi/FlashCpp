...
| `<deque>` | `test_std_deque.cpp` | ❌ Compile Error | ~920ms (targeted retest 2026-04-09). Deferred alias-template reparsing now handles leading cv-qualified targets such as `const remove_reference_t<_Tp>&`, so `<deque>` gets past the earlier `<concepts>` alias stop. The current blocker is later in `bits/stl_pair.h` where `pair::swap` still resolves `swap(first, __p.first)` to a deleted candidate during template instantiation. |
...
| `<memory>` | `test_std_memory.cpp` | ❌ Compile Error | ~1640ms (targeted retest 2026-04-09). The `<concepts>` alias-template blocker is fixed, so `<memory>` now gets further and currently stops in `bits/stl_pair.h` on `pair::swap` where `swap(first, __p.first)` is still resolved as a deleted function during template instantiation. |
| `<functional>` | `test_std_functional.cpp` | ❌ Compile Error | ~1150ms (targeted retest 2026-04-09). Same new state as `<memory>` / `<deque>`: the earlier `<concepts>` alias-template parse failure is gone, and the current blocker is the later deleted-`swap` resolution in `bits/stl_pair.h`'s `pair::swap`. |
| `<map>` | `test_std_map.cpp` | ❌ Codegen Error | member 'first' not found in struct 'std::iterator' |
| `<set>` | `test_std_set.cpp` | ❌ Compile Error | ~910ms (targeted retest 2026-04-09). The earlier `<concepts>` alias-template parse failure is fixed; `<set>` now progresses into `bits/stl_pair.h` and currently fails on the same deleted-`swap` resolution in `pair::swap`. |
