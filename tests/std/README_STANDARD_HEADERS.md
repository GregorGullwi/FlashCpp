...
| `<any>` | `test_std_any.cpp` | ❌ Codegen Error | ~1.38s (retested 2026-05-24, Linux/libstdc++-14). The sema-normalized converting-constructor annotation hard stop in `main` is now bypassed by codegen fallback constructor overload selection; current first hard stop is unresolved semantic type category 25 for local `__arg`. |
...
### 2026-05-24 Linux/libstdc++ call-return placeholder-auto + normalized converting-constructor fallback follow-up
...
- **Normalized-body copy-initialization no longer hard-fails when sema did not pre-annotate a converting constructor; codegen now falls back to existing constructor overload resolution in that path.**
