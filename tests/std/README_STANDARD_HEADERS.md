...
**Targeted retest note (2026-04-05):** `<atomic>` and `<latch>` were re-checked again after the namespace-qualified explicit-function-template deduction fix and new `__builtin_memcpy` builtin support below. They now get past the old `std::__atomic_impl::__compare_exchange<_AtomicRef>` / `__builtin_memcpy` stop and fail later in `std::__atomic_impl::store` with an `Itanium name mangling: unknown type` abort. `<ratio>` was also re-checked: it still compiles, but `std::ratio_less` remains blocked by non-type default template arguments that depend on qualified constexpr members. Fresh 2026-04-05 targeted retests for `<chrono>` and `<shared_mutex>` confirm the old parse blocker remains gone; `<chrono>` still bottoms out at the later `__time_point` non-dependent-name error after repeated ratio/time_point static-assert fallout, while `<shared_mutex>` now reaches a later codegen failure on missing symbol `_S_epoch_diff` instead of the older `denorm_absent` stop. The earlier 2026-04-04 retest results still apply for `<optional>`, `<variant>`, `<string_view>`, `<array>`, `<algorithm>`, and `<vector>`: the wide-`wmemchr` ambiguity is still gone, infix free operator templates still instantiate in simple cases, CRTP-style `static_cast<const Derived*>(this)` codegen no longer erases the target type, chained member-access IR now preserves concrete user-defined payload types in simple inherited cases, and the old `<variant>` post-parse pack-expansion stop is gone. The overall header counts above still reflect the older full sweep and need a future comprehensive rerun before they are updated.
