...
| `<optional>` | `test_std_optional.cpp` | ❌ Parse Error | ~9179ms (`TOTAL`) / ~12.1s runner wall (retested 2026-07-27, Windows/MSVC STL 14.44). First hard stop is structural class-type non-type template parameters in MSVC `optional:269`. |
| `<any>` | `test_std_any.cpp` | ❌ Codegen Error | ~10.5s wall (retested 2026-07-26, Windows/MSVC STL 14.44). Partial-spec member `const`/`volatile` is now preserved, so `_Get_rest` overload selection works; current stop is late IR/codegen (`view_interface` layout / `tuple` ctor / `swap` / pack-expansion survivors). |
...
| `<span>` | `test_std_span.cpp` | ❌ Codegen Error | Full parse completes in ~7.5s / ~9.1s direct compiler wall (retested 2026-07-27, Windows/MSVC STL 14.44). Declaration-namespace lookup now evaluates the unqualified `dynamic_extent` NTTP default, so parsing and semantic analysis complete. Current stops are late deferred-member/codegen failures, ending at incomplete canonical layout metadata for `view_interface`. Regression: `tests/test_class_template_later_default_nttp_ret42.cpp`. |
| `<tuple>` | `test_std_tuple.cpp` | ❌ Semantic Error | ~2296ms (`TOTAL`) / ~5.5s runner wall (retested 2026-07-27, Windows/MSVC STL 14.44). Nested variadic function-template deduction and pack replay through dependent alias return types are fixed generically, including the explicit NTTP in a `get<0>`-shaped call. The full header still fails `auto val = std::get<0>(t)`: its next blocker is materializing the MSVC `tuple_element_t` return alias across the friend declaration/overload set. Link of a construct-only probe also still misses emitted `tuple`/`swap` symbols. |
| `<vector>` | `test_std_vector.cpp` | ❌ Semantic Error | ~9413ms (`TOTAL`) / ~12.9s runner wall (retested 2026-07-27, Windows/MSVC STL 14.44). Interleaved `__declspec(allocator)` now parses; current stop is `auto` deduction for allocator state member `_Mylast`. |
...
| `<ranges>` | `test_std_ranges.cpp` | ❌ Compile Error | ~15s wall (retested 2026-07-27, Windows/MSVC STL 14.44). Parse/noexcept stops fixed; full compile still dies on recursive `invoke` / `operator()` SoftProbe chains, but shared sticky nesting/iteration guards now abort the TU instead of multi-minute soft-retry storms. |
