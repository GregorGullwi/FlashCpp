...
| `<atomic>` | `test_std_atomic.cpp` | ❌ Codegen Error | ~1660ms (targeted retest 2026-04-07). Lazy member-resolution cache invalidation now gets `<atomic>` past the earlier `remove_volatile` / `__is_integral_helper` Itanium-mangling abort. The current blocker is deeper atomic-wait codegen fallout: unresolved `_M_do_wait` calls with missing default-argument recovery, zero-size dependent payloads in `std::__atomic_impl::__compare_exchange`, and a later missing `memory_order_seq_cst` symbol during IR generation. |
...
| `<latch>` | `test_std_latch.cpp` | ❌ Compile Error | ~880ms (targeted retest 2026-04-07). The same lazy member-resolution cache fix also gets `<latch>` past the earlier `remove_volatile` / Itanium-mangling failure. The current blocker remains the later semantic checking for scoped-enum bitwise expressions (`memory_order | memory_order`) in the latch/atomic-wait path. |
...
4. **Late atomic implementation follow-ons after the lazy-member cache fix**: Pointer-style `__atomic_add_fetch` / `__atomic_fetch_sub`, namespace-qualified explicit function-template calls such as `std::__atomic_impl::__compare_exchange<_AtomicRef>(...)`, `__builtin_memcpy`, and the later `remove_volatile` / `__is_integral_helper` mangling path now all get past parsing. `<atomic>` now fails later in codegen on `_M_do_wait`, dependent `__compare_exchange` payload sizing, and `memory_order_seq_cst` symbol recovery; `<latch>` still fails later in semantic analysis on scoped-enum bitwise expressions. Affects: `<atomic>`, `<latch>`.
