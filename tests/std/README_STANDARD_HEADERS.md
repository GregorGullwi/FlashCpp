...
| `<atomic>` | `test_std_atomic.cpp` | ❌ Codegen Error | ~2250ms (targeted retest 2026-04-06). Alias replay now preserves pointer/CV/array modifiers when the terminal alias target is a template parameter, so `<atomic>` gets well past the old `bits/atomic_wait.h` `sizeof(__waiter_type)` stop and the earlier mangling abort. The current blocker is deeper codegen fallout in the atomic/mutex stack (`std::__mutex_base` constructor recovery, `_M_do_wait`, and zero-size dependent payloads in `__compare_exchange` / `__clear_padding`). |
...
| `<latch>` | `test_std_latch.cpp` | ❌ Compile Error | ~850ms (targeted retest 2026-04-06). The old `bits/atomic_wait.h` `sizeof(__waiter_type)` blocker is gone here too, and the current failure is now later semantic checking for scoped-enum bitwise expressions (`memory_order | memory_order`) in the latch/atomic wait path. |
