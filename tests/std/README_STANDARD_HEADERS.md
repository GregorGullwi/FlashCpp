...
| `<latch>` | `test_std_latch.cpp` | 💥 Crash | ~2.35s wall (retested 2026-05-25, Linux/libstdc++-14). Still terminates with SIGSEGV after `std::__atomic_ref` template-arity warnings; trace shows crash while current template is `std::integral_constant`. |
