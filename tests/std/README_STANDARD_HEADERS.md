...
Merged work on `main` plus in-flight template-arg-list interning:

- **TypeInfo cold arenas** for dependent qualified names, template-arg lists, and instantiation contexts (#1814).
- **Slim `TemplateArgInfo`** with cold-payload arena for `function_signature` / `dependent_expr` (#1822).
- **Template arg-list interning** in `storeTypeInfoTemplateArgs()` (~60% dedup on heavy template headers).

Validation snapshot (`x64/Sharded/FlashCppMSVC.exe`, Windows/MSVC STL 14.44; 5-sample medians; compile-only `TOTAL` from `-time`, runner wall in seconds and includes compile/link/run):

| Header/Test | Status | Compile `TOTAL` | Runner wall | First-order stop / note |
|-------------|--------|-----------------|-------------|-------------------------|
| `<type_traits>` (`test_std_type_traits.cpp`) | ✅ Pass | ~2000ms | ~7.2s | Still compiles, links, and returns 0. |
| `<concepts>` (`test_std_concepts.cpp`) | ❌ Compile error | ~1230ms | ~4.3s | `callable object has no matching operator()`. |
| `<limits>` (`test_std_limits.cpp`) | ❌ Compile error | (fails in IR) | ~8.8s | `infinity` / `quiet_NaN` / `signaling_NaN`: sema missed return conversion (`double` → `long double`). |
| `<iterator>` (`test_std_iterator.cpp`) | ❌ Compile error | ~6750ms | ~9.9s | `callable object has no matching operator()` plus `std::swap` overload failures. |
...
- **Deferred implicit constructor/assignment triviality analysis now treats pointer and reference data members as scalar indirections instead of recursing into their pointee `StructTypeInfo`.** This avoids infinite recursion for self-referential node-like types (`T* next`) encountered while generating deferred implicit members in `<latch>` / atomic implementation paths.
- Checked for existing reusable helpers after the fix; the closest match is the nothrow-destructor member filtering in `TypeTraitEvaluator.cpp`, but the deferred implicit-member helper is file-local and also walks bases, so keeping the small guard local avoids adding another shared abstraction.

Regression coverage:

- `tests/test_self_pointer_implicit_copy_ret0.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_self_pointer_implicit_copy_ret0.cpp` | ✅ Pass | focused runner | New regression verifies implicit copy generation for a self-referential pointer member no longer crashes during deferred implicit-member triviality checks. |
| `<latch>` (`test_std_latch.cpp`) | ❌ Codegen Error | ~1.86s wall | Progresses past the prior SIGSEGV; current stop is `if constexpr condition is not a constant expression: _EntersWait::value` plus a normalized member-call receiver query gap. |
| `<type_traits>` (`test_std_type_traits.cpp`) | ✅ Pass | ~810ms (`TOTAL`) / ~0.88s wall | Control retest still compiles. |
| `<ratio>` (`test_std_ratio.cpp`) | 💥 Crash | ~2.69s wall | No material change; still SIGSEGVs during `__static_sign` after `__are_both_ratios` failures. |
| `<optional>` (`test_std_optional.cpp`) | ❌ Codegen Error | ~2.46s wall | No material change; still reaches `_Optional_payload_base` unresolved runtime-size / category-25 codegen gaps. |
| `<string_view>` (`test_std_string_view.cpp`) | ❌ Codegen Error | ~4.58s wall | No material change; still reaches unresolved direct-call return size. |
| `<string>` (`test_std_string.cpp`) | ❌ Compile Error | ~6.19s (`TOTAL`) / ~6.68s wall | No material change; still depth-guarded recursive `basic_string` instantiation. |
| `<memory>` (`test_std_memory.cpp`) | ❌ Compile Error | ~6.64s (`TOTAL`) / ~7.15s wall | No material change; still depth-guarded recursive `basic_string` instantiation. |
| `<functional>` (`test_std_functional.cpp`) | ❌ Compile Error | ~4.76s (`TOTAL`) / ~5.13s wall | No material change; still depth-guarded `rebind`. |

### 2026-05-26 Linux/libstdc++ current-instantiation template-owner follow-up

Fix landed:

- **Current-instantiation template-owner recovery now also works while reparsing an instantiated class body, not only inside an active member-function context.** `tryResolveCurrentInstantiationTemplateOwner(...)` now falls back to `struct_parsing_context_stack_` and `instantiate_and_register_base_template(...)` reuses that helper before attempting another class-template instantiation.
- **The base-template materializer now reuses the existing current-instantiation helper instead of open-coding another self-reference path.** This keeps the fix small and avoids another duplicate current-specialization matcher.

Regression coverage:

- `tests/test_current_instantiation_member_alias_base_ret0.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_current_instantiation_member_alias_base_ret0.cpp` | ✅ Pass | focused runner | New regression verifies a nested base clause reached through a member alias template (`rebound<T>`) can name the current specialization without re-instantiating it into an error. |
| `<type_traits>` (`test_std_type_traits.cpp`) | ✅ Pass | ~706ms (`TOTAL`) / ~0.76s wall | Control retest still compiles. |
| `<string>` (`test_std_string.cpp`) | ❌ Compile Error | ~5.45s (`TOTAL`) / ~5.82s wall | No material change yet; first hard stop is still depth-guarded recursive `basic_string` instantiation. |
| `<memory>` (`test_std_memory.cpp`) | ❌ Compile Error | ~5.84s (`TOTAL`) / ~6.22s wall | No material change yet; first hard stop is still depth-guarded recursive `basic_string` instantiation. |
| `<functional>` (`test_std_functional.cpp`) | ❌ Compile Error | ~4.19s (`TOTAL`) / ~4.47s wall | No material change yet; first hard stop is still depth-guarded `rebind`. |
| `<string_view>` (`test_std_string_view.cpp`) | ❌ Codegen Error | ~4.02s wall | No material change yet; still reaches unresolved direct-call return type plus `_CharT` constructor-call struct-info failures. |
| `<optional>` (`test_std_optional.cpp`) | ❌ Codegen Error | ~2.14s wall | No material change yet; still reaches `_Optional_payload_base` unresolved runtime-size / category-25 codegen gaps. |
| `<ratio>` (`test_std_ratio.cpp`) | 💥 Crash | ~2.29s wall | No material change yet; still SIGSEGVs during `__static_sign`. |
| `<latch>` (`test_std_latch.cpp`) | 💥 Crash | ~1.96s wall | No material change yet; still SIGSEGVs in the later atomic implementation path. |
...
- **Copy-initialization converting-constructor fallback in declaration IR now continues constructor candidate scanning when sema has no selected constructor annotation, instead of terminating immediately on the normalized-body invariant.**
- **Copy-init annotation retry bookkeeping now deduplicates deferred entries and also retries struct-backed targets after lazy-member/auto-return resolution when no cast slot was emitted yet.**

Regression coverage:

- `tests/std/test_std_any_template_copy_init_ctor_ret0.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_std_any_template_copy_init_ctor_ret0.cpp` | ✅ Pass | ~1.21s | New regression verifies copy-init `std::any a = source_value();` compiles and emits object code. |
| `<any>` (`test_std_any.cpp`) | ✅ Pass | ~1.13s | No longer fails with `Sema did not annotate converting constructor for normalized body`. |
| `<utility>` (`test_std_utility.cpp`) | ✅ Pass | ~1.78s | Control retest still compiles. |
...
- **Prefixed literal handling is now consistent across lexer, parser expression typing, and semantic-analysis canonical type inference.** `L""`, `u8""`, `u""`, and `U""` string literals now tokenize and map to `wchar_t`, `char8_t`, `char16_t`, and `char32_t` element types instead of partially falling through as plain `char`.

Regression coverage:

- `tests/test_wide_string_literal_overload_ret0.cpp`
- `tests/test_prefixed_literal_lexer_ret0.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_wide_string_literal_overload_ret0.cpp` | ✅ Pass | focused runner | New regression verifies `L"..."` selects `const wchar_t*` overloads over `const char*`. |
| `<string_view>` (`test_std_string_view.cpp`) | ❌ Compile Error | 4.44s | Progresses past previous wide-literal mismatch and now stops later in IR conversion: `Type with no runtime size reached codegen in direct call return size (type=25)` in `find`. |
| `<string_view>` wide literal repro (`test_std_wstring_view_find_ret0.cpp`) | ❌ Compile Error | 4.43s | Also advances to the same late IR `type=25` stop in `find`; no early `const char[6]` vs wide-literal initialization mismatch remains. |
| `<compare>` (`test_std_compare_ret42.cpp`) | ✅ Pass | 0.06s | Still compiles. |
| `<utility>` (`test_std_utility.cpp`) | ✅ Pass | 1.58s | Still compiles. |
| `<array>` (`test_std_array.cpp`) | ✅ Pass | 2.64s | Still compiles. |
...
This pass fixed two high-layer issues in the `<tuple>` path:

- Constexpr qualified-id evaluation can now materialize a template-instantiation
  owner that has stored dependent arguments but no `StructTypeInfo` yet.  This
  lets default NTTPs such as `_Head_base<_Idx, _Head, bool =
  __empty_not_final<_Head>::value>` rebind alias-local placeholders to the
  current default-argument context.
- The post-parse semantic boundary now treats constructors with still-dependent
  pack-expanded initializer arguments as parser/template-owned, matching the
  existing function-template handling.  This avoids rejecting uninstantiated
  member-template constructors such as libstdc++ tuple forwarding constructors.

Regression coverage:

- `tests/test_member_template_ctor_pack_initializer_boundary_ret0.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_member_template_ctor_pack_initializer_boundary_ret0.cpp` | ✅ Pass | n/a | New regression verifies a member-template constructor with a pack-expanded base initializer can remain parser-owned until template instantiation. |
| `<tuple>` (`test_std_tuple.cpp`) | 💥 Codegen Crash | 2.50s | Progressed past `_Head_base` default-NTTP evaluation and the uneliminated constructor `PackExpansionExprNode` boundary stop; current stop is later IR/codegen failure: `Sema-normalized constructor call is missing a resolved constructor for 'std::partial_ordering'`, followed by unresolved semantic type category 25. |
...
- `tests/std/test_libstdcxx_bits_move_block_scope_using_swap.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_libstdcxx_bits_move_block_scope_using_swap.cpp` | ✅ Compiled | 0.54s | New regression verifies `using std::swap; swap(int&, int&)` inside a class with a deleted member `swap` keeps the block-scope using-declaration result. |
...
This pass fixed a high-layer template substitution issue in partial
specialization instantiation: deduced specialization arguments are now used
when resolving deferred template bases, including dependent non-type arguments
and variadic tails.  This lets the libstdc++ tuple path evaluate
`_Head_base<_Idx, _Head, bool = __empty_not_final<_Head>::value>` instead of
leaving `__is_empty_non_tuple<_Tp>::value` bound to the alias template's
unsubstituted `_Tp`.

Regression coverage:

- `tests/std/test_tuple_partial_specialization_default_nttp_ret0.cpp`

Validation snapshot (`x64/Sharded/FlashCpp`, Linux/libstdc++-14):

| Header/Test | Status | Time | First-order stop / note |
|-------------|--------|------|-------------------------|
| `test_tuple_partial_specialization_default_nttp_ret0.cpp` | ✅ Pass | 0.02s | New regression verifies a partial-specialization deferred base can feed an alias-chain bool default NTTP through the deduced head argument. |
| `<tuple>` (`test_std_tuple.cpp`) | ❌ Compile Error | 1.26s | Progressed past `Could not evaluate non-type template default for parameter 2 of '_Head_base'`; current stop is uneliminated `PackExpansionExprNode` in `_Tuple_impl` constructors at the post-parse semantic boundary. |
| `<memory>` (`test_std_memory.cpp`) | ❌ Compile Error | 2.37s | Progressed past the tuple/default-NTTP blocker; current stop is `Max template instantiation depth (24) exceeded for 'list'`. |
| `<string>` (`test_std_string.cpp`) | ❌ Compile Error | 2.93s | Progressed past the tuple/default-NTTP blocker; current stop is `Max template instantiation depth (24) exceeded for 'basic_string'`. |
| `<list>` (`test_std_list.cpp`) | ❌ Compile Error | 3.78s | Progressed past the tuple/default-NTTP blocker; current stop is `Max template instantiation depth (24) exceeded for 'basic_string'`. |
