...
| `<ratio>` | `test_std_ratio.cpp` | ✅ Compiled | ~5282ms (`TOTAL`) / ~8.0s runner wall (retested 2026-07-26, Windows/MSVC STL 14.44). Definition-context lookup now preserves namespace-scope constexpr calls during template replay, and deferred base aliases resolve in their declaration namespace. Regressions: `tests/test_dependent_base_static_member_multi_expr_ret0.cpp` and `tests/std/test_std_ratio_equal_only.cpp`. |
| `<optional>` | `test_std_optional.cpp` | ❌ Parse Error | ~9466ms (`TOTAL`) / ~9.58s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). First hard stop is structural class-type non-type template parameters in MSVC `optional:269`. |
| `<any>` | `test_std_any.cpp` | ❌ Codegen Error | ~9.53s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Partial-spec member `const`/`volatile` is now preserved, so `_Get_rest` overload selection works; current stop is late IR/codegen (`view_interface` layout / `tuple` ctor / `swap` / pack-expansion survivors). |
...
| `<string_view>` | `test_std_string_view.cpp` | ❌ Lazy Replay Error | ~11.43s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Current stop is lazy member-body replay for `_String_view_iterator::operator+`. |
...
| `<span>` | `test_std_span.cpp` | ❌ Codegen Error | ~8.73s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Declaration-namespace lookup now evaluates the unqualified `dynamic_extent` NTTP default, so parsing and semantic analysis complete. Current stops are late deferred-member/codegen failures, ending at incomplete canonical layout metadata for `view_interface`. Regression: `tests/test_class_template_later_default_nttp_ret42.cpp`. |
| `<tuple>` | `test_std_tuple.cpp` | ❌ Semantic Error | ~2193ms (`TOTAL`) / ~2.25s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Nested variadic function-template deduction and pack replay through dependent alias return types are fixed generically, including the explicit NTTP in a `get<0>`-shaped call. The full header still fails `auto val = std::get<0>(t)`: its next blocker is materializing the MSVC `tuple_element_t` return alias across the friend declaration/overload set. Link of a construct-only probe also still misses emitted `tuple`/`swap` symbols. |
| `<vector>` | `test_std_vector.cpp` | ❌ Semantic Error | ~7901ms (`TOTAL`) / ~7.99s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Dependent local `auto&` now defers and materializes through nested member chains, moving the header past `_Vaporization_guard::_Mylast`. The next stop is eager `view_interface<subrange<...>>::empty` substitution retaining the pattern return type `_Derived` from its `_Cast()` call. Regression: `tests/test_nested_template_member_access_auto_ref_ret42.cpp`. |
...
| `<ranges>` | `test_std_ranges.cpp` | ❌ Compile Error | ~9989ms (`TOTAL`) / ~10.04s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). Parse/noexcept stops fixed; full compile still dies on recursive `invoke` / `operator()` SoftProbe chains, but shared sticky nesting/iteration guards now abort the TU instead of multi-minute soft-retry storms. |
...
| `<iterator>` | `test_std_iterator.cpp` | ❌ Codegen Error | ~8.77s direct compiler wall (retested 2026-07-28, Windows/MSVC STL 14.44). First hard stops remain sema-normalized return lowering for `begin`/`end`, `view_interface` equality, and incomplete aggregate layout. |
...
| `<any>` | Codegen: `view_interface` / nested `tuple` / pack-expansion IR | Exact result types, complete aggregate layout, and pack expansion before codegen |
| `<span>` | Codegen: deferred `first`/`last`/`subspan`, `view_interface` equality and incomplete aggregate layout | Deferred member replay, exact comparison types, and complete canonical aggregate layout |
| `<tuple>` | Sema: `auto` from `std::get<0>` after generic nested-pack deduction; residual link holes for construct-only | Materialize the `tuple_element_t` return alias across friend declarations/overloads; deferred partial-spec member emission |
| `<vector>` | Sema: eager `view_interface<subrange<...>>::empty` substitution leaves `_Cast()`'s pattern return `_Derived` unresolved | Rebind dependent member-call declarations/return types to the concrete current owner during eager class-template member-body substitution |
...
| `<ranges>` | Template-depth / `invoke` recursion (now sticky-aborts; underlying CPO instantiation still broken) | Variadic `invoke` / CPO instantiation without SoftProbe retry storms |
...
The 2026-07-28 dependent-local regression is `tests/test_nested_template_member_access_auto_ref_ret42.cpp`. It covers direct and staged `auto&` deduction through a class-template member, a nested class-template member, and a final pointer member for both `int` and `long long` specializations. Parser-time deduction now recognizes implicit-object expressions that still depend on an unsubstituted class parameter and preserves `auto` when the provisional type still contains a dependent placeholder. Instantiation-time substitution supplies the concrete owner to the shared expression-type resolver and retains newly deduced local types for later declarations in the same substituted body. This moves `<vector>` past `_Target->_Mypair._Myval2._Mylast` without recognizing `vector` or any vendor helper name.

For the next `<vector>` implementation, the first failure is in eager substitution of `view_interface$...::empty`: the enclosing owner is already concrete (`view_interface<subrange<...>>`), but the retained call declaration for `_Cast()` still exposes the pattern return type `_Derived`. The failure is raised by the post-substitution hard-use audit in `Parser_Templates_Inst_ClassTemplate.cpp`. The existing owner-aware `ExpressionSubstitutor` path is the likely place to rebind that member call; the fix should be reduced as a generic CRTP member-call/return-type test before changing the standard-header path.
