...
7. **Template full specialization forward declarations** (January 8, 2026) - Added support for forward declarations of full template specializations for top-level templates. Patterns like `template<> struct make_unsigned<bool>;` now parse correctly without requiring a body definition. The parser detects `;` after template arguments and registers the specialization as a forward declaration. This **advances `<type_traits>` from line 1917 to line 2180 (263 more lines!)**. Test case `test_template_full_spec_forward_decl_ret0.cpp` successfully compiles! **Next blocker**: `__alignof__` with typename expression in template parameter defaults at line 2180.
