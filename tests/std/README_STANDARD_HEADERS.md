...
| `<deque>` | `test_std_deque.cpp` | ❌ Compile Error | ~930ms (targeted retest 2026-04-10). A smaller std-style `pair::swap` unused-body regression now compiles again (`tests/std/test_std_pair_swap_deleted_member.cpp`), so the remaining `<deque>` stop is a deeper real-libstdc++ path: `bits/stl_pair.h:308` still resolves `swap(first, __p.first)` to a deleted candidate during template instantiation. |
