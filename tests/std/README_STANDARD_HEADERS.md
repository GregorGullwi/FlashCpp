...
| `<utility>` | `test_std_utility.cpp` | ✅ Runs | ~1.8–2.0s (`TOTAL`/wall, retested 2026-07-29, Windows/MSVC STL 14.44). `pair<int, float>` now links and returns 0. Address-valued IR copies use the 64-bit GPR path even when the pointee is floating-point, fixing out-of-line `float&&` returns. Explicit-template-argument materialization also runs the generic pure-expression heuristic, ignores compile-time-only empty statements, and inlines both `forward` calls. Regressions: `tests/test_float_rvalue_ref_return_ret0.cpp`, `tests/test_template_pure_expr_rvalue_ref_ret0.cpp`, and the exact-exit header probe `tests/test_std_utility_pair_float_ret0.cpp`. |
...
| `<tuple>` | `test_std_tuple.cpp` | ❌ Codegen Error | ~2.6s direct compiler wall (retested 2026-07-29, Windows/MSVC STL 14.44). Contextual explicit-template-argument parsing now backtracks across overload candidates with different parameter kinds, so `std::get<0>(t)` parses and completes semantic analysis. The current first hard stop remains late type materialization (`Struct type info not found for type_index=2455`, previously surfaced as unresolved semantic category 25). Existing deferred `tuple` constructor and `swap` emission gaps also remain. |
