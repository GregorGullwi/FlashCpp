...
| `<tuple>` | `test_std_tuple.cpp` | ❌ Semantic Error | ~2061ms (`TOTAL`) / ~5.0s runner wall (retested 2026-07-27, Windows/MSVC STL 14.44). Qualified-name OOL ctor recognition and partial-spec member cv-qualifiers are fixed; `#include` + construct emits an object, but `auto val = std::get<0>(t)` fails auto deduction. Link of a construct-only probe still misses emitted `tuple`/`swap` symbols. |
...
| `<tuple>` | Sema: `auto` from `std::get<0>`; residual link holes for construct-only | Return-type deduction for `get` / tuple element aliases; deferred partial-spec member emission |
