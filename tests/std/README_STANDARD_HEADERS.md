...
| `<utility>` | `test_std_utility.cpp` | ✅ Compiled | ~1.8–2.0s (`TOTAL`/wall, retested 2026-07-29, Windows/MSVC STL 14.44). Template-parameter spelling now wins over stale dependent `TypeInfo`, so `pair<int, float>::first` has type `int`. Object generation succeeds; the linked probe still crashes at runtime, leaving a separate codegen issue. |
