...
int main() {
	...
	d = 2.5;
	if (dr1 != 2.5) return 9;
	if (dr2 != 2.5) return 10;

	// ---- struct: ref-to-ref ----
	Point p{ 3, 4 };
	Point& pr1 = p;
	const Point& pr2 = pr1;
	if (pr2.x != 3 || pr2.y != 4) return 11;
	p.x = 10;
	if (pr1.x != 10) return 12;
	if (pr2.x != 10) return 13;
	if (sum_through_ref(pr2) != 14) return 14;  // 10+4
	...
	for (int idx = 0; idx < 3; ++idx) {
		...
		sum += elem_ref2;
		arr[idx] += 10;
		if (elem_ref2 != arr[idx]) return 17;  // ref2 must see the update
	}
	if (sum != 6) return 18;  // 1+2+3 before updates
	...
	while (counter > 0) {
		...
		w += wr2;
		--counter;
		if (wr2 != counter) return 19;  // ref2 must see the decrement
	}
	if (w != 6) return 20;  // 3+2+1
