...
// Case 4: Return with explicit std::move (when implemented)
// This would prevent copy elision and force move
// Commented out since std::move not yet implemented
/*
Value explicitMove() {
...
int main() {
    ...
    // Validate results
    if (r1.x == 10 && r2.x == 50 && r3.x == 35) {
        printf("\nAll values correct!\n");
        printf("TEST PASSED\n");
        return 0;
    } else {
        printf("\nERROR: Incorrect values (r1.x=%d expected 10, r2.x=%d expected 50, r3.x=%d expected 35)\n", 
               r1.x, r2.x, r3.x);
        return 1;
    }
}
