...
enum class GlobalSeason { Spring = 10,
						  Summer = 20,
						  Fall = 30,
						  Winter = 40 };

int test_local_enum_class_underlying() {
	enum class Priority : unsigned char { Low = 1,
										  Medium = 2,
										  High = 3 };
...
int main() {
	return test_local_enum_class_underlying() + test_local_enum_class_cast() + test_local_enum_class_compare() + test_global_and_local_enum_coexist();
}
