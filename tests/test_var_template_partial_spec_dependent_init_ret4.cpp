...
// Array-based partial specialization - validates array deduction stripping:
// For T[], T should be deduced as int (not int[]).
template<typename T>
constexpr int arr_elem_size_v = -1;

template<typename T>
constexpr int arr_elem_size_v<T[]> = sizeof(T);
...
int main() {
	...
	// Array pattern: primary should be -1
	static_assert(arr_elem_size_v<int> == -1, "primary should be -1");
	// Array pattern: T deduced as int (not int[]), sizeof(int) == 4
	static_assert(arr_elem_size_v<int[]> == 4, "T should be int, sizeof(int)==4");
