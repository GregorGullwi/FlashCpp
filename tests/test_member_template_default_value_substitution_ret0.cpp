...
int main() {
	...
	if (ci.convert<long long>(42LL, 99) != 42) return 1;

	// Call relying on default: T{} should become int{} == 0
	// convert<long long>(0LL) → static_cast<int>(0) != 0 is false → returns fallback = int{} = 0
	if (ci.convert<long long>(0LL) != 0) return 2;

	// Non-zero input with default fallback
	if (ci.convert<short>(7, 0) != 7) return 3;

	Container<char> cc;
	cc.stored = 'A';

	// char{} == '\0' == 0
	if (ci.convert<char>('\0') != 0) return 4;
	if (ci.convert<char>('Z') != static_cast<int>('Z')) return 5;
