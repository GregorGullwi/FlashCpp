...
int main() {
 ...
 // 0xFFFFFFFF fits in unsigned int (32 bits unsigned)
	if (sizeof(0xFFFFFFFF) != sizeof(unsigned int))
		return 3;
 ...
 // Small decimal literal fits in int
	if (sizeof(42) != sizeof(int))
		return 5;

 // Small octal literal fits in int
	if (sizeof(077) != sizeof(int))
		return 6;

 // Small binary literal fits in int
	if (sizeof(0b1010) != sizeof(int))
		return 7;

 // 0b followed by 32 one-bits = 0xFFFFFFFF → unsigned int (hex/octal/binary rule)
	if (sizeof(0b11111111111111111111111111111111) != sizeof(unsigned int))
		return 8;
