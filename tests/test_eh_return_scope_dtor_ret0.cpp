...
int test_return_from_loop() {
	...
	for (int i = 0; i < 3; i++) {
		Guard loop_g;
		if (i == 1)
			return (g_dtor_count - prev);  // Guard for i=0 destroyed at loop-body end, Guard for i=1 destroyed before return
	}
	return -1;  // unreachable
}
...
int main() {
 ...
 // Test 1: function-scope Guard
	...
	if (ret != 0)
		return 1;			  // return value must be 0 (pre-dtor)
	if (g_dtor_count != 1)
		return 2;  // Guard must have been destroyed
 ...
 // Test 2: return from inside try block
	...
	if (ret != 0)
		return 3;			  // return value must be 0 (pre-dtor)
	if (g_dtor_count != 2)
		return 4;  // both inner and outer must be destroyed
 ...
 // Test 3: return from inside a loop
	...
	if (ret != 1)
		return 5;			  // 1 dtor for i=0 (before return at i=1)
	if (g_dtor_count != 2)
		return 6;  // loop_g for i=0 (normal exit) + i=1 (before return)
