...
int main() {
    // 1
    if (Plain::A != 10) return 1;
    if (Plain::B != 20) return 2;

    // 2
    if ((int)WithUsing::Color::Red != 1) return 3;
    ...
    if ((int)r != 1) return 5;
    if (TBox<int>::Hi != 2) return 6;

    // 5
    Wrap<int> w;
    w.data = 99;
    if (w.data != 99) return 7;

    // 6
    ns2::NS2Container::Switch s = ns2::NS2Container::On;
    if ((int)s != 1) return 8;
    if (ns2::NS2Container::Off != 0) return 9;
