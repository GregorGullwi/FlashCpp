...
// These are compiler built-ins supported by FlashCpp
...
int test_is_integral() {
	...
	if (__is_integral(int))
		result += 1;
	if (__is_integral(char))
		result += 2;
	if (__is_integral(bool))
		result += 4;
	if (__is_integral(long))
		result += 8;
	if (__is_integral(unsigned int))
		result += 16;

	// These should be false
	if (!__is_integral(float))
		result += 32;
	if (!__is_integral(double))
		result += 64;
...
int test_is_floating_point() {
	...
	if (__is_floating_point(float))
		result += 1;
	if (__is_floating_point(double))
		result += 2;

	// These should be false
	if (!__is_floating_point(int))
		result += 4;
	if (!__is_floating_point(char))
		result += 8;
...
int test_is_pointer() {
	...
	if (__is_pointer(int*))
		result += 1;
	if (__is_pointer(void*))
		result += 2;
	if (__is_pointer(MyStruct*))
		result += 4;
...
int test_is_reference() {
	...
	if (__is_lvalue_reference(int&))
		result += 1;

	// Test rvalue references
	if (__is_rvalue_reference(int&&))
		result += 2;
