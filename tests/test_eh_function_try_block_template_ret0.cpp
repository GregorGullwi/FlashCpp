...
struct Box {
	...
	template<typename T>
	int checked_add(T delta)
	try {
		if (delta < (T)0) throw -1;
...
int main() {
	...
	if (safe_divide(10, 0) != -1) return 2;
	if (g_caught != 42) return 3;

	// Template member function: normal path
	Box b(10);
	if (b.checked_add(5) != 15) return 4;

	// Template member function: exception path
	if (b.checked_add(-1) != -1) return 5;
