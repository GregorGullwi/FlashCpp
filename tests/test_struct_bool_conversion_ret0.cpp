...
int test_struct_bool_condition() {
	...
	int result = 0;
	if (on)
		result += 1;	 // operator bool() returns true
	if (!off)
		result += 1; // operator bool() returns false, negated → true
	return result - 2;	   // expect 0
}

int main() {
	return test_struct_bool_init() + test_struct_bool_function_arg() + test_struct_bool_condition();
}
