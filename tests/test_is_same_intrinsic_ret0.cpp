...
int main() {
	...
	if (__is_same(int, int))
		result += 1;
	if (__is_same(double, double))
		result += 2;
	if (__is_same(char, char))
		result += 4;
	if (__is_same(MyStruct, MyStruct))
		result += 8;

	// Test different types - should be false
	if (!__is_same(int, double))
		result += 16;
	if (!__is_same(int, long))
		result += 32;
	if (!__is_same(char, int))
		result += 64;

	// Test with const qualifiers - const int != int
	if (!__is_same(const int, int))
		result += 128;
	if (__is_same(const int, const int))
		result += 256;
