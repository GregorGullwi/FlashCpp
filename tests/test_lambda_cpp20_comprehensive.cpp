...
int main() {
    ...
    result += test_basic_lambda();                    // 42
    result += test_lambda_with_params();              // 42
    result += test_capture_by_value();                // 42
    result += test_capture_by_reference();            // 42
    result += test_capture_all_by_value();            // 42
    result += test_capture_all_by_reference();        // 42
    result += test_mixed_captures();                  // 42
    result += test_init_capture();                    // 42
    result += test_init_capture_modified();           // 42
    result += test_mutable_lambda();                  // 42
    result += test_explicit_return_type();            // 42
    result += test_generic_lambda();                  // 42
    result += test_nested_lambdas();                  // 42
    result += test_lambda_returning_lambda();         // 42
    result += test_iife();                            // 42
    result += test_multiple_statements();             // 42
    result += test_const_capture();                   // 42
    result += test_lambda_in_conditional();           // 42
    result += test_multiple_different_captures();     // 42
    result += test_ref_capture_modify();              // 42
    
    TestStruct ts;
    result += ts.test_capture_all_with_this();        // 42
    
    TestThis tt;
    result += tt.test_this_capture();                 // 42
    
    result += test_multiple_params();                 // 42
    result += test_init_capture_by_ref();             // 42
    
    // Total: 24 tests * 42 = 1008
    return result;
}
