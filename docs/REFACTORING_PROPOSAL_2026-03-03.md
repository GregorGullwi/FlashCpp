...
| 2B | ⏳ Deferred | Remaining inline registration sites: the SFINAE loops in `Parser_Templates_Inst_Deduction.cpp` (~471) and `Parser_Templates_Inst_MemberFunc.cpp` (~240) also update `sfinae_type_map_`, so they cannot use `registerTypeParamsInScope` directly without a new `sfinae_type_map*` parameter. The deduced-args map path in `Parser_Templates_Inst_ClassTemplate.cpp` (~1893) iterates over an `unordered_map` rather than parallel vectors; extracting it requires structural refactoring. |
