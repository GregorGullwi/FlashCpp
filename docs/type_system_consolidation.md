...
**Status**: Milestones 1–7+ complete. **All IR Op structs, FunctionParam, StaticLocalInfo, StructMember, StructStaticMember, LazyStaticMemberInfo, ArrayAccessOp, CatchBeginOp, ThrowOp, StackAllocOp, HeapAllocOp, ConversionOp, TypeConversionOp, GlobalVariableInfo, AddressOfMemberOp, ComputeAddressOp, VirtualCallOp, DereferenceOp, DereferenceStoreOp, and `ExprResult`** have been migrated off raw `Type` storage. **All classification call-sites** (`== Type::` / `!= Type::` comparisons) have been fully migrated to `TypeCategory` — only 5 lines remain, and they are: 2 bridge function **bodies** in `AstNodeTypes.cpp` (intentional definitions), 1 comment in `TemplateRegistry_Pattern.h`, and 2 `Token::Type` comparisons in `Token.h` (a completely separate nested enum for token kinds, unrelated to the type system). The thin raw-`Type` wrapper cleanup is further along: `get_integer_rank(Type)`, `get_floating_point_rank(Type)`, `promote_integer_type(Type)`, `promote_floating_point_type(Type)`, `get_common_type(Type, Type)`, `requires_conversion(Type, Type)`, and `buildConversionPlan(Type, Type)` are now deleted, leaving the larger bridge helpers (`get_type_size_bits(Type)`, `toIrType(Type)`, `categoryToType`, `typeToCategory`) as the remaining raw-`Type` surface. **Next step**: migrate the ~479 remaining `Type::` construction/usage sites (e.g., `setType(Type::Struct)`, `emitReturn(v, Type::Int, ...)`, `NumericLiteralNode(tok, val, Type::Int, ...)`) to accept `TypeCategory` directly, then delete bridge functions and the `Type` enum. Test baseline: **1756 pass / 105 expected-fail / 0 failures**.

**Remaining to delete the `Type` enum**: all classification comparisons (`== Type::` / `!= Type::`) are fully migrated. The remaining raw-`Type` usage is at **construction/emission API call sites** (~479 sites) that pass `Type::Int`, `Type::Struct`, etc. as arguments to functions like `setType()`, `emitReturn()`, `makeTypedValue()`, and AST node constructors. The final deletion steps are: (1) change those APIs to accept `TypeCategory` directly, (2) delete bridge functions (`categoryToType`, `typeToCategory`, `toIrType(Type)`), (3) delete the `Type` enum.
...
| 5 | Port Parser / ConstExpr / IR-generator local `Type type = …` variables to `TypeCategory`/`TypeIndex` | ✅ Done — all classification call-sites migrated; remaining 8 `== Type::` comparisons are bridge function **definitions** (`AstNodeTypes_TypeSystem.h`/`AstNodeTypes.cpp`) and `Token::Type` (unrelated nested enum) |
| 5 | Port `buildConversionPlan` (`OverloadResolution.h`) to `TypeCategory`/`TypeIndex` | ✅ Complete — `buildConversionPlan(const TypeSpecifierNode&, const TypeSpecifierNode&)`, the adjacent binary-operator helper layer, and the primitive arithmetic ranking/promotion helpers now use `TypeCategory`/`TypeIndex`; the raw-`Type` primitive overload has been deleted |
| 6 | Remove `Type` enum and all `Type::*` references from the codebase | ◐ Almost — all classification sites migrated; only bridge function bodies + `Token::Type` remain; next: delete raw-Type wrappers → bridge functions → enum |
...
4. **Remaining raw-`Type` compatibility wrappers**: the easy arithmetic wrappers (`get_integer_rank(Type)`, `get_floating_point_rank(Type)`, `promote_integer_type(Type)`, `promote_floating_point_type(Type)`, `get_common_type(Type, Type)`, `requires_conversion(Type, Type)`) and the primitive `buildConversionPlan(Type, Type)` bridge are now deleted, but the larger compatibility surface still exists in conversion/codegen helpers and keeps a few callers from going fully TypeCategory-native.
...
1. **Migrate `Type::` construction/usage sites** (~479 sites) — e.g., `setType(Type::Struct)`, `emitReturn(v, Type::Int, ...)`, `NumericLiteralNode(tok, val, Type::Int, ...)`, function overloads that take `Type` parameters. Change these APIs to accept `TypeCategory` directly. The largest clusters are in `IrGenerator_Visitors_Decl.cpp` (~30 sites), `IrGenerator_Stmt_Decl.cpp` (~15 sites), `IrGenerator_Expr_Conversions.cpp` (~15 sites), `IrGenerator_Call_Direct.cpp` (~10 sites), and parser/codegen files that still pass `Type::Int`/`Type::Struct` to constructors.
2. **Delete the remaining bridge functions** (`categoryToType`, `typeToCategory`, `toIrType(Type)`) — once all callers are migrated off the raw-`Type` API surface, delete these adapter functions.
3. **Delete any last raw-`Type` helper overloads** that still forward straight into TypeCategory implementations once the bridge-function callers are gone.
4. **Delete the `Type` enum definition** — once bridge functions and raw-Type helpers are gone, delete `enum class Type { … }` and its file, completing the migration.
5. **Add a static_assert** after each bridge-function deletion to prevent regression.
