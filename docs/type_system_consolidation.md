...
1. **Delete thin raw-`Type` compatibility wrappers** — `buildConversionPlan(Type, Type)`, `get_integer_rank(Type)`, `promote_integer_type(Type)`, `get_common_type(Type, Type)` are now the only non-bridge/non-definition callers of `== Type::`. Delete each wrapper and fix its callers to use the TypeCategory overload.
