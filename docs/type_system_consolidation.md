...
**Status**: Milestones 1–7 complete. **All IR Op structs, FunctionParam, StaticLocalInfo, StructMember, StructStaticMember, LazyStaticMemberInfo, ArrayAccessOp, CatchBeginOp, ThrowOp, StackAllocOp, HeapAllocOp, ConversionOp, TypeConversionOp, GlobalVariableInfo, AddressOfMemberOp, ComputeAddressOp, VirtualCallOp, DereferenceOp, DereferenceStoreOp, and `ExprResult`** have been migrated off raw `Type` storage. `ExprResult` now stores `TypeCategory category_` + `IrType ir_type`, with `typeEnum()` as a bridge accessor. `TypedValue::setType()` atomically stamps `TypeCategory`+`IrType`+`is_signed`. `effectiveIrType()` + `is_signed` are the codegen classification authorities. `current_function_return_type_index_` defaults to `{0, TypeCategory::Void}`. The `TypeSpecifierNode` overload of `buildConversionPlan()` now uses `TypeCategory`/`TypeIndex` for its alias-resolution, pointer/reference, and struct-identity checks, the adjacent binary-operator helper layer (`effectiveBinaryOperatorTypeFromSpec()` / `makeBinaryOperatorTypeSpecifier()`) now operates on `TypeCategory`, the primitive arithmetic helper core (`get_integer_rank`, `promote_integer_type`, `get_common_type`) is `TypeCategory`-native, `TypeTraitEvaluator.cpp` internal helpers (`isScalarType`, `isIntegral`, `isFloatingPoint`, `isSigned`, `isUnsigned`) now operate on `TypeCategory`, `SemanticAnalysis.cpp` arithmetic promotion helpers operate on `TypeCategory`, and `IrGenerator_Expr_Primitives.cpp` member struct-checks use `is_struct_type(type_index.category())`. `ObjFileWriter_*.cpp` is already clean (0 `== Type::` comparisons). Added `category()` method to `CanonicalTypeDesc` in `SemanticTypes.h` for consistent `TypeCategory`-based classification. `SemanticAnalysis.cpp` fully migrated (44 → 0 `== Type::` sites). `IRConverter_ConvertMain.cpp` fully migrated (44 → 0 `== Type::` sites). All `Parser_Decl_*.cpp` files already clean (0 sites). Added TypeCategory overloads for `is_integer_type`, `is_bool_type`, `is_unsigned_integer_type`, `is_standard_arithmetic_type` to `AstNodeTypes_TypeSystem.h`. `IrGenerator_Expr_Operators.cpp` migrated (ternary `common_type`, `tryGlobalSemaConv`, lhs/rhsType, va_arg, lvalue helpers). `IrGenerator_MemberAccess.cpp` migrated (`isScalarType`, `HasUniqueObjectRepresentations`, struct/nullptr/enum guards). `IrGenerator_Expr_Operators.cpp` + `IrGenerator_MemberAccess.cpp`: **55 `== Type::` sites eliminated** (169 → 114 → 89 total remaining). Test baseline: **1784 pass / 105 expected-fail / 0 failures**.

**Remaining to delete the `Type` enum**: no active compiler data-structure fields, but still a few hundred call sites / switches / thin bridge APIs remain in the compiler source. See "What's left" section below.
...
| 5 | Migrate `ExprResult::type` field to pure `IrType`+`TypeCategory` (remove `Type type` from `ExprResult`) | ✅ Done — `ExprResult` now stores `TypeCategory category_`; factories and IR-generator callers use `typeEnum()`/`category()` bridges |
...
| 5 | Port Parser / ConstExpr / IR-generator local `Type type = …` variables to `TypeCategory`/`TypeIndex` | ◐ Partial — `Parser_TypeSpecifiers.cpp`, `Parser_Templates_Inst_ClassTemplate.cpp` template-argument `sizeof` handling, `Parser_Expr_BinaryPrecedence.cpp` SFINAE/alignment classification paths, substantial `IrGenerator_Expr_Conversions.cpp`, `IrGenerator_NewDeleteCast.cpp` allocation/deallocation classification paths, the `ExprResult` fallout batch across call/member/lambda IR generators, `ConstExprEvaluator_Core.cpp` target-type conversion dispatch, the member-access type-trait evaluator in `IrGenerator_MemberAccess.cpp`, arithmetic/type-trait consumers in `ConstExprEvaluator_Members.cpp` / `SemanticAnalysis.cpp`, `TypeTraitEvaluator.cpp` namespace helpers, `SemanticAnalysis.cpp` arithmetic promotion helpers (`tryAnnotateBinaryOperandConversions`, `tryAnnotateCompoundAssignBackConversion`, `tryAnnotateShiftOperandPromotions`, `tryAnnotateUnaryOperandPromotion`), `IrGenerator_Expr_Primitives.cpp` member struct-check pattern, `IrGenerator_Expr_Operators.cpp` (ternary common_type, lhsType/rhsType, va_arg, lvalue helpers), and `IrGenerator_MemberAccess.cpp` (isScalarType, HasUniqueObjectRepresentations, struct/nullptr/enum guards) are migrated; remaining ~89 sites in `Parser_Templates_Inst_ClassTemplate.cpp`, `IrGenerator_Expr_Conversions.cpp`, `IrGenerator_Call_Direct.cpp`, `IrGenerator_Call_Indirect.cpp`, and smaller files |
| 5 | Port `buildConversionPlan` (`OverloadResolution.h`) to `TypeCategory`/`TypeIndex` | ◐ Partial — `buildConversionPlan(const TypeSpecifierNode&, const TypeSpecifierNode&)`, the adjacent binary-operator helper layer, and the primitive arithmetic ranking/promotion helpers now use `TypeCategory`/`TypeIndex`; the raw-`Type` primitive overload remains as a thin compatibility wrapper |
...
| *(none left in active compiler data structures)* | — | Remaining raw `Type` usage is now in call-site logic / helper APIs rather than persistent semantic or IR result structs |
...
1. **Local variable `Type type = …` / raw `Type` helper calls used only for classification** (~89 sites remain across ~20 files). The largest remaining hotspots are `Parser_Templates_Inst_ClassTemplate.cpp` (22 sites), `IrGenerator_Expr_Conversions.cpp` (12 sites), `IrGenerator_Call_Direct.cpp` (9 sites), `IrGenerator_Call_Indirect.cpp` (7 sites), `IrGenerator_NewDeleteCast.cpp` (6 sites), and `Parser_Expr_QualLookup.cpp` (5 sites).
   - Recent progress: `ExprResult` storage is now category-based; `Parser_TypeSpecifiers.cpp`, `Parser_Templates_Inst_ClassTemplate.cpp` template-argument `sizeof` logic, `Parser_Expr_BinaryPrecedence.cpp` SFINAE/alignment dispatch, `ConstExprEvaluator_Core.cpp` target-type conversion, `ConstExprEvaluator_Members.cpp` arithmetic/fundamental/type-trait classification, `IrGenerator_Expr_Conversions.cpp`, `IrGenerator_NewDeleteCast.cpp` new/delete classification, the member-access type-trait evaluator, a broad IR-generator fallout batch, `TypeTraitEvaluator.cpp` namespace helpers, `Parser_Statements.cpp` remaining struct/member classification paths, `IrGenerator_Helpers.cpp` runtime size/type guards, `IrGenerator_Stmt_Decl.cpp` float/enum/struct classification gates, `IrGenerator_Expr_Primitives.cpp` member struct-check pattern, **`SemanticAnalysis.cpp` (all 44 sites migrated via new `CanonicalTypeDesc::category()`)**, **`IRConverter_ConvertMain.cpp` (all 44 sites migrated via `typeToCategory()` bridge)**, **`IrGenerator_Expr_Operators.cpp`** (ternary/lhsType/rhsType/va_arg/lvalue helpers, ~30 sites migrated), and **`IrGenerator_MemberAccess.cpp`** (`isScalarType`, `HasUniqueObjectRepresentations`, struct/nullptr/enum guards, ~25 sites migrated) were all migrated. `CanonicalTypeDesc::category()` added to `SemanticTypes.h`. TypeCategory overloads for `is_integer_type`, `is_bool_type`, `is_unsigned_integer_type`, `is_standard_arithmetic_type` added to `AstNodeTypes_TypeSystem.h`.

2. **Parser `Type`-based dispatch** — All `Parser_Decl_*` files, `Parser_Statements.cpp`, `Parser_Expr_BinaryPrecedence.cpp`, `Parser_Expr_PrimaryExpr.cpp`, `Parser_Expr_PrimaryUnary.cpp`, and `Parser_TypeSpecifiers.cpp` are clean (0 `== Type::` comparisons).

3. **`ObjFileWriter_*.cpp` symbol/EH/RTTI** — already clean; all four ObjFileWriter files have 0 `== Type::` comparisons.

4. **Thin raw-`Type` compatibility wrappers** (`buildConversionPlan(Type, Type)`, `get_integer_rank(Type)`, `promote_integer_type(Type)`, `get_common_type(Type, Type)`): the core implementations are now category-based, but the legacy `Type` wrappers still exist as public compatibility currency and keep many callers from migrating fully.
...
5. **`get_type_size_bits(Type)`, `categoryToType()`, `typeToCategory()`** helper functions: these are the *bridge* functions that allow migration to be incremental; they must be deleted last, after all callers are ported.
   - Note: `IRConverter_ConvertMain.cpp` (44 sites) now uses `typeToCategory(X) == TypeCategory::Y` pattern throughout, eliminating raw `Type::` comparisons. The `typeToCategory()` calls remain as bridge until codegen layer variables carry `TypeCategory` directly.

### Recommended next steps (current reality, largest blockers first)

1. **Finish file-by-file local-variable cleanup** (~89 `== Type::` sites remain): prioritize `Parser_Templates_Inst_ClassTemplate.cpp` (22), `IrGenerator_Expr_Conversions.cpp` (12), `IrGenerator_Call_Direct.cpp` (9), `IrGenerator_Call_Indirect.cpp` (7), `IrGenerator_NewDeleteCast.cpp` (6 residual), `Parser_Expr_QualLookup.cpp` (5). `SemanticAnalysis.cpp` (0), `IRConverter_ConvertMain.cpp` (0), `IrGenerator_Expr_Operators.cpp` (0), `IrGenerator_MemberAccess.cpp` (0), and all `Parser_Decl_*.cpp` (0) are done.
2. **Collapse the remaining thin raw-`Type` compatibility wrappers** — once callers are gone, delete the `Type` overloads of `buildConversionPlan`, `get_integer_rank`, `promote_integer_type`, and `get_common_type`.
3. **Delete bridge functions** (`categoryToType`, `typeToCategory`, `toIrType(Type)`) and then the `Type` enum itself.
