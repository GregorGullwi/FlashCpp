...
**Status**: Milestones 1–7+ complete. **All IR Op structs, FunctionParam, StaticLocalInfo, StructMember, StructStaticMember, LazyStaticMemberInfo, ArrayAccessOp, CatchBeginOp, ThrowOp, StackAllocOp, HeapAllocOp, ConversionOp, TypeConversionOp, GlobalVariableInfo, AddressOfMemberOp, ComputeAddressOp, VirtualCallOp, DereferenceOp, DereferenceStoreOp, and `ExprResult`** have been migrated off raw `Type` storage. All classification call-sites have been migrated to `TypeCategory`; only 8 `== Type::` comparisons remain — 4 are the raw-`Type` **implementations** of TypeCategory helper functions in `AstNodeTypes_TypeSystem.h`/`AstNodeTypes.cpp` (intentional bridge definitions), and 4 are `Token::Type` (a completely separate nested enum in `Token.h`, unrelated to the type system). The thin raw-`Type` wrapper cleanup is further along: `get_integer_rank(Type)`, `get_floating_point_rank(Type)`, `promote_integer_type(Type)`, `promote_floating_point_type(Type)`, `get_common_type(Type, Type)`, `requires_conversion(Type, Type)`, and `buildConversionPlan(Type, Type)` are now deleted, leaving the larger bridge helpers (`get_type_size_bits(Type)`, `toIrType(Type)`, `categoryToType`, `typeToCategory`) as the remaining raw-`Type` surface. **Next step**: delete the remaining bridge functions (`categoryToType`, `typeToCategory`, `toIrType(Type)`), then the `Type` enum. Test baseline: **1756 pass / 105 expected-fail / 0 failures**.

**Remaining to delete the `Type` enum**: all active compiler data-structure fields and classification call-sites have been migrated. Only 8 `== Type::` uses remain: 4 in bridge function **bodies** in `AstNodeTypes_TypeSystem.h`/`AstNodeTypes.cpp` (the raw-Type overloads themselves), and 4 in `Token::Type` (a separate nested enum). The final steps are: (1) delete the remaining bridge functions (`categoryToType`, `typeToCategory`, `toIrType(Type)` and any raw-Type helpers that still survive), (2) delete the `Type` enum definition.
...
1. **Delete the remaining bridge functions** (`categoryToType`, `typeToCategory`, `toIrType(Type)`) — the thin primitive wrapper layer is now gone, so these are the main remaining raw-`Type` bridge surface.
2. **Delete any last raw-`Type` helper overloads** that still forward straight into TypeCategory implementations once the bridge-function callers are gone.
