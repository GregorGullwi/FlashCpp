...
**Status**: Milestones 1–7+ complete. **All IR Op structs, FunctionParam, StaticLocalInfo, StructMember, StructStaticMember, LazyStaticMemberInfo, ArrayAccessOp, CatchBeginOp, ThrowOp, StackAllocOp, HeapAllocOp, ConversionOp, TypeConversionOp, GlobalVariableInfo, AddressOfMemberOp, ComputeAddressOp, VirtualCallOp, DereferenceOp, DereferenceStoreOp, and `ExprResult`** have been migrated off raw `Type` storage. All classification call-sites have been migrated to `TypeCategory`; only 8 `== Type::` comparisons remain — 4 are the raw-`Type` **implementations** of TypeCategory helper functions in `AstNodeTypes_TypeSystem.h`/`AstNodeTypes.cpp` (intentional bridge definitions), and 4 are `Token::Type` (a completely separate nested enum in `Token.h`, unrelated to the type system). The first cleanup pass over thin raw-`Type` wrappers is also done: `get_integer_rank(Type)`, `get_floating_point_rank(Type)`, `promote_integer_type(Type)`, `promote_floating_point_type(Type)`, `get_common_type(Type, Type)`, and `requires_conversion(Type, Type)` were deleted, leaving the larger bridge helpers (`buildConversionPlan(Type,Type)`, `get_type_size_bits(Type)`, `toIrType(Type)`, `categoryToType`, `typeToCategory`) as the remaining raw-`Type` surface. **Next step**: delete the remaining thin raw-`Type` wrappers such as `buildConversionPlan(Type,Type)`, then the bridge functions (`categoryToType`, `typeToCategory`, `toIrType(Type)`), then the `Type` enum. Test baseline: **1756 pass / 105 expected-fail / 0 failures**.

**Remaining to delete the `Type` enum**: all active compiler data-structure fields and classification call-sites have been migrated. Only 8 `== Type::` uses remain: 4 in bridge function **bodies** in `AstNodeTypes_TypeSystem.h`/`AstNodeTypes.cpp` (the raw-Type overloads themselves), and 4 in `Token::Type` (a separate nested enum). The final steps are: (1) delete the remaining raw-`Type` compatibility wrappers (`buildConversionPlan(Type,Type)` and friends that still survive), (2) delete bridge functions (`categoryToType`, `typeToCategory`, `toIrType(Type)`), (3) delete the `Type` enum definition.
...
4. **Remaining raw-`Type` compatibility wrappers** (`buildConversionPlan(Type, Type)` and a small set of bridge overloads): the easy arithmetic wrappers (`get_integer_rank(Type)`, `get_floating_point_rank(Type)`, `promote_integer_type(Type)`, `promote_floating_point_type(Type)`, `get_common_type(Type, Type)`, `requires_conversion(Type, Type)`) are now deleted, but the larger compatibility surface still exists in conversion/codegen helpers and keeps a few callers from going fully TypeCategory-native.
...
1. **Delete the remaining raw-`Type` compatibility wrappers** — the trivial arithmetic wrappers are gone; the next wrapper to delete is `buildConversionPlan(Type, Type)`, followed by any remaining raw-`Type` helper overloads that only forward into the TypeCategory implementation.
2. **Delete bridge functions** (`categoryToType`, `typeToCategory`, `toIrType(Type)`) — once the wrappers above are gone, these functions have no callers.
3. **Delete the `Type` enum definition** — once bridge functions and raw-Type helpers are gone, delete `enum class Type { … }` and its file, completing the migration.
4. **Add a static_assert** after each bridge-function deletion to prevent regression.
