...
1. **Finish file-by-file local-variable cleanup** — prioritize `IrGenerator_Expr_Operators.cpp`, the remaining bridge/conversion sites in `IrGenerator_Stmt_Decl.cpp`, and the remaining `ConstExprEvaluator_Members.cpp` struct/array helper paths. `SemanticAnalysis.cpp` (0 sites), `IRConverter_ConvertMain.cpp` (0 raw `Type::` sites; all converted to `typeToCategory()` bridge), and all `Parser_Decl_*.cpp` (0 sites) are done.
