...
**Date**: 2026-03-26  
**Status**: Phase 1 (Option A) complete. Milestone 1 TODO 3 done. Milestone 2 TypeInfo helpers and sentinel done. Milestone 2.5 `is_type_alias_` flag done. Milestone 4 `isTemplatePlaceholder()` done. Milestone 6 (Option D Step 0) `gTypeInfo` accessor API done. Milestone 7 `TypeSpecifierNode::type()` comparison sites fully migrated. `TypeInfo::category()` method added (reads from `type_index_.category()`); `TypeInfo::isStruct()`, `isEnum()`, and `isTemplatePlaceholder()` updated to use `category()`; 'this' pointer TypeCategory corrected to `TypeCategory::Struct` in `IrGenerator_Visitors_TypeInit.cpp` and `Parser_Templates_MemberOutOfLine.cpp`; direct `type_ == Type::Struct` comparisons in `Parser_Decl_StructEnum.cpp`, `Parser_Expr_QualLookup.cpp`, and `Parser_Templates_Inst_ClassTemplate.cpp` replaced with `isStruct()`. Remaining: `TypeInfo::type_` effective-type reads (alias traversal, switch-based codegen dispatch in `IRConverter_ConvertMain.cpp`, `IrType.cpp`, etc.), the separate `buildConversionPlan` migration, and eventual `Type` deletion.  
...
| 7 | Add `TypeCategory`, embed in `TypeIndex`, migrate all `Type` usages — Option D Steps 1-3 (§5, Milestone 7) | 🔄 `TypeSpecifierNode::type()` sites migrated; `TypeInfo::category()` added; `isStruct()`/`isEnum()`/`isTemplatePlaceholder()` use `category()`; 'this' pointer categories fixed. Remaining: `TypeInfo::type_` effective-type reads (alias traversal + codegen switch dispatch) and `Type` deletion |
...
- [x] Migrate remaining `TypeSpecifierNode::type()` comparison sites to `TypeCategory::`/`TypeIndex` queries file by file. All `TypeSpecifierNode::type() == Type::X` comparisons across all files migrated in batches (batches 1–7): `IRTypes_Instructions.h`, `IrGenerator_Stmt_TryCatchSeh.cpp`, `IrGenerator.h`, `IrGenerator_Stmt_Control.cpp`, `IrGenerator_Lambdas.cpp`, `Parser_Expr_ControlFlowStmt.cpp`, `Parser_Templates_Inst_ClassTemplate.cpp`, `IrGenerator_MemberAccess.cpp`, `IrGenerator_Stmt_Decl.cpp`, `Parser_Statements.cpp`, `IrGenerator_Expr_Operators.cpp`, `IrGenerator_Expr_Conversions.cpp`, `IrGenerator_Expr_Primitives.cpp`, `Parser_Decl_StructEnum.cpp`, `Parser_Templates_Inst_Deduction.cpp`, `ConstExprEvaluator_Core.cpp`, and 15 more files. Zero `.type() == Type::X` comparison sites remain.
...
- [x] Update `TypeInfo::isStruct()`, `isEnum()`, `isTemplatePlaceholder()` to use `category()` instead of raw `type_` comparisons.
- [x] Migrate direct `type_ == Type::Struct` / `!= Type::Struct` comparisons in `Parser_Decl_StructEnum.cpp`, `Parser_Expr_QualLookup.cpp`, and `Parser_Templates_Inst_ClassTemplate.cpp` to use `isStruct()`.
- [ ] Migrate `TypeInfo::type_` effective-type reads used for alias traversal (`IrGenerator_MemberAccess.cpp`, `Parser_Expr_QualLookup.cpp`, `Parser_Templates_Params.cpp`, `AstNodeTypes_DeclNodes.h canonicalize_type_alias`) — these read the *resolved* type from `type_`, which differs from `category()` for TypeAlias entries; needs a `resolvedType()` helper or similar.
- [ ] Migrate codegen switch-dispatch sites that take a `Type` local variable and switch over it (`IRConverter_ConvertMain.cpp`, `IrType.cpp`, `ConstExprEvaluator_Core.cpp`) — add `TypeCategory` overloads and update callers.
