...
| `inferExpressionType(TemplateParameterReferenceNode)` | recover instantiated template-parameter value types not visible through local sema scope alone | narrowed bridge: function/ctor/dtor outer-template bindings now seed sema scope; parser fallback remains for contexts without that metadata |
