...
anonymous forms remain deferred. Qualified
type-ids resolve through the published nested `EntityId` and
`TemplateDeclId` rather than registry aliases: the former
`TemplateRegistry` owner-chain spelling aliases are deleted, and
`parse_type_specifier` resolves the primary through
`findClassTemplatePatternBySpelling` (identity first, registry fail-closed
fallback), with the shared Spec-stamp choke point taking the resolved primary
pattern. While a published
