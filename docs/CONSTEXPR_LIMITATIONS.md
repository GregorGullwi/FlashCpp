...
- Signed overflow rejection at the promoted operand width (`int`, `short`, etc.) under C++20 [expr.const]/4, including unary negation of the type minimum and `INT_MIN / -1`.
...
**Recent improvement:** Copy-list-initialization from same-type constexpr objects now preserves fully materialized member bindings (`T b{a};`). This fixes stored-iterator round-trips where an iterator-bearing object is copied through same-type copy-list-initialization before dereference (including nested member-call paths).
