...
- ⚠️ **Static-storage emission can still disagree with constexpr struct results that used constructor-based brace returns** — The constexpr evaluator now computes the right value for non-aggregate `return {a, b}` cases, but the emitted object for a `constexpr` global/static struct can still be wrong at runtime when that value came from a constexpr function and the constructor reorders or transforms its arguments. Repro:
  ...
  This was found while validating `test_constexpr_return_braced_init_ctor_ret0.cpp`: the `static_assert` succeeds, so the constexpr evaluator has the correct member bindings, but the runtime object emitted for `p` does not preserve that constructor-materialized state. **Workaround:** keep the validation in `static_assert`, or avoid depending on such globals/statics at runtime when the constructor changes member values/order. **TODO:** audit static-storage struct initialization/codegen for constexpr function-call results so emitted init data is derived from the fully materialized `EvalResult::object_member_bindings`, not from the original brace-order arguments or an aggregate-style fallback.
