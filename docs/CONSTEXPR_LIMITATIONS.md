...
8. ⚠️ **Bare `new T` / `new T[n]` now reject scalar/element reads, but aggregate/member tracking is still incomplete** — FlashCpp now distinguishes bare default-initialization from value-initializing forms such as `new T()`, `new T{}`, and `new T[n]{}` for supported shapes. Reading a scalar fundamental object allocated with bare `new T` or an element of a bare `new T[n]` array is rejected during constexpr evaluation as an indeterminate read. This closes the false-accept cases for patterns like `int* p = new int; return *p;` and `int* p = new int[3]; return p[0];`. Remaining work: aggregate/object default-initialization still does not track indeterminate state per member, so the full C++20 rule for bare `new T` on aggregate/class objects is not enforced yet. Value-initializing forms still zero-initialize fundamental scalars.
