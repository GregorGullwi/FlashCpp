...
- ✅ **Constructor default arguments now evaluated in constexpr contexts** *(Implemented)* — `find_matching_constructor` (`src/ConstExprEvaluator_Members.cpp`) previously returned `nullptr` when the call had fewer explicit arguments than the constructor's total parameter count, even when the missing parameters all had default values. Fixed by adding a fallback to `resolve_constructor_overload_arity` (which already accounts for default-arg arity). Additionally, `bind_evaluated_arguments` (`src/ConstExprEvaluator_Core.cpp`) previously only bound explicit arguments; it now also evaluates and binds default values for any remaining parameters. Together these fixes make `Pair g(5)` correct when `Pair` has `constexpr Pair(int a, int b = 7)`. Covered by `tests/test_global_ctor_default_arg_ret0.cpp`.
  - ⚠️ **`bind_pre_evaluated_arguments` does not bind default values** — The sibling function `bind_pre_evaluated_arguments` (`src/ConstExprEvaluator_Core.cpp:3327-3349`) was **not** updated with equivalent default-binding logic because it lacks an `EvaluationContext&` parameter needed to call `evaluate()` on default-value AST nodes. Code paths that call `find_matching_constructor` (which now returns constructors callable via default params) and then use `bind_pre_evaluated_arguments` — specifically array element construction in `evaluate_array_subscript_member_access` (`src/ConstExprEvaluator_Members.cpp:3963-3968`) and nested member access materialization (`src/ConstExprEvaluator_Members.cpp:3601-3608`) — will leave defaulted parameters unbound if `evaluated_arguments.size() < parameters.size()`. **Impact:** Narrow edge case — requires a constexpr array whose elements are constructed with fewer arguments than the constructor's total parameter count (relying on defaults), combined with member access on the array element that references a defaulted parameter. **TODO:** Add `EvaluationContext&` parameter to `bind_pre_evaluated_arguments`, add a default-binding loop mirroring `bind_evaluated_arguments`, and update all call sites.
