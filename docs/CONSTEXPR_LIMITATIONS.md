...
For aggregates / implicit-default-constructor cases, the evaluator still zero-fills members (applying default member initializers where present). For non-aggregate types with user-declared constructors, a missing default constructor is now rejected instead of silently defaulting to zeroed members.
...
8. ⚠️ **Bare `new T` now rejects scalar reads, but aggregate/member tracking is still incomplete** — FlashCpp now distinguishes bare `new T` from value-initializing forms such as `new T()`, so reading a scalar fundamental object allocated with bare `new T` is rejected during constexpr evaluation as an indeterminate read. This closes the false-accept case for patterns like `int* p = new int; return *p;`. Remaining work: aggregate/object default-initialization still does not track indeterminate state per member, so the full C++20 rule for bare `new T` on aggregate/class objects is not enforced yet. `new T()` remains value-initialization and still zero-initializes fundamental scalars.
   **Follow-up items for indeterminate-value tracking:**
   - **Array `new T[n]` ignores `has_value_init`** — The array allocation path in `evaluate_new_expression` (`src/ConstExprEvaluator_Core.cpp`) always calls `make_default_init()` for each element regardless of `has_value_init()`. This means `new int[5]` (bare, no braces) still zero-initializes elements instead of marking them indeterminate. The parser already sets `has_value_init` correctly for arrays (`has_array_brace_init`), but the evaluator does not consume it yet. **TODO:** Branch on `new_expr.has_value_init()` in the array path, producing `EvalResult::indeterminate()` elements for bare `new T[n]` and `make_default_init()` elements for `new T[n]{}`.
   ...
   - **Parser does not handle `new T{}` brace-init syntax for non-array new** — After parsing the type in a `new`-expression, the parser (`src/Parser_Expr_PrimaryUnary.cpp`) checks for `[` (array), `(` (constructor args), then falls through to bare `new T`. There is no check for `{` to handle `new T{}` or `new T{args}`. Per C++11/C++20, `new int{}` is value-initialization and should produce `has_value_init=true`, but it currently either misparsers or leaves the `{}` tokens unconsumed. This is a pre-existing parser limitation. **TODO:** Add a `{` branch in the non-array new-expression parser that parses brace-init arguments and sets `has_value_init=true`.
...
- ✅ **Default constructor invocation for uninitialized local struct variables in constexpr functions** *(Implemented)* — Declaring a local struct variable without an explicit initializer (e.g., `Counter c;`) now invokes the default constructor, including constructors with a body. Both member-initializer-list and constructor-body styles work. Aggregate / implicit-default-constructor cases still zero-fill with default member initializers where appropriate, while non-aggregate types with user-declared constructors now correctly require a matching default constructor.
...
- ⚠️ **Enforce C++20 aggregate-initialization rules** — In C++20, a type with any
  ...
  shared helper.
- ⚠️ **Consolidate remaining find→bind→materialize call sites onto `try_materialize_struct_from_ctor_args`** — The shared helper `try_materialize_struct_from_ctor_args` (`src/ConstExprEvaluator_Members.cpp`) centralizes the `find_matching_constructor` → `bind_evaluated_arguments` → `materialize_members_from_constructor` sequence. `extract_object_members` now delegates to it too, including the path used by member calls on global constexpr objects initialized through constructor calls/default arguments (covered by `tests/test_constexpr_global_ctor_default_arg_member_call_ret0.cpp`). One remaining site still inlines the pattern:
...
- ✅ **`return {x, y}` in struct-returning constexpr functions now honors constructors** *(Implemented)* — The return-statement handler in `evaluate_statement_with_bindings` now mirrors the variable-declaration and `new`-expression paths: if the struct return type has user-defined constructors, `return {a, b}` routes through constructor matching (`try_materialize_struct_from_ctor_args`) instead of unconditionally binding members as an aggregate. This fixes non-aggregate cases where the constructor reorders, transforms, or validates its arguments, so `return {a, b}` now behaves consistently with `Type result(a, b); return result;` during constexpr evaluation.
  **Implicit contract: `try_materialize_struct_from_ctor_args` must set `object_type_index`** — The new return-statement constructor path at `src/ConstExprEvaluator_Core.cpp:3400-3403` returns `*ctor_result` directly from `try_materialize_struct_from_ctor_args` without explicitly setting `object_type_index` on the result (unlike the aggregate-init path at line 3414 which sets it manually). This relies on `try_materialize_struct_from_ctor_args` (`src/ConstExprEvaluator.h:527-532`) setting `object_type_index` internally on the returned `EvalResult`. This is a proven contract — the `new`-expression path (`src/ConstExprEvaluator_Core.cpp:1566-1574`) uses the same pattern: it pre-sets `object_result.object_type_index` at line 1557 but then overwrites it entirely with `*ctor_result` at line 1574, so the function must be setting the field. The variable-declaration path (`src/ConstExprEvaluator_Core.cpp:3515-3523`) also returns `*ctor_result` directly. All three call sites work correctly in practice, and `tests/test_constexpr_return_braced_init_ctor_ret0.cpp` validates member access on the result (which requires a valid `object_type_index`). However, this contract is implicit — if `try_materialize_struct_from_ctor_args` were ever refactored to omit `object_type_index`, all three sites would silently produce objects that downstream member-function chains and nested member access cannot identify. **TODO:** Add an assertion or post-condition comment in `try_materialize_struct_from_ctor_args` documenting that the returned `EvalResult` must have `object_type_index` set to the passed `type_index` parameter.
- ✅ **Static-storage emission now preserves constexpr struct results that used constructor-based brace returns** *(Implemented)* — When a `constexpr` global/static struct is initialized from a constexpr function result, the static-storage codegen path now packs the fully materialized `EvalResult::object_member_bindings` into emitted init bytes instead of collapsing the whole object through the scalar `evalToValue()` fallback. This keeps runtime global/static objects consistent with the compile-time constexpr result, including non-aggregate return types whose constructors reorder or transform their arguments. Repro that now works:
  ...
  **Known limitation — float/double arrays in `array_values`:** The `packStructEvalResultIntoInitData` helper in `src/IrGenerator_Stmt_Decl.cpp` has two array-element paths: `array_elements` (a `vector<EvalResult>`, handled via `evalResultMemberToRaw` with correct IEEE-754 `memcpy` bit-casting) and `array_values` (a `vector<int64_t>`, written as raw integers). The `array_values` fallback path casts `int64_t` directly to `unsigned long long` without IEEE-754 conversion, so if a `float`/`double` array member were represented via `array_values`, the emitted bit pattern would be incorrect. In practice this is unreachable today because the constexpr evaluator populates `array_values` via `as_int()` (`src/ConstExprEvaluator.h:855`, `:1770`), which truncates floating-point values to integers — so float arrays already lose precision upstream before reaching the codegen layer. A full fix would require two changes: (1) upstream in the evaluator, store float/double array elements in `array_elements` (which preserves the `double` variant) instead of `array_values`; (2) downstream in `packStructEvalResultIntoInitData`, route the `array_values` path through `evalResultMemberToRaw` using the member's type. **TODO:** Fix the upstream evaluator to use `array_elements` for float/double arrays, then update the `array_values` fallback in `packStructEvalResultIntoInitData` to use `evalResultMemberToRaw` for defense-in-depth.
