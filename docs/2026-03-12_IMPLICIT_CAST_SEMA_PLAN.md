...
3. ✅ **`emitConversionOperatorCall` missing assertion for non-struct source values.** *(fixed in Phase 21)* The `else { throw InternalError("emitConversionOperatorCall: source value is neither StringHandle nor TempVar"); }` branch in `IrGenerator_MemberAccess.cpp` covers non-`StringHandle`/non-`TempVar` values (including `unsigned long long` and `double` literals), throwing instead of silently emitting a zero-arg `CallOp`.
...
5. ✅ **`Struct→FunctionPointer` annotation now permitted.** *(fixed in Phase 5)* `structHasConversionOperatorTo` in `SemanticAnalysis.cpp` calls `getTypeName(to_desc.base_type)` for non-struct targets. `getTypeName(Type::FunctionPointer)` returns `""` (empty), so the helper returns `false` immediately — preventing spurious `UserDefined` slots for `Struct→FunctionPointer` pairs.
6. ✅ **`ir_type` now set on `this` arg.** *(done in Phase 21)* `emitConversionOperatorCall` sets `this_arg.ir_type = toIrType(source.type)` on both the `StringHandle` (address-of) and `TempVar` branches. The old duplicated code left `ir_type` default-initialized.
...
### Phase 23 ✅: `operator bool()` contextual conversion via sema UserDefined annotation

**Goal:** Fix `applyConditionBoolConversion` to actually call `operator bool()` when
a struct is used in a contextual-bool position (`if`, `while`, `for`, `do/while`,
ternary condition, logical `!`). Previously it relied on the backend TEST instruction
testing raw struct bits, which gives accidentally-correct results only when the struct's
first-member int value matches the logical bool (e.g., `Flag{0}` → false, `Flag{1}` →
true). A struct like `Flag{0xDEAD}` where `operator bool()` returns `value == 42`
would be incorrectly truthy.

**Root cause:** `applyConditionBoolConversion` checked `BooleanConversion` and
`PointerConversion` sema annotation kinds, but not `UserDefined` (the kind emitted
by sema for struct → bool via `operator bool()`).

**Implementation:**
- `src/IrGenerator_Expr_Conversions.cpp` (`applyConditionBoolConversion`):
  Added a `UserDefined && from_desc.base_type == Type::Struct` branch in the sema
  annotation check: looks up `TypeInfo` via `source_type_id.type_index`, calls
  `findConversionOperator(..., Type::Bool, TypeIndex{})`, and calls
  `emitConversionOperatorCall` (target type: `Type::Bool`, target size: 8 bits).
  Sets `sema_applied_bool_conv = true` immediately when `UserDefined && Type::Struct` is
  detected (before the `findConversionOperator` call) to suppress a redundant duplicate
  lookup in the fallback path. Sema has already verified operator existence via
  `structHasConversionOperatorTo`, so the lookup should always succeed.
  Added a matching fallback path (after the floating-point fallback) for the case
  where sema is absent: checks `condition.type == Type::Struct` and attempts
  `findConversionOperator(..., Type::Bool, TypeIndex{})`.
- `tests/test_conv_op_bool_contextual_ret0.cpp`: New test with `Flag{0xDEAD}` whose
  raw bits are truthy but `operator bool()` returns false. Covers `if`, `while`,
  ternary, and logical `!` contextual-bool sites. Also covers `BoxedDouble` whose
  `operator bool()` compares against a threshold.

**Test result:** 1686 pass, 98 expected-fail (was 1685/98 before this phase).
...
- Direct-call and member-call argument reference-binding classification is now sema-owned, including per-argument direct-bind vs temporary-materialization decisions and pre-bind standard-conversion metadata for reference parameters.
- For those direct/member explicit-argument call paths, codegen now consumes the sema reference-binding decision through temporary materialization and end-of-full-expression cleanup for materialized struct temporaries. Broader reference-binding coverage outside direct/member explicit-argument call paths is still follow-up work.
- ✅ **Member-call fallback skips legacy reference handling when sema ref-binding returns nullopt.** *(fixed)* Added `sema_ref_binding_applied` flag in `IrGenerator_Call_Indirect.cpp` to track whether `tryApplySemaCallArgReferenceBinding` actually succeeded. Changed all three legacy reference-handling guard conditions from `(!sema_ref_binding || !sema_ref_binding->is_valid())` to `!sema_ref_binding_applied`, so the legacy paths remain active when sema tried but failed to apply ref binding. Regression coverage: `tests/test_sema_ref_binding_applied_ret0.cpp`.
- ✅ **Reference member access in Load context missing Dereference.** *(fixed)* `IrGenerator_MemberAccess.cpp`: after emitting the `MemberAccess` instruction to load the stored pointer for a reference member (e.g. `const int& val`), the code path for Load context (reading the value) was missing an `emitDereference` call, causing the raw 64-bit pointer address to be used as the value (and then truncated to the pointee type). Fix: added a block after the LValueAddress path that emits a `Dereference` instruction using `member->referenced_size_bits` as the pointee size and marks the result as an indirect LValue, matching the identical pattern used for reference identifier variables in `IrGenerator_Expr_Primitives.cpp`. Regression coverage: `tests/test_ref_member_load_ret0.cpp`.
- ✅ **Constructor argument pre-bind type conversion skipped for reference parameters.** *(fixed)* `applyConstructorArgConversion` in `IrGenerator_Expr_Conversions.cpp` had an unconditional early return for `is_reference()` / `is_rvalue_reference()` parameter types, which prevented sema-annotated type conversions (e.g. `int → const double&`) from being applied before the argument is materialized as a temporary. Fix: removed the early return so the sema-annotation check and standard-conversion fallback run for reference parameters as well; same-type cases are still skipped correctly because `buildConversionPlan(T, T)` returns no-op. Regression coverage: Part 4 of `tests/test_ref_member_load_ret0.cpp`.
...
- ✅ **ref-to-ref binding (`const T& r2 = r1` where r1 is T&) produced wrong value.** *(fixed)* `IRConverter_ConvertMain.cpp` `handleVariableDecl` TempVar path: when `r1` is evaluated in `LValueAddress` context the resulting `addr_temp` carries `LValueInfo::Kind::Indirect` metadata and a `size_in_bits` equal to the *pointee* size (e.g. 32 for `int&`), not 64. The old `is_likely_pointer` guard (`size==64 && isIrIntegerType`) therefore missed it and emitted LEA (address of `r1`'s stack slot) instead of MOV (copy of `r1`'s stored pointer). Fix: check `getTempVarLValueInfo(temp_var)->kind == LValueInfo::Kind::Indirect`; if set, always use MOV. Regression coverage: `tests/test_ref_binding_ref_to_ref_ret0.cpp`.
- ✅ **`const long long& r = get_ll()` caused SIGSEGV.** *(fixed)* Same TempVar path in `handleVariableDecl`: a 64-bit integer function-return value (e.g. `long long`) matched the old `isIrIntegerType` branch of `is_likely_pointer`, causing MOV to treat the value (42) as a pointer address and segfault on first dereference. Fix: removed `isIrIntegerType` from the fallback `is_likely_pointer` condition; only `isIrPointerLikeType` and `isIrStructType` qualify for unconditional MOV in the non-indirect-lvalue path, so integer data values now correctly take the LEA path (materialising a stack temporary). Regression coverage: `tests/test_ref_binding_const_ref_prvalue_ret0.cpp`.
