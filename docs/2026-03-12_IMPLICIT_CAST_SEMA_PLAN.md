...
- **Member-call fallback skips legacy reference handling when sema ref-binding returns nullopt.** In `IrGenerator_Call_Indirect.cpp`, when `sema_ref_binding->is_valid()` is true, the argument is evaluated early and `tryApplySemaCallArgReferenceBinding` is attempted. If it returns `std::nullopt` (e.g., the `binds_directly()` path when the argument is neither an identifier with a found declaration nor a TempVar with lvalue/xvalue metadata — see `IrGenerator_Expr_Conversions.cpp:2579`), execution falls through to the main argument-handling code. However, the legacy reference-handling guards at three sites (lines ~1471, ~1509, ~1564) all check `(!sema_ref_binding || !sema_ref_binding->is_valid())`, which evaluates to `false`. This causes the code to skip the old reference-binding paths and fall into "Regular pass by value" branches, which push the argument via `toTypedValue()` **without setting `ref_qualifier`**. The IR converter relies on `ref_qualifier` to distinguish pass-by-reference from pass-by-value for `CallOp` arguments. **Fix:** Track whether `tryApplySemaCallArgReferenceBinding` actually succeeded (e.g., a `bool sema_ref_binding_applied` flag), and change the guard conditions to `(!sema_ref_binding_applied && param_type && (param_type->is_reference() || ...))` so the legacy paths remain active when sema's attempt failed. The direct-call path in `IrGenerator_Call_Direct.cpp` does not have this issue because it evaluates the argument once before the sema binding attempt and reuses the result in fallback paths without suppressing the legacy reference-handling guards.
