...
- `EvaluationContext` now has explicit sema-backed modes for both parser-owned and standalone evaluation: `attachSemaOnly(...)` names sema-only contexts, `requireSema(...)` / `requireParserAttachedSema(...)` centralize the remaining invariants, `requireParserOwnedSema(...)` now also verifies that `parser` and `sema` come from the same owner, `AstToIr::tryEvaluateAsConstExpr(...)` now uses `makeEvalContext(...)`, and `ConstExpr::Evaluator::evaluate(...)` hard-fails immediately on any bare sema-less context that still slips through.
...
- `EvaluationContext` now also has explicit parser-owned and sema-only constructors, and both the shared helpers and the remaining simple parser-owned `*this` call sites now use those constructors directly instead of constructing a raw context and attaching semantic ownership afterward. The last raw constructor+attach setup is the `const Parser&` sema helper, which needs a separate const-correctness contract pass rather than another mechanical swap.

Remaining Stage 6 work:

- `EvaluationContext` still has a raw symbol-table constructor for the remaining `const Parser&` setup path and any future genuinely mode-delayed construction; deciding whether that last parser helper should become `Parser&` or gain a const-safe constructor is now narrow cleanup rather than a broad API gap
