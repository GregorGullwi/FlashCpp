...
- `EvaluationContext` now has explicit sema-backed modes for both parser-owned and standalone evaluation: `attachSemaOnly(...)` names sema-only contexts, `requireSema(...)` / `requireParserAttachedSema(...)` centralize the remaining invariants, `AstToIr::tryEvaluateAsConstExpr(...)` now uses `makeEvalContext(...)`, and `ConstExpr::Evaluator::evaluate(...)` hard-fails immediately on any bare sema-less context that still slips through.
...
- constexpr `sizeof(array[index])` now falls back through `global_symbols` before expression-type decay, so inferred global arrays keep the correct element size during codegen folding, and `sizeof(S{0})` on a struct prvalue now stays locked in as a passing regression test instead of an expected failure.

Remaining Stage 6 work:

- `EvaluationContext` construction still starts from a raw nullable shape before callers choose `attachParserOwnedSema(...)` or `attachSemaOnly(...)`; replacing that with mode-specific constructors would be optional cleanup now rather than a live semantic gap
