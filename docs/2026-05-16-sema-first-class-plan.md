...
- `EvaluationContext` now also has explicit parser-owned and sema-only constructors, and the shared `AstToIr`, parser-template helper, and sema helper setup sites now use those constructors directly instead of constructing a raw context and attaching semantic ownership afterward. The remaining raw-shape sites are now mostly the runtime-conditional or `const Parser&` cases that need a separate contract cleanup rather than a mechanical swap.

Remaining Stage 6 work:

- `EvaluationContext` still has a raw symbol-table constructor for the few runtime-conditional or `const Parser&` setup paths that cannot be rewritten mechanically yet; retiring those last attach-after-construction sites is now narrow cleanup rather than a broad API gap
