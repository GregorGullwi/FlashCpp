...
- `AstToIr::visitRangedForStatementNode(...)` now consumes sema query-state range typing plus sema-owned `resolved_range_type`; codegen-side member-access range-type reconstruction and parser fallback guards are removed for normalized bodies.
