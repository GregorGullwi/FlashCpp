...
- **Fallback 1 (`resolveStructTypeFromReceiverNode` in `IrGenerator_Call_Indirect.cpp`)**: confirmed dead — no test fires the parser fallback. `allow_parser_type_fallback` is now gated before identifier/static-cast recovery and returns `std::nullopt` immediately when fallback is disallowed; callers handle `nullopt` gracefully.
