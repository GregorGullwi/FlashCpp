...
- **Ordinary direct-call fallback reasons (`SemanticAnalysis.cpp`, `IrGenerator_Call_Direct.cpp`)**: the prior coarse `unresolved_call_args_` escape hatch has been replaced with explicit sema-recorded direct-call fallback reasons. Normalized ordinary direct calls now permit codegen recovery only when sema recorded a specific compatibility reason for that call site; they no longer treat every analyzed-absent direct-call miss as equally recoverable. The codegen-side direct-call fallback block has also been reduced again: qualified-owner member lookup, global qualified-static-member scanning, and template-type-parameter-qualified static-call recovery have been deleted from `IrGenerator_Call_Direct.cpp` because sema-side owner/member recovery already covers those cases. This narrows the sema/codegen contract toward eventual full fallback deletion while preserving the remaining compatibility path for dependent base-template qualified calls and other residual legacy cases.
