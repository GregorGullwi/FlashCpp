...
- **Return-statement converting-constructor codegen fallback (probe 2026-05-24)**: Probe-verified the `!sema_applied_conversion && is_struct_type(return_category) && operands.type_index != return_type_spec.type_index()` block in `visitReturnStatementNode`. The probe fired for **6 template tests** involving nested-struct same-type returns (e.g. `return Entry{}`, `return __tmp` for template nested types). Root cause: template instantiation creates a `TypeIndex` mismatch between the return expression (template pattern's nested type) and the function return type (instantiated nested type). Sema correctly emits no conversion annotation (same logical type), so the codegen fallback legitimately handles the type-index reconciliation by resolving the copy/value constructor. The fallback is retained with an explanatory comment; the root-cause type-index identity fix is tracked in `KNOWN_ISSUES.md`.
