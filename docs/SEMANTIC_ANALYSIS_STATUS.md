...
- **Ordinary direct-call fallback burn-down should continue next**: the next highest-value cleanup is eliminating the remaining explicit direct-call fallback reasons one by one until even sema’s compatibility bookkeeping can disappear and non-normalized direct-call recovery can be deleted outright. See `docs/2026-04-04-codegen-name-lookup-investigation.md` Phase 1 for the detailed roadmap and exit criteria.
