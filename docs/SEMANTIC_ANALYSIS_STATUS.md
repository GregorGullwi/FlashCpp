...
- **Same-type constructor preference hardening (IrGenerator_Stmt_Decl.cpp)**: `getSameTypeConstructorPreference(...)` no longer swallows `InternalError` from sema-owned overload-argument typing for identifier-based same-type constructor selection in sema-normalized bodies. This removes a silent escape hatch back to codegen identifier/symbol-table recovery in that normalized path, while preserving existing non-identifier compatibility behavior.
