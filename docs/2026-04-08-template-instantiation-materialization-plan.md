...
3. **Explicit dependent-placeholder state everywhere**
   ...
   - Activity note: a hard-fail probe of the invalid-`type_index` dependent-placeholder path in `src\Parser_Templates_Params.cpp` broke `comparison_operators_ret1.cpp`, `float_comparisons_ret1.cpp`, `spaceship_basic_ret253.cpp`, `test_const_member_with_param_ret255.cpp`, `test_decltype_function_template_base_ret42.cpp`, `test_float_cmp_ret6.cpp`, `test_operator_overload_template_ret40.cpp`, `test_spaceship_3member_ret15.cpp`, `test_spaceship_custom_ordering_ret3.cpp`, `test_spaceship_multi_member_ret8.cpp`, `test_spaceship_precedence_ret42.cpp`, and `test_spaceship_synthesized_ret8.cpp`. This unresolved-placeholder path is still active and must be replaced by explicit placeholder metadata before removal.
   - Activity note: wiring `parse_member_function_template(...)` in `src\Parser_Templates_Function.cpp` to preserve `registered_type_index()` on type template parameters is safe and now done, but it did not change that remaining invalid-`type_index` cluster, so there are still other producer paths that leave dependent placeholder types without canonical indices.
   ...
   - Activity note: the narrower canonical-full-name path in `src\Parser_Templates_Params.cpp` is also still active; hard-failing the old `type_name = full_type_name` recovery broke `tests\test_variable_template_in_enable_if_ret0.cpp`, so parsed template-argument types can still arrive without a usable token spelling in current dependent-variable-template cases.
...
6. **ExpressionSubstitutor role clarification**
   ...
   - Activity note: the analogous lazy static-member general substitution path in `src\Parser_Templates_Lazy.cpp` is still active; hard-failing the `ExpressionSubstitutor` step broke a broad cluster including `template_ttp_static_constexpr_member_ret0.cpp`, `test_alias_base_static_member_ret0.cpp`, `test_integral_constant_simple_ret30.cpp`, `test_ratio_lazy_static_member_ret0.cpp`, `test_template_static_member_initializer_scalar_brace_ret42.cpp`, and `test_type_traits_dependent_member_nttp_ret42.cpp`.
   - Activity note: the later `rebindStaticMemberInitializerFunctionCalls(...)` step in the same lazy path is independently active; hard-failing it broke `template_ttp_static_constexpr_member_mixed_args_ret0.cpp`, `test_alias_template_nested_member_value_ret42.cpp`, `test_integral_constant_comprehensive_ret100.cpp`, `test_ratio_equal_deferred_base_ret1.cpp`, `test_template_template_forward_decl_definition_ret0.cpp`, and `test_type_traits_patterns_ret42.cpp`.
   - Activity note: the later dependency pre-instantiation scan in the same lazy path is also independently active; hard-failing `collectLazyStaticDependencies(...)` / `instantiateLazyStaticMember(...)` broke the same broad lazy-static-member cluster.
...
8. **Function template declaration/body reconstruction**
   ...
   - Activity note: forcing return-type reparse whenever declaration source was available shrank the remaining failure set to `concept_abbreviated_ret0.cpp`, `test_abbrev_tmpl_trailing_decltype_ret0.cpp`, `test_constrained_auto_param_ret0.cpp`, `test_constrained_auto_u64_codegen_ret1.cpp`, `test_eh_function_try_block_auto_param_ret0.cpp`, plus a runtime crash in `test_ns_qualified_template_call_ret42.cpp`. This means the current gate is too conservative, but a smaller abbreviated/auto/function-try-block cluster still blocks full fallback removal.
