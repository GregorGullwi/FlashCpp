...
- `src\IrGenerator_Call_Direct.cpp` / `src\SemanticAnalysis.cpp` — sema no longer exposes a coarse `unresolved_call_args_` bit for direct calls. Qualified-owner member lookup, qualified-static-member scanning, template-parameter-qualified static call recovery, dependent base-template qualified-call remap, stale precomputed pattern-owner remap, declaration-address overload rescans, and mangled-symbol retry lookup have all been removed from direct-call lowering. Sema-normalized direct calls now hard-fail during annotation if they still need a recorded compatibility reason or lack a resolved target, and sema now covers the first exposed follow-up gaps itself (qualified namespace lookup, ADL-only hidden-friend candidate augmentation, reuse of concrete parser-instantiated direct-call targets after dependent-unqualified POI failure, direct consumption of parser-stored definition-bound call records, definition-time filtering for namespace-qualified function templates without filtering dependent ADL completion, and exclusion of indirect calls from the invariant). A 2026-05-26 follow-up narrowed the sema-side compatibility path again by consuming definition-record mangled identity before broad lookup and by resolving qualified-owner namespaces from the source spelling directly, so relative and leading-`::` qualifiers no longer depend on manual forced-global splitting. A subsequent follow-up now resolves concrete postfix `operator()` member calls from receiver type information before creating the direct-call node when the receiver/call arguments are already concrete, and the shared postfix-call parser finalization path now covers zero-arg/default-arg forms as well, narrowing the remaining `ReceiverMemberRecovery` surface instead of preserving it as a routine parser placeholder path. The non-terminal `GlobalMangledLookupMiss` reason has been removed because it was always erased on success or overwritten on terminal failure. The remaining compatibility-reason contract is therefore only the non-normalized/synthesized transition boundary, not an active codegen-side direct-call recovery chain.
- Standard C++20 follow-up for this cluster: remove the remaining reason-coded compatibility branches entirely so ordinary direct-call acceptance depends only on standard lookup/overload rules captured by sema, never on transitional recovery bookkeeping.
...
1. `src\Parser_Templates_Inst_ClassTemplate.cpp` — unresolved template-default catch-all
    ...
    - Follow-up narrowing (2026-05-10): the class-template default-fill path now retries unresolved type/non-type defaults through the shared `tryAppendDefaultTemplateArg(...)` materialization pipeline before entering the local catch-all. This reuses the dependent-default reparse/substitution flow already used by function-template deduction/member-function instantiation and narrows residual type/default fallback traffic.
    - Follow-up narrowing (2026-05-10): class-template default retries now run with the template declaration namespace as reparse context, matching deduction paths. Unresolved **type** defaults now hard-fail with `CompileError` instead of injecting a `void` placeholder.
    - Validation: `test_template_template_default_ret42.cpp`, `test_expr_subst_noexcept_wrap_ret0.cpp`, `test_template_spec_outofline_default_arg_ret42.cpp`, `test_template_spec_outofline_default_arg_namespaced_ret42.cpp`, `test_variable_template_nttp_base_class_ret0.cpp`, and the full Linux suite (2263 regular / 170 expected-fail) passed.
    ...
    - Conclusion: the catch-all no longer covers template-template defaults, and unresolved type defaults no longer silently inject placeholders. Remaining fallback traffic is unresolved non-type defaults until those substitution gaps are fixed at the root.
...
2. `src\Parser_Templates_Inst_ClassTemplate.cpp` — unresolved class-template type argument falls back to using the original `TypeSpecifierNode` as-is
    ...
    - Follow-up narrowing (2026-05-05): the primary deferred-template-base type-argument path now runs `substitute_template_parameter(...)` and then `materializeDeferredBasePlaceholderIfNeeded(...)` before falling back to the original `TypeSpecifierNode`. This root-fixes ordinary member-alias/deferred-base substitutions such as `typename choose_base<T>::type` without changing the still-needed dependent-placeholder behavior in specialization/pattern flows.
    - Validation: `tests\test_deferred_base_member_alias_type_arg_ret0.cpp`, `tests\test_nttp_base_class_substitution_ret0.cpp`, `tests\test_pack_expansion_base_class_ret0.cpp`, `tests\test_pack_expansion_member_type_base_ret0.cpp`, `tests\test_ratio_equal_deferred_base_ret1.cpp`, `tests\test_std_swap_enable_if_alias_base_ret0.cpp`, `tests\test_type_traits_dependent_member_nttp_ret42.cpp`, and the full Linux suite (2277 regular / 156 expected-fail) passed after the change.
    - Conclusion: this fallback class is narrower. Primary deferred-base instantiation now preserves more authoritative type metadata up front, while residual passthrough still remains for unresolved dependent-member and pattern-owned placeholder cases.
...
10. `src\Parser_Templates_Inst_ClassTemplate.cpp` — static-member initializer substitution fallback
    ...
    - Follow-up narrowing: eager instantiated static-member initialization now uses `substituteTemplateParameters(...)` as the primary initializer rewrite, then feeds the result into the shared static-member normalization pipeline (`rebindStaticMemberInitializerFunctionCalls(...)`, deferred call instantiation, constexpr normalization). The previous hand-written special cases plus trailing general `ExpressionSubstitutor` pass were removed from this main path.
    - Validation: the documented static-member cluster, the representative lazy-static/type-trait follow-on cases, and the full Windows suite passed after the change.
    - Conclusion: the main instantiated static-member initializer path no longer depends on a separate catch-all substitution pass. Remaining static-member fallback traffic, if any, is now limited to other residual paths rather than the primary eager instantiation flow.
...
- the codegen-side arity-based `operator()` lookup, the `scoped_overloads` /
  `gSymbolTable_overloads` single-overload recovery branches, the
  precomputed-mangled `gSymbolTable` pointer-equality scan, the current-struct
  + base-class member-by-name recovery, and the qualified-static-member
  struct-iteration recovery in `IrGenerator_Call_Direct.cpp` (audit §1, the
  "pointer / symbol-table / precomputed-mangle / current-struct" cluster) were
