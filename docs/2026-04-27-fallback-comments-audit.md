...
9. `src\Parser_Templates_Lazy.cpp` — lazy static-member general substitution path
    ...
    - Validation: the representative lazy-static cluster above passed after this sequencing change.
    - Conclusion: lazy static-member initialization still depends on the three steps, so none are removable yet. Their role is narrower and more explicit: general ExpressionSubstitutor performs remaining expression substitution, then all initializer paths share the same materialization/normalization tail. The dependency scan no longer owns a recursive expression walk; it uses the shared AST traversal helper, including explicit call template arguments.
