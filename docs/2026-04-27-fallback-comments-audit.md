...
- `src\IrGenerator_Call_Direct.cpp` / `src\SemanticAnalysis.cpp` — the remaining large direct-call recovery block is now explicitly gated behind `allow_lookup_recovery`, and sema no longer exposes a coarse `unresolved_call_args_` bit for it. Qualified-owner member lookup, qualified-static-member scanning, template-parameter-qualified static call recovery, dependent base-template qualified-call remap, and stale precomputed pattern-owner remap have been removed from codegen; the residual block is now limited to the earlier non-normalized lookup shims. Instead, sema records explicit direct-call fallback reasons per call site, so sema-normalized bodies only run that codegen-side chain for known compatibility cases.
