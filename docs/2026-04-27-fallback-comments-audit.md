...
- `src\IrGenerator_Call_Direct.cpp` / `src\SemanticAnalysis.cpp` — the remaining large direct-call recovery block (dependent qualified call remap, qualified-static-member recovery, template-parameter-qualified static call recovery) is now explicitly gated behind `allow_lookup_recovery`, and sema no longer exposes a coarse `unresolved_call_args_` bit for it. Instead, sema records explicit direct-call fallback reasons per call site, so sema-normalized bodies only run that codegen-side chain for known compatibility cases.
